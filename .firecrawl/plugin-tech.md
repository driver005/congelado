[NullDeref](https://nullderef.com/ "NullDeref (Alt + H)")

- [About](https://nullderef.com/about "About")
- [Series](https://nullderef.com/series "Series")
- [Search](https://nullderef.com/search "Search (Alt + /)")
- [Let's chat!](https://chat.nullderef.com/ "Let's chat!")

# Plugins in Rust: The Technologies

May 17, 2021
·
[Mario Ortiz Manero](https://nullderef.com/about)
·
From the series “ [A Plugin System in Rust](https://nullderef.com/series/rust-plugins/)”


Table of Contents

- [The Problem](https://nullderef.com/blog/plugin-tech/#the-problem)
- [Safety Concerns](https://nullderef.com/blog/plugin-tech/#safety-concerns)
  - [Unsafe Code](https://nullderef.com/blog/plugin-tech/#unsafe-code)
  - [Error Resilience](https://nullderef.com/blog/plugin-tech/#error-resilience)
  - [Remote Code Execution via Plugins](https://nullderef.com/blog/plugin-tech/#remote-code-execution-via-plugins)
- [Backward Compatibility](https://nullderef.com/blog/plugin-tech/#backward-compatibility)
  - [Possible Solutions](https://nullderef.com/blog/plugin-tech/#possible-solutions)
  - [Avoiding Breakage](https://nullderef.com/blog/plugin-tech/#avoiding-breakage)
- [Possible Approaches](https://nullderef.com/blog/plugin-tech/#possible-approaches)
  - [Scripting Languages](https://nullderef.com/blog/plugin-tech/#scripting-languages)
  - [Inter-Process Communication](https://nullderef.com/blog/plugin-tech/#inter-process-communication)
  - [Dynamic Loading](https://nullderef.com/blog/plugin-tech/#dynamic-loading)
  - [WebAssembly](https://nullderef.com/blog/plugin-tech/#webassembly)
- [Prior Art](https://nullderef.com/blog/plugin-tech/#prior-art)
- [Conclusion](https://nullderef.com/blog/plugin-tech/#conclusion)
...
_**NOTE**: Google didn’t end up allocating a slot for me in the GSoC, but I’ll still be able to continue through [a LFX Mentorship](https://mentorship.lfx.linuxfoundation.org/project/b90f7174-fc53-40bc-b9e2-9905f88c38ff)._
...
5. **Extra:** [Safety](https://nullderef.com/blog/plugin-tech/#safety)
6. **Extra:** [Backwards compatibility](https://nullderef.com/blog/plugin-tech/#back-comp)
...
_**NOTE**: Depending on the approach that is taken for the Plugin System, implementing solutions to some of the points I make below can be really complex, so I wouldn’t work on them for the initial deadline. For now, it’s something we have to be aware of._

### Unsafe Code [\#](https://nullderef.com/blog/plugin-tech/\#unsafe-code)

Many of the technologies that can be used to implement a Plugin System work with unsafe code, like [FFIs](https://nullderef.com/blog/plugin-tech/#ffi) or [IPC Memory Sharing](https://nullderef.com/blog/plugin-tech/#memory-sharing). This isn’t necessarily a problem if it’s self-contained and thoroughly reviewed, but we lose some safety guarantees provided by Rust, increasing the maintenance cost of the library.
...
Other more involved methods for binary representations include[\[3\]](https://nullderef.com/blog/plugin-tech/#fn3):
...
Lua is seen in game development; it’s a quite simple language with a very performant JIT implementation, which in any case I think would be the best option here. It could be embedded into our runtime (it’s only 281 kB compiled![\[4\]](https://nullderef.com/blog/plugin-tech/#fn4)) and used to load plugins at either start-time or run-time. And knowing it’s used in games, which are _obsessed_ with performance, it might not be that much of a big deal in that regard.

_**NOTE**: There are [languages specifically designed to extend Rust’s capabilities](https://arewegameyet.rs/ecosystem/scripting/), which you might be interested in, but I’ll just simplify this part by going for Lua._
...
Lua gets extra points in safety. It’s possible to sandbox it by blocking whichever functions you don’t want users to access[\[5\]](https://nullderef.com/blog/plugin-tech/#fn5) (though it’s “tricky and generally speaking difficult to get right”). Similarly, one can also handle resource exhaustion issues within Lua programs. Not to mention that this wouldn’t require `unsafe` at all with an optimal set of bindings.
...
There are three ways to do IPC, which I’ll briefly list below. Performance-wise, this graph shows a comparison of the overhead of each of them[\[6\]](https://nullderef.com/blog/plugin-tech/#fn6):

![IPC comparison](https://nullderef.com/blog/plugin-tech/7UXWJEWW8N-1170.png)

#### Option 1: Based on Sockets [\#](https://nullderef.com/blog/plugin-tech/\#option-1-based-on-sockets)

Sockets are the “worst”-performing alternative in the previous chart, but they’re so common and easy to use in most languages that it’s worth taking a look at. Using relatively lightweight protocols like [Protocol Buffers](https://developers.google.com/protocol-buffers), the performance would be close to passing raw structs, but with improved backwards/forwards compatibility[\[7\]](https://nullderef.com/blog/plugin-tech/#fn7). JSON would probably not make that big of a difference in terms of performance either. This would make it possible to write a plugin in any language as well — including Rust — as long as there’s an implementation of the protocol available. But there’s still noticeable overhead when communicating via sockets; sending and receiving the messages can be much costlier than just calling a function, even if this happens on localhost.

This alternative is much more interesting than [Scripting Languages](https://nullderef.com/blog/plugin-tech/#scripting-langs) for Tremor’s specific case: we don’t have to completely rewrite everything, since Rust can still be used, and implementing the protocol to communicate between the Tremor core and its plugins should be as easy as `#[derive(Serialize)]` for sending and `#[derive(Deserialize)]` for receiving.
...
#### Option 2: Based on Pipes [\#](https://nullderef.com/blog/plugin-tech/\#option-2-based-on-pipes)
...
The article also includes a ~~not reliable at all~~ benchmark, which can serve us as a way to compare its performance with the native code you’d get with e.g., [Dynamic Loading](https://nullderef.com/blog/plugin-tech/#ffi). It estimates that Wasm is a bit less than an order of magnitude slower than native code, and the same applies to memory usage. [A more thorough benchmark](https://github.com/jedisct1/webassembly-benchmarks/tree/master/2021-Q1) was done in libsodium that shows better results: Wasm can be just about 3 times slower than native code. Do note that this depends on the runtime that’s being used, and it may improve in the future, as WebAssembly is just 4 years old.

There’s a whole series on how to make a Plugins System with Wasmer [here](https://freemasen.com/blog/wasmer-plugin-pt-1/), which will come in handy to know what to expect. The usability doesn’t actually seem to be that good, since by default you can only use integers, floating-point or vectors[\[15\]](https://nullderef.com/blog/plugin-tech/#fn15) as parameters when calling Wasm plugins. For more complex types, you have to resort to encoding and decoding via a crate like [`bincode`](https://github.com/bincode-org/bincode), although most of the boilerplate can be reduced with procedural macros or a wrapper like [`wasm_plugin`](https://github.com/alec-deason/wasm_plugin), and this opens up the possibility of using serialization with support for backwards compatibility within Tremor. The [last part](https://freemasen.com//blog/wasmer-plugin-pt-4/index.html) of the series is the most interesting one, as it includes a real-world example, with a version of the final code in [this repository](https://github.com/FreeMasen/wasmer-plugin).

All in all, WebAssembly seems to win against [Dynamic Loading](https://nullderef.com/blog/plugin-tech/#ffi) in terms of security by not needing `unsafe` at all and including sandboxing by default, at the cost of efficiency. This is up to the managers of the project and what they consider more important.
...
6. **3/5** **Ease of porting existing implementations**
...
This is a very interesting approach, specially because of how simple it is to use. Cargo doesn’t seem to need to communicate with the extension at all, but `mdbook` does use stdin to receive messages and stdout to send them, via serialization. So it’s basically the IPC [Based on Pipes](https://nullderef.com/blog/plugin-tech/#pipes) approach.
...
This is an extremely valuable resource in case the [WebAssembly](https://nullderef.com/blog/plugin-tech/#wasm) option is chosen, as it’s very similar to what Tremor needs. One can even subscribe to events in order to simulate the traits in Tremor that currently use `async`.
...
Text editors overall are very interesting, because they must be built with extensibility in mind and thus have to implement some kind of plugin system.
  ...
  - Eclipse’s plugin architecture is based on Java classes[\[16\]](https://nullderef.com/blog/plugin-tech/#fn16) loaded at runtime and configured with XML manifests.
...
This article has covered quite a few ways to approach a Plugin System. The final choice depends on what trade-offs the project wants to make. Most of them require sacrificing some level of performance for safety or usability. Here’s a drawing that _**very roughly**_ compares the main methods:

![Triangle Chart](https://nullderef.com/blog/plugin-tech/HP_jfT2mNl-500.jpeg)
...
_Free and without spam. You can also use my [RSS](https://nullderef.com/index.xml), [X](https://x.com/null_deref), or [Mastodon](https://mstdn.social/@nullderef)._

_Powered by [Kit](https://kit.com/)._

## Footnotes

01. [How to build an npm worm — Jamie Kyle](https://jamie.build/how-to-build-an-npm-worm) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref1)

02. [Yet another malicious package found in npm, targeting cryptocurrency wallets — snyk](https://snyk.io/blog/yet-another-malicious-package-found-in-npm-targeting-cryptocurrency-wallets/) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref2)

03. [How Swift Achieved Dynamic Linking Where Rust Couldn’t — Alexis Beingessner](https://gankra.github.io/blah/swift-abi/) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref3)

04. [About Lua — lua.org](https://www.lua.org/about.html#why) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref4)

05. [Sand Boxes — lua-users](http://lua-users.org/wiki/SandBoxes) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref5)

06. [Evaluation of Inter-Process Communication Mechanisms (paper)](https://pages.cs.wisc.edu/~adityav/Evaluation_of_Inter_Process_Communication_Mechanisms.pdf) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref6)

07. [C++ Benchmarks — FlatBuffers](https://google.github.io/flatbuffers/flatbuffers_benchmarks.html) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref7)

08. [Performance difference between IPC shared memory and threads memory — StackOverflow](https://stackoverflow.com/a/14512554/11488352) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref8)

09. [Compare with other libraries — dlopen v0.1.8](https://docs.rs/dlopen/0.1.8/dlopen/#compare-with-other-libraries) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref9)

10. [Idiomatic Rust plugin system — StackOverflow](https://stackoverflow.com/a/46249019/11488352) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref10)

11. [`papyrus/src/compile/execute.rs` — GitHub kurtlawrence/papyrus](https://github.com/kurtlawrence/papyrus/blob/1c7f0a669fed59d220bdefb161c568072126d3d5/src/compile/execute.rs#L36) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref11)

12. [Thread-safety — libloading v0.7.0](https://docs.rs/libloading/0.7.0/libloading/struct.Library.html#thread-safety) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref12)

13. [Safety — dlopen v0.1.8](https://docs.rs/dlopen/0.1.8/dlopen/#safety) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref13)

14. [\[NEW FEATURE\] WebAssembly scripting system amethyst/amethyst#1729](https://github.com/amethyst/amethyst/issues/1729) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref14)

15. [`Val` — Wasmer v2.0.0](https://docs.rs/wasmer/2.0.0/wasmer/type.Val.html) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref15)

16. [Notes on the Eclipse Plug-in Architecture - Eclipse](https://www.eclipse.org/articles/Article-Plug-in-architecture/plugin_architecture.html) [↩︎](https://nullderef.com/blog/plugin-tech/#fnref16)


* * *

[#tech](https://nullderef.com/tags/tech) [#programming](https://nullderef.com/tags/programming) [#rust](https://nullderef.com/tags/rust) [#open-source](https://nullderef.com/tags/open-source)

This post is part of the series “ [A Plugin System in Rust](https://nullderef.com/series/rust-plugins/)”, which is now finished:

1. **[Plugins in Rust: The Technologies](https://nullderef.com/blog/plugin-tech/)** ← you’re here!
2. [Plugins in Rust: Getting Started](https://nullderef.com/blog/plugin-start/)
3. [Plugins in Rust: Diving into Dynamic Loading](https://nullderef.com/blog/plugin-dynload/)
4. [Plugins in Rust: Reducing the Pain with Dependencies](https://nullderef.com/blog/plugin-abi-stable/)
5. [Plugins in Rust: Getting our Hands Dirty](https://nullderef.com/blog/plugin-impl/)
6. [Plugins in Rust: Wrapping Up](https://nullderef.com/blog/plugin-end/)
7. [\[Talk\] Rust, the best and worst thing to happen to Tremor](https://nullderef.com/blog/tremorcon22/)
8. [Blindsided by Rust's Subtyping and Variance](https://nullderef.com/blog/rust-variance/)

## No comments yet

Leave a comment for this article [on GitHub](https://github.com/marioortizmanero/nullderef.com-comments/issues/9)!

See this site’s source code [on GitHub](https://github.com/marioortizmanero/nullderef.com), licensed under GPLv3. Powered by [11ty](https://www.11ty.dev/)[Go to Top (Alt + G)](https://nullderef.com/blog/plugin-tech/#top "Go to Top (Alt + G)")