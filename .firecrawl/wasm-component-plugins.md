[Sy Brand](https://tartanllama.xyz/)

- [Posts](https://tartanllama.xyz/posts)
- [Tags](https://tartanllama.xyz/tags)
- [About](https://tartanllama.xyz/about)
- [Search](https://tartanllama.xyz/search "Search")

* * *

[Go back](https://tartanllama.xyz/)

# Building Native Plugin Systems with WebAssembly Components

Published:5 Aug, 2025

WebAssembly components can help you build secure, fast plugin systems with well-defined interfaces. I’ll show you how, then we’ll build a plugin system for a game called Guy Fighter, which will let users install plugins that invent entirely new types of guy for them to pit against each other.

![A console based game, showing a fighter roster of various types of guys, some of whom are made of nails](https://tartanllama.xyz/_astro/intro-graphic.BE9FKiz4_SDLbk.webp)

## Table of Contents [\#](https://tartanllama.xyz/posts/wasm-plugins/\#table-of-contents)

Open Table of Contents

- [Introduction](https://tartanllama.xyz/posts/wasm-plugins/#introduction)
- [Usual Design for Native Plugins](https://tartanllama.xyz/posts/wasm-plugins/#usual-design-for-native-plugins)
  - [Security](https://tartanllama.xyz/posts/wasm-plugins/#security)
  - [Interface Definition](https://tartanllama.xyz/posts/wasm-plugins/#interface-definition)
  - [Binary Compatibility](https://tartanllama.xyz/posts/wasm-plugins/#binary-compatibility)
- [WebAssembly](https://tartanllama.xyz/posts/wasm-plugins/#webassembly)
  - [Modules](https://tartanllama.xyz/posts/wasm-plugins/#modules)
  - [Components](https://tartanllama.xyz/posts/wasm-plugins/#components)
    - [WIT Types](https://tartanllama.xyz/posts/wasm-plugins/#wit-types)
    - [WIT Interfaces, Worlds, and Packages](https://tartanllama.xyz/posts/wasm-plugins/#wit-interfaces-worlds-and-packages)
- [Building a Plugin System](https://tartanllama.xyz/posts/wasm-plugins/#building-a-plugin-system)
  - [Guy Fighter](https://tartanllama.xyz/posts/wasm-plugins/#guy-fighter)
  - [The WIT Definition](https://tartanllama.xyz/posts/wasm-plugins/#the-wit-definition)
  - [The Host](https://tartanllama.xyz/posts/wasm-plugins/#the-host)
    - [Generating bindings](https://tartanllama.xyz/posts/wasm-plugins/#generating-bindings)
    - [Defining types](https://tartanllama.xyz/posts/wasm-plugins/#defining-types)
    - [Loading and running plugins](https://tartanllama.xyz/posts/wasm-plugins/#loading-and-running-plugins)
    - [Implementing the host function](https://tartanllama.xyz/posts/wasm-plugins/#implementing-the-host-function)
  - [C Plugin](https://tartanllama.xyz/posts/wasm-plugins/#c-plugin)
    - [Generating bindings](https://tartanllama.xyz/posts/wasm-plugins/#generating-bindings-1)
    - [Implementing the plugin](https://tartanllama.xyz/posts/wasm-plugins/#implementing-the-plugin)
    - [Building the component](https://tartanllama.xyz/posts/wasm-plugins/#building-the-component)
  - [JavaScript Plugin](https://tartanllama.xyz/posts/wasm-plugins/#javascript-plugin)
    - [The Magic](https://tartanllama.xyz/posts/wasm-plugins/#the-magic)
- [Limitations](https://tartanllama.xyz/posts/wasm-plugins/#limitations)
- [Conclusion](https://tartanllama.xyz/posts/wasm-plugins/#conclusion)
- [Acknowledgements](https://tartanllama.xyz/posts/wasm-plugins/#acknowledgements)

## Introduction [\#](https://tartanllama.xyz/posts/wasm-plugins/\#introduction)

When shipping software, we often want to enable user-driven customisation and extension. For example, we could support custom instruments for our audio application, or user-created content for our video game. To support non-programmer users and closed-source development, we should enable users to load these customisations at runtime rather than requiring that users compile them into the application. This is often achieved through _plugins_, which are code and data packages that users place in some designated plugin directory in their filesystem. The application that they’re using, known as the _host_, loads these plugins and interacts with them in an application-defined manner to run the code in the plugin and transfer any necessary data.

There are several common issues with _native_ plugin systems——ones that load machine code plugins that were likely written in a compiled language like C++ or Rust. Three big problems are: security, interface definition, and binary compatibility. WebAssembly Components are [sandboxed](https://en.wikipedia.org/wiki/Sandbox_(computer_security)), interoperable units of code and data with clearly-defined interfaces. In this post, I’ll walk through exactly what they are and how they address those three problems with existing native plugin systems. Then, I’ll show a concrete example of adding a plugin system to a Rust program that interfaces with plugins written in C and JavaScript.
...
![Image showing the architecture described below](https://tartanllama.xyz/_astro/shared-libs.light.0JzH4dTU_Z1xls8Y.svg)![Image showing the architecture described below](https://tartanllama.xyz/_astro/shared-libs.dark.DmI21lRU_1jDTg6.svg)
...
- Text types: `char` and `string`, both Unicode[1](https://tartanllama.xyz/posts/wasm-plugins/#user-content-fn-1)
...
```
type cat-registry = list<cat>;Copy
```
...
// Or the Rust function fn eat_food(hungry_friend: cat, amount_of_food: u32) -> mess;Copy
```

- `resource`: An opaque handle to an object that satisfies a specified interface[2](https://tartanllama.xyz/posts/wasm-plugins/#user-content-fn-2)
...
![Image showing the architecture described below](https://tartanllama.xyz/_astro/components.light.C9qrGbkG_jrfEz.svg)![Image showing the architecture described below](https://tartanllama.xyz/_astro/components.dark.BQSCncdi_jrfEz.svg)

The host application embeds the Wasm runtime. Rather than loading the plugins into the host application, we load them into the Wasm runtime, which sandboxes their execution. The host application communicates with the plugins through a carefully-controlled interface supplied by the Wasm runtime.

The last piece of the puzzle is tooling. Based on the WIT description, we can use tools such as [`wit-bindgen`](https://github.com/bytecodealliance/wit-bindgen) to automatically generate bindings for whatever programming language we’re using that handles the binary format for us, letting us call functions that other components export without having to think about the communication layer.
...
The application I’ve written is called [Guy Fighter](https://github.com/TartanLlama/guy-fighter/tree/no-plugins). It’s a battle simulation game that pits randomly-generated guys of different types against each other in mortal combat. Here’s how it looks in action:

![A console based game, showing a fighter roster of two types of guys: a guy who's made of nails, and a guy who's made of normal guy stuff, except his hands, which are made of nails](https://tartanllama.xyz/_astro/guy-fighter-1.Cn-UWyhN_2mORRM.webp)

As you can see, there’s only two types of guy built in to the game. This very quickly becomes stale. To add more variety to the game, we should allow the player to install plugins that register entirely new types of guys, who can then fight each other.
...
```
use tl::guy_fighter::host::TypeOfGuy;Copy
```
...
```
    ...
    load_plugins(&mut state, plugins_dir)?;Copy
```
...
```
    Plugin::add_to_linker(&mut linker, |state| state)?;Copy
```
...
The `load_plugin` function is responsible for loading the plugin from disk, instantiating a `Plugin` component from it, calling the plugin’s `init` function and storing aside its name and path:
...
```
$ cargo install wasm-toolsCopy
```

For generating bindings, we’ll use [`wit-bindgen`](https://github.com/bytecodealliance/wit-bindgen), which can generate bindings for C, along with a bunch of other languages. You can install this by running:

```
$ cargo install wit-bindgen-cliCopy
```
...
```
$ wit-bindgen c guy-fighter/wit -w plugin --out-dir c-plugin/genCopy
```
...
} tl_guy_fighter_host_type_of_guy_t;Copy
```
...
$ wasm-tools component new \
    ...
    -o c-plugin/build/c-plugin.wasmCopy
```
...
![Updated version of Guy Fighter that can list the plugins available and display types that are provided by plugins](https://tartanllama.xyz/_astro/guy-fighter-2.BIbpGGBp_1HM88d.webp)
...
```
$ npm install -g @bytecodealliance/componentize-jsCopy
```
...
$ componentize-js js-plugin/plugin.js \
    ...
    -o js-plugin/build/js-plugin.wasmCopy
```
...
![Updated version of Guy Fighter that shows the data from the JS component as well as the C one](https://tartanllama.xyz/_astro/guy-fighter-3.B0x-lnns_2eKKar.webp)
...
What ComponentizeJS does is say, well, if we’re not going to compile the JavaScript to WebAssembly, how about we compile the JavaScript engine to WebAssembly, dump it into the component, and use that to run the JavaScript? You can read more of the details in the [Making JavaScript Run Fast on WebAssembly](https://bytecodealliance.org/articles/making-javascript-run-fast-on-webassembly) explainer, but that’s the gist of it: it takes the [SpiderMonkey](https://spidermonkey.dev/) JS engine (which is used in Firefox), customises it for WebAssembly to make the [StarlingMonkey](https://github.com/bytecodealliance/StarlingMonkey) engine, then embeds that in your component.
...
## Footnotes [\#](https://tartanllama.xyz/posts/wasm-plugins/\#footnote-label)

1. Specifically, `char` is a [Unicode Scalar Value](https://unicode.org/glossary/#unicode_scalar_value). [↩](https://tartanllama.xyz/posts/wasm-plugins/#user-content-fnref-1)

2. Resources have a fair amount to them and they’re not super relevant for this use case, so have a look at [the documentation](https://component-model.bytecodealliance.org/design/wit.html#resources) if you want to learn more. [↩](https://tartanllama.xyz/posts/wasm-plugins/#user-content-fnref-2)


* * *

- [Rust](https://tartanllama.xyz/tags/rust/)
- [WebAssembly](https://tartanllama.xyz/tags/web-assembly/)
...
[Next Post\\
\\
std::generator: Standard Library Coroutine Support](https://tartanllama.xyz/posts/std-generator)

* * *

[Sy Brand on Github](https://github.com/TartanLlama "Sy Brand on Github") [Sy Brand on Bluesky](https://bsky.app/profile/tartanllama.xyz "Sy Brand on Bluesky") [Sy Brand on Mastodon](https://mastodon.social/@TartanLlama "Sy Brand on Mastodon") [Sy Brand on LinkedIn](https://www.linkedin.com/in/sy-brand-36520857/ "Sy Brand on LinkedIn") [Send an email to Sy Brand](mailto:tartanllama@gmail.com "Send an email to Sy Brand")

Based on [AstroPaper](https://astro-paper.pages.dev/)
