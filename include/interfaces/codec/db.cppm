...
public:
    ...
    /**
     * @brief Encodes a `T` into whatever payload the backend needs to run a query/select for
     * it. Four encode_* methods below, four different jobs — don't mix them up.
     * @param T the value driving the query.
     * @return the encoded query payload.
     */
    [[nodiscard]] virtual std::string encode_query(const T&) const = 0;
    /**
     ...
     */
    [[nodiscard]] virtual std::string encode_insert(const T&) const = 0;
    /**
     ...
     */
    [[nodiscard]] virtual std::string encode_update(const T&) const = 0;
    /**
     ...
     */
    [[nodiscard]] virtual std::string encode_remove(const T&) const = 0;
