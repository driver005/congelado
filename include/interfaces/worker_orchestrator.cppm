...
/// `WorkerInfo`. `queued_total`/`in_flight_total` are the dispatch backlog view; `alive` flips
/// false if the backend loses its store. Fed to the health callback whenever the snapshot
/// changes.
...
/// `IWorkerManager`/`ICron`/`ICache`: a deployment picks exactly one at a time and the engine
/// host resolves it before build() (see `worker_orchestrator_ctx`). Where `IWorkerManager` owns
/// worker LIFECYCLE on the worker side, this owns task DISPATCH + workflow ORCHESTRATION on the
/// engine side — the queue workers claim from, result submission, and workflow start.
///
...
/// blocking. Kept dependency-free (flat key/value in/out + serialized JSON strings, like
/// `IWorker`/ `IAppDefs`) so `interfaces` stays free of any model/serde coupling; the config
/// for a concrete backend arrives through the plugin's own `on_load(host, cfg)`, exactly as the
/// external worker manager reads its `config_path`.
class IWorkerOrchestrator
{
public:
    /**
     * @brief Virtual dtor, default's good — polymorphic orchestrator backends clean up fine
    ...
    /**
     * @brief Says whether this backend is load-bearing. Defaults to false — with no
     * orchestrator plugin the engine falls back to its built-in Orchestrator, so its absence is
     * a soft degrade.
    ...
    /**
     * @brief Enqueues a task for a worker type, delivering the new task's id — the dispatch
     * entry point. Workers later claim() this and submit_result() against the returned id.
     * @param task_type the worker/queue type to enqueue under.
     * @param input the task's input as a dynamic value (object of string/int/bool/nested
     * fields), not a flat string map.
    ...
    /**
     * @brief Claims the next ready task for `worker_type` (optionally scoped to `domain`),
     * delivering it as a serialized TaskInstance (JSON) — the same shape a worker polls today.
     * std::nullopt means nothing is queued.
    ...
    /**
     * @brief Requeues every stuck/in-flight task for a worker type back to ready — the recovery
     * path for a worker that claimed work and never reported back.
     * @param worker_type the worker type to requeue.
     * @param callback gets how many tasks were requeued.
     */
    virtual void
    requeue(std::string_view worker_type, std::move_only_function<void(std::size_t)> callback) = 0;
    /**
    ...
    /**
     * @brief Stands up the backend's own inbound surface, if any (a distributed backend may
     * expose a control API). Called once by the engine host after the backend is resolved. A
     * backend with no inbound surface just no-ops.
     */
    ...
    /**
     * @brief Installs the callback the backend invokes whenever its health snapshot changes
     * (queue depth, liveness). Called once, before any dispatch.
