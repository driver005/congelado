...
enum class SpanKind : std::uint8_t { INTERNAL, SERVER, CLIENT, PRODUCER, CONSUMER };

enum class SpanStatus : std::uint8_t { UNSET, OK, ERROR };

enum class LogSeverity : std::uint8_t { DEBUG, INFO, WARN, ERROR, FATAL };
 ...
 * registered `ITracerProvider` (so fan-out providers agree on the same span/trace ids), captured
 * by value across thread hops (e.g. `WorkerContext::resolve_response()`), and serialized into
 * the outbound `traceparent` wire header.
...
/**
 * @brief One per-provider span handle — a logical span held by `core::otel::ScopedSpan` gets one
 * of these per currently-registered `ITracerProvider` (the fan-out itself lives one level up,
 * here it's just "this one provider's view of this one span").
 */
class ISpan {
  public:
    ISpan() = default;
...
class ITracerProvider {
  public:
     ...
     */
    [[nodiscard]] virtual std::shared_ptr<ISpan> start_span(std::string_view name, SpanKind kind,
                                                             const SpanContext &ctx,
                                                             std::span<const Attribute> attrs) = 0;
};

/**
 * @brief A single counter instrument under one provider — monotonically-increasing sums (request
 ...
 * optional sub-providers. Instrument creation is by name; `core::otel::MeterRegistry` caches the
 * returned handles so repeated `counter_add("task.completed", ...)`-style facade calls don't
 * re-create the instrument every time.
 */
class IMeterProvider {
  public:
    ...
    create_histogram(std::string_view name, std::string_view description,
 ...
 * `core::logger::*` call, carrying whatever ambient trace/span context was active on the calling
 * thread at the time.
 */
struct LogRecord {
 ...
 * per-signal contracts. Each accessor defaults to `nullptr` so a plugin can support any subset of
 * the three signals — e.g. a metrics-only exporter just overrides `get_meter_provider()`.
 */
class IOtelProvider {
  public:
     ...
     */
    [[nodiscard]] virtual ITracerProvider *get_tracer_provider() noexcept { return nullptr; }
     ...
     */
    [[nodiscard]] virtual IMeterProvider *get_meter_provider() noexcept { return nullptr; }
     ...
     */
    [[nodiscard]] virtual ILogRecordProvider *get_log_provider() noexcept { return nullptr; }
};
