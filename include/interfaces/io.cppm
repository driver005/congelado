...
concept SyncReceivable =
    ReceiveOutput<Out> && requires(T sock, Out out, std::size_t len, const std::size_t offset, Args... args) {
        { sock.sync_receive(out, len, offset, args...) } -> std::same_as<std::pair<std::size_t, Status>>;
    };

// template <typename T, typename Status, typename Out, typename... Args>
// concept SyncReceivableWait = ReceiveOutput<Out> && requires(T sock, Out out, std::size_t len, bool wait, Args...
// args) {
//     { sock.sync_receive(out, len, wait, args...) } -> std::same_as<std::pair<std::size_t, Status>>;
// };
...
template <typename T, typename Status, typename Out, typename... Args>
concept IoSyncOps =
    SyncSendable<T, Status, Args...> && SyncReceivable<T, Status, Out, Args...> && SyncGetter<T> && SyncClose<T>;

// ASYNC

template <typename T, typename Status, typename... Args>
concept AsyncSendable = requires(T sock, const std::byte *buf, std::size_t len, IoCallback<Status> cb, Args... args) {
    { sock.async_send(buf, len, std::move(cb), args...) } noexcept -> std::same_as<void>;
};

template <typename T, typename Status, typename Out, typename... Args>
concept AsyncReceivable =
    ReceiveOutput<Out> && requires(T sock, Out out, std::size_t len, IoCallback<Status> cb, Args... args) {
...
template <typename T, typename Status, typename Out, typename... Args>
concept IoAsyncReceive = AsyncReceivable<T, Status, Out, Args...> && AsyncGetter<T> && AsyncClose<T>;

template <typename T, typename Status, typename Out, typename... Args>
concept IoAsyncOps =
    AsyncSendable<T, Status, Args...> && AsyncReceivable<T, Status, Out, Args...> && AsyncGetter<T> && AsyncClose<T>;
...
concept IoOps =
    AsyncSendable<T, Status, Args...> && SyncSendable<T, Status, Args...> && SyncReceivable<T, Status, Out, Args...> &&
