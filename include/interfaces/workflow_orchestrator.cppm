...
/// @brief Pluggable workflow-orchestrator backend (the "workflow_orchestrator" capability) — owns
/// the workflow EXECUTION LIFECYCLE (the DAG side), the counterpart to `IWorkerOrchestrator` (which
/// owns task DISPATCH). Single-active-backend, resolved by the engine host before build() (see
/// `workflow_orchestrator_ctx`). Every op is asynchronous (callback-per-op), mirroring
/// `connector::Connector` and the route handlers' `send` callback, so the engine's already-async
/// orchestration wires in with no blocking. Dependency-free at the boundary (ids/refs as strings,
/// variables/payload as flat key/value maps) so `interfaces` stays free of any model/serde coupling;
/// a concrete backend's config arrives through the plugin's own `on_load(host, cfg)`.
class IWorkflowOrchestrator {
  public:
    /**
     * @brief Virtual dtor, default's good — polymorphic workflow-orchestrator backends clean up fine
    ...
    /**
     * @brief Tells you which workflow-orchestrator backend is running (the built-in in-process DAG
     * walker, a distributed one, whatever got plugged in).
     ...
     */
    virtual void on_task_terminal(std::string_view task_id,
     ...
     */
    virtual void on_execution_terminal(std::string_view exec_id,
                                       std::move_only_function<void(bool)> callback) = 0;
     ...
     */
    virtual void restart(std::string_view exec_id, std::move_only_function<void(bool)> callback) = 0;
    /**
     ...
     */
    virtual void terminate(std::string_view exec_id,
                           std::move_only_function<void(bool)> callback) = 0;
    /**
     * @brief Re-derives what should be scheduled next for an execution — the self-healing reconcile.
     * @param exec_id the execution to reconcile.
     * @param callback gets true if the execution/def was found.
     */
    virtual void reconcile(std::string_view exec_id,
                           std::move_only_function<void(bool)> callback) = 0;
    /**
     ...
     */
    virtual void rerun(std::string_view exec_id, std::string_view node_ref,
     ...
     */
    virtual void signal(std::string_view exec_id, std::string_view node_ref,
                        std::optional<std::string_view> payload,
                        std::move_only_function<void(bool)> callback) = 0;

    /**
     * @brief Completes a node's instance by `(exec_id, node_ref)` with a success/failure outcome +
     * output, then cascades the DAG — the generic external-signal completion path (behind
     * `POST /api/v1/queue/update`) for callers that key off the execution + node reference rather
     * than an internal task id.
     * @param exec_id the owning execution.
     ...
     * @param callback gets true once applied.
     */
    virtual void complete_task(std::string_view exec_id, std::string_view node_ref, bool success,
                               const std::unordered_map<std::string, std::string> &output,
                               std::move_only_function<void(bool)> callback) = 0;

    /**
     * @brief Stands up the backend's own inbound surface, if any. Called once by the engine host.
     */
...
// Minimal IWorkflowOrchestrator fixture — every pure virtual gets a trivial body so required()'s
...
class MockWorkflowOrchestrator final : public IWorkflowOrchestrator {
  public:
    ...
    void on_execution_terminal(std::string_view,
