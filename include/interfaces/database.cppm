...
public:
     ...
     * a failed connect and degrades every query to an empty result rather than crashing.
     * Callers that gate work on "is a database attached" (migrations, anything that can't
     * tolerate a silently-empty result) should check this, not just pointer-non-null.
     * @return true if not overridden — matches the previous nullptr-only gating behavior for
     * any backend that has no distinct connect step to fail.
     */
