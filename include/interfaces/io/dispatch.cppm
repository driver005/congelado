...
class DispatchResult {
  public:
     ...
     */
    [[nodiscard]] static DispatchResult ready() noexcept { return {}; }
     ...
     */
    [[nodiscard]] bool is_ready() const noexcept { return !m_on_ready; }
...
class DispatchFunction {
  public:
    ...
    template <typename Fn>
        requires(!std::same_as<std::remove_cvref_t<Fn>, DispatchFunction> &&
     ...
     */
    DispatchResult operator()(IRequest &req, IResponse &res) const { return m_fn(req, res); }
  ...
  private:
    ...
    static Erased wrap(Fn &&fn) {
        ...
        if constexpr (std::is_void_v<Result>) {
            ...
            return Erased{[target = Target{std::forward<Fn>(fn)}](
                              IRequest &req, IResponse &res) mutable -> DispatchResult {
                target(req, res);
                return DispatchResult::ready();
            }};
        } else {
            static_assert(std::convertible_to<Result, DispatchResult>,
