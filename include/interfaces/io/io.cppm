...
using ReceiveDispatchFn = std::function<void(IRequest&, IResponse&, std::function<void()> send)>;
...
concept SyncReceivable =
    ReceiveOutput<Out> &&
    requires(T sock, Out out, std::size_t len, const std::size_t OFFSET, Args... args) {
        {
...
concept AsyncReceivable =
    ReceiveOutput<Out> &&
    requires(T sock, Out out, std::size_t len, IoCallback<Status> callback, Args... args) {
        {
            sock.async_receive(out, len, std::move(callback), args...)
