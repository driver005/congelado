...
/// @brief Pluggable scheduling engine (the "cron" capability). Single-active-backend, same
/// shape as `ISearchProvider`/`ICache` — a deployment picks exactly one cron backend at a time.
/// Owns both the cron-expression math (validate/next_after) and the actual timing loop: the
/// host registers named jobs, hands over one fire callback, and the backend invokes that
/// callback with a job's name when its schedule comes due. The backend knows nothing about what
/// a "job" does — the name is opaque, and the callback owner (the engine plugin) maps it back
/// to a workflow.
class ICron
{
public:
    ...
    /**
     * @brief Computes the next fire time strictly after `base` for a cron expression — powers
     * the engine's next-few-runs preview without registering a job.
     * @param cron_expression the expression to evaluate.
     * @param base the time to search strictly after.
     * @return the next matching time_point, or std::nullopt if the expression is invalid or has
    ...
    /**
     * @brief Installs the callback the backend invokes (with the job's name) whenever a
     * registered job comes due. Called once, before any job is registered.
