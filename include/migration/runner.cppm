...
using BaselineFn = std::move_only_function<
    void(interfaces::IDatabase&, connector::Connector&, std::move_only_function<void(bool)>)>;

/**
 * @brief Process-wide "has the one global migration pass finished" signal.
 *
 * Set once by whoever drives the host-owned migration run
 * (`congelado::heart::App::load_plugins`, after `migration::Runner::run_all_blocking` returns —
 * success, failure, or a skip because no database is attached/connected all count as "as done
 * as it's going to get"). Anything that can't safely touch the database until its own tables
 * are guaranteed to exist — e.g. a plugin's background sweep thread, started during its own
 * `on_load` before the global migration pass has even run — should poll this instead of
 * guessing at a fixed startup delay.
 */
class Status
{
public:
    /// @brief Marks the global migration pass as finished (successfully or otherwise settled).
    static void mark_ready() noexcept
    {
    ...
    [[nodiscard]] static bool is_ready() noexcept
    {
...
public:
     ...
     * @param migrations_dir directory scanned for `<timestamp>_description.sql` migration
     * files.
            ...
            *db, [db, connector, dir = std::string{migrations_dir},
                ...
                load_applied(
                    *db, [db, connector, dir = std::move(dir),
                        ...
                        // run_baselines()'s body (which needs `applied` by reference, via the
...
private:
    ...
    static void
    load_applied(interfaces::IDatabase& db, std::move_only_function<void(AppliedMap)> done)
    {
        db.query(
     ...
     */
    [[nodiscard]] static std::vector<MigrationFile>
    scan_migrations(const std::filesystem::path& dir)
    {
