...
class TomlParser {
  public:
    ...
    static std::expected<void, std::string> from_toml_impl(const toml::table &, T &);
};
}
...
error_code tag_invoke(deserialize_tag, V &val, T &obj);
} // namespace simdjson
...
struct FieldConverter<std::unordered_map<std::string, std::string>> {
    ...
    static simdjson::error_code from_simdjson(simdjson::ondemand::value &v,
        ...
        for (auto field : obj) {
            ...
            if (auto ec = field.unescaped_key().get(key); ec)
                return ec;
            if (auto ec = field.value().get_string().get(val); ec)
                return ec;
            out.emplace(key, val);
        }
        return simdjson::SUCCESS;
    ...
    from_toml(const toml::table &t, std::string_view fname) {
        ...
        for (auto &&[key, value] : *sub) {
            auto sv = value.value<std::string>();
            if (!sv)
                return std::unexpected{std::format("value in '{}' must be a string", fname)};
            result.emplace(std::string{key.str()}, std::move(*sv));
        }
        return result;
    }
    ...
    return std::apply(
        [&](auto... fds) {
            return rfl::NamedTuple(
                rfl::make_field<decltype(fds)::name>(
...
struct FieldConverter<VT> {
    ...
    static std::expected<VT, std::string> from_toml(const toml::table &t, std::string_view fname) {
        ...
        VT obj;
        if (auto r = TomlParser::from_toml_impl(*sub, obj); !r)
...
template <ISerializable T>
std::expected<void, std::string> TomlParser::from_toml_impl(const toml::table &t, T &obj) {
...
error_code tag_invoke(deserialize_tag, V &val, T &obj) {
    ...
    std::apply(
        [&](auto... fds) {
            ((result == SUCCESS
                  ? (result = serde::extract_simdjson_field(json_obj, obj, fds))
                  : SUCCESS),
             ...);
        },
