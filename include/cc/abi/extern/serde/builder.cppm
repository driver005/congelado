...
public:
    ...
    static TF_SerdeOps* get_generic_vtable()
    {
        static TF_SerdeOps vtable = {
            ...
            {
                auto* self = Serde::create(serde);
                auto res = self->get_content_type(ice::sonic::String::wrap(out_content_type));
                if (!res) {
            ...
            {
                auto* self = Serde::create(serde);
                auto res = self->get_format_name(ice::sonic::String::wrap(out_format_name));
                if (!res) {
                    res.error().to_c(status);
                }
            },
