...
public:
    ...
    add_module(const ice::sonic::Generator_module& module) noexcept = 0;
    [[nodiscard]] std::expected<void, ice::Status>
    get_module(const ice::sonic::String& name) noexcept = 0;
    [[nodiscard]] std::expected<void, ice::Status> list_modules() noexcept = 0;
    [[nodiscard]] std::expected<void, ice::Status>
    ...
    static TF_GeneratorOps* get_generic_vtable()
    {
        static TF_GeneratorOps vtable = {
            ...
            .add_module =
                [](TF_Generator* generator, TF_Generator_Module* module, TF_Status* status) noexcept
            {
                auto* self = Generator::create(generator);
                auto res = self->add_module(ice::sonic::Generator_module::wrap(module));
                if (!res) {
            ...
            {
                auto* self = Generator::create(generator);
                auto res = self->get_module(ice::sonic::String::wrap(name));
                if (!res) {
                    res.error().to_c(status);
                }
            },
            .list_modules =
                [](TF_Generator* generator, TF_Status* status) noexcept
            {
                auto* self = Generator::create(generator);
                auto res = self->list_modules();
                if (!res) {
                    res.error().to_c(status);
                }
            },
