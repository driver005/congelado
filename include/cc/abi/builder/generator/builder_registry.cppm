...
// sonic/generator's ice::sonic::generator::Runtime uses this registry to create generator builders by
// name — this is the whole reason cc_stable_hlo doesn't need cc_abi to know it exists:
// stable_hlo registers itself here (a downward dependency, stable_hlo ->
// cc_abi_builder_generator) instead of the ABI importing stable_hlo directly.
class BuilderRegistry
{
public:
    using Factory = Builder::Factory;
    ...
    create(std::string_view name, std::string_view output_dir, std::string_view source_dir)
    {
    ...
    Builder*
    get_or_create(std::string_view name, std::string_view output_dir, std::string_view source_dir)
    {
    ...
    static BuilderRegistry& default_registry()
    {

        static BuilderRegistry registry;
        return registry;
    }

private:
    std::map<std::string, Factory> factories_;
    std::map<std::string, std::unique_ptr<Builder>> instances_;
};
