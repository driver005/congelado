...
public:
    ...
    static void
    new_writable_file(const char* filename, WritableFileHandleView* handle, TF_Status* status)
    {

        TF_WritableFileHandle* raw = nullptr;
        TF_NewWritableFile(filename, &raw, status);
        if (raw) {
            *handle = WritableFileHandleView(raw);
        }
    }
    ...
    static void append_writable_file(
        WritableFileHandleView* handle, const char* data, size_t length, TF_Status* status
    )
    {
