...
public:
  ...
  static void add_control_input(TF_Graph *graph, TF_Operation *op,
                                TF_Operation *input) {
  ...
  set_attr(TF_Graph *graph, TF_Operation *op, const ice::String &attr_name,
           TF_Buffer_Handle *attr_value_proto) {

    ice::Status status;
    tensorflow::SetAttr(graph, op, attr_name.c_str(), attr_value_proto,
                        status.get_handle());
    if (!status.ok()) {
  ...
  clear_attr(TF_Graph *graph, TF_Operation *op, const ice::String &attr_name) {
    ...
    ice::Status status;
    tensorflow::ClearAttr(graph, op, attr_name.to_std_string().data(),
                          status.get_handle());
    if (!status.ok()) {
      return std::unexpected{status};
    }
  ...
  static void set_full_type(TF_Graph *graph, TF_Operation *op,
                            const TF_Buffer_Handle *full_type_proto) {

    tensorflow::SetFullType(graph, op, full_type_proto);
  }

  static void set_requested_device(TF_Graph *graph, TF_Operation *op,
                                   const ice::String &device) {
  ...
  [[nodiscard]] static std::expected<void, ice::Status>
  extend_session(TF_Session *session) {
  ...
  static ice::String get_handle_shape_and_type(TF_Graph *graph,
                                               TF_Output output) {
    return ice::String{tensorflow::GetHandleShapeAndType(graph, output)};
  }

  [[nodiscard]] static std::expected<void, ice::Status>
  set_handle_shape_and_type(TF_Graph *graph, TF_Output output,
                            const void *proto, size_t proto_len) {

    ice::Status status;
    tensorflow::SetHandleShapeAndType(graph, output, proto, proto_len,
                                      status.get_handle());
    if (!status.ok()) {
      return std::unexpected{status};
    }
