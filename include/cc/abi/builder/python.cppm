...
class PythonApiBuilder {
 public:
  ...
  static void set_attr(TF_Graph* graph, TF_Operation* op, const ice::StringBuilder& attr_name,
  ...
  static void clear_attr(TF_Graph* graph, TF_Operation* op, const ice::StringBuilder& attr_name,
                        TF_Status* status) {
  ...
  static void set_full_type(TF_Graph* graph, TF_Operation* op,
  ...
  static void set_requested_device(TF_Graph* graph, TF_Operation* op, const ice::StringBuilder& device) {

    tensorflow::SetRequestedDevice(graph, op, device.c_str());

  }

  static void update_edge(TF_Graph* graph, TF_Output new_src, TF_Input dst,
                         TF_Status* status) {
  ...
  static StringBuilder get_handle_shape_and_type(TF_Graph* graph, TF_Output output) {

    return StringBuilder(tensorflow::GetHandleShapeAndType(graph, output).c_str());

  }

  static void set_handle_shape_and_type(TF_Graph* graph, TF_Output output,
  ...
  static void add_while_input_hack(TF_Graph* graph, TF_Output new_src,
                                TF_Operation* dst, TF_Status* status) {
