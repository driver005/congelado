...
{
public:
    FilesystemPluginOpsBuilder() :
        m_handle{TF_FilesystemPluginOpsNew()}
    {
    }
    ...
    FilesystemPluginOpsBuilder(FilesystemPluginOpsBuilder&& other) noexcept :
    ...
    void set_version_metadata()
    {
        TF_SetFilesystemVersionMetadata(m_handle);
    }

    // Underlying handle — pass directly to the C ABI
    TF_FilesystemPluginOps* get_handle()
    {
        return m_handle;
    }

    const TF_FilesystemPluginOps* get_handle() const
    {
        return m_handle;
    }
