...
export module cc_abi_sonic:search_runtime;
...
public:
    ...
    explicit SearchRuntime(TP_Search* handle) :
        m_handle{handle}
    {
    }
    ...
    ) const
    {
        ...
        if (m_handle && m_handle->index_cb) {
            m_handle->index_cb(
                m_handle->ext, collection, id, document_json, completion, cb_user_data
            );
        }
    }
    ...
    void invoke_search(
        const TF_TString* collection,
        const SearchQueryRuntime& query,
        TF_Search_CompletionFn completion,
        void* cb_user_data
    ) const
    {
        ...
        if (m_handle && m_handle->search_cb) {
            m_handle->search_cb(
                m_handle->ext, collection, query.get_handle(), completion, cb_user_data
            );
        }
    }
