...
public:
    ...
    explicit Tracer(TF_Otel* ops, void* handle) :
        m_ops{ops},
        m_handle{handle}
    {
    }

    std::unique_ptr<ice::sonic::Span> start_span(const ice::String& name, ice::SpanKind kind)
    {
        ice::Status status;
...
public:
    ...
    explicit Meter(TF_Otel* ops, void* handle) :
        m_ops{ops},
        m_handle{handle}
    {
    }
    ...
    create_counter(const ice::String& name, const ice::String& description, const ice::String& unit)
    {
        ice::Status status;
...
{
public:
    explicit Otel(TF_Otel* ops, void* plugin_context) :
    ...
    [[nodiscard]] std::expected<std::unique_ptr<ice::sonic::Tracer>, ice::Status> get_tracer()
    {
        ice::Status status;
    ...
    std::unique_ptr<ice::sonic::Meter> get_meter()
    {
        ice::Status status;
