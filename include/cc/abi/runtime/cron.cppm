...
export module cc_abi_runtime:cron;
...
class CronRuntime {
 public:
  CronRuntime() : m_handle{nullptr} {}
  explicit CronRuntime(TP_Cron* handle) : m_handle{handle} {}
  ...
  TF_Bool invoke_validate(const TF_TString* expression) const {
    ...
    return (m_handle && m_handle->validate_cb) ? m_handle->validate_cb(m_handle->ext, expression) : 0;

  }
  TF_Bool invoke_next_after(const TF_TString* expression, int64_t base_time_ms, int64_t* out_time_ms) const {
  ...
  TP_Cron *get_handle() const { return m_handle; }
