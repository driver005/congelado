...
export module cc_abi_sonic_socket;
...
class Socket : public ice::sonic::Runtime<Socket, TF_Socket>
{
public:
    explicit Socket(TF_Socket* ops, void* plugin_context) noexcept :
        Runtime(ops, plugin_context)
    {
    }

    static constexpr std::string_view domain_name = "socket";

    [[nodiscard]] std::expected<void, ice::Status>
    new_socket(TF_Socket_Protocol protocol, const ice::sonic::String& host, uint16_t port) noexcept
    {
        ice::Status status;
        m_ops->new_socket(get_handle(), protocol, host.get_handle(), port, status.get_handle());
    ...
    [[nodiscard]] std::expected<void, ice::Status> accept(TF_Socket_Handle** out_accepted) noexcept
    {
        ice::Status status;
        m_ops->accept(get_handle(), out_accepted, status.get_handle());
    ...
    [[nodiscard]] std::expected<void, ice::Status>
    accept_async(TF_Socket_AcceptFn completion, void* user_data) noexcept
    {
        ice::Status status;
    ...
    [[nodiscard]] std::expected<void, ice::Status>
    connect_async(int64_t timeout_ms, TF_Socket_AckFn completion, void* user_data) noexcept
    {
        ice::Status status;
    ...
    [[nodiscard]] std::expected<void, ice::Status> get_error_code() noexcept
    {
        ice::Status status;
        m_ops->get_error_code(get_handle(), status.get_handle());
    ...
    [[nodiscard]] std::expected<void, ice::Status>
    get_local_endpoint(TF_String* out_host, uint16_t* out_port) noexcept
    {
        ice::Status status;
        m_ops->get_local_endpoint(get_handle(), out_host, out_port, status.get_handle());
    ...
    [[nodiscard]] std::expected<void, ice::Status> get_protocol() noexcept
    {
        ice::Status status;
        m_ops->get_protocol(get_handle(), status.get_handle());
    ...
    [[nodiscard]] std::expected<void, ice::Status> get_fd() noexcept
    {
        ice::Status status;
        m_ops->get_fd(get_handle(), status.get_handle());
    ...
    [[nodiscard]] std::expected<void, ice::Status> is_valid() noexcept
    {
        ice::Status status;
        m_ops->is_valid(get_handle(), status.get_handle());
