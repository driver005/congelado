...
public:
    ...
    std::expected<std::string, std::string> render(const parser::vtable::Model& model)
    {
        ...
        m_writer +=
            helper::format_footer(helper::GenTarget::Builder, m_namespace_name, m_repo_root);
...
private:
    ...
    write_cpp_parameter_list(std::span<const parser::helper::Parameter> parameters)
    {
        for (auto&& [index, parameter]: parameters | std::views::enumerate) {
            ...
            m_writer += helper::format_parameter(
                type_name->get().to_pointee_type(m_namespace_name),
                parameter.get_name()
            );
        }
    ...
    // Position 0's declared name — "plugin_context" for a domain-level slot, or the owned instance
    // handle's name for an instance-level slot (which drops the separate plugin_context param and
    // takes only its own handle as self).
