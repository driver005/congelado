...
private:
    ...
    int run_generate_single(const CliOptions& options)
    {
        ...
        auto write_result =
            m_writer.write(render(*model, *options.m_tier == "sonic"), *options.m_out, repo_root);
        if (!write_result) {
    ...
    int run_generate(const CliOptions& options)
    {
        ...
        std::cerr
            << std::format("[cc_abi_gen] done: {} domain(s) regenerated\n", m_pilot_domains.size());
    ...
    int run_check(const CliOptions& options)
    {
        ...
        for (const std::string& domain: m_pilot_domains) {
            ...
            all_identical =
                report_diff(*model, paths.m_builder_cppm, repo_root, false) && all_identical;
            all_identical =
                report_diff(*model, paths.m_sonic_cppm, repo_root, true) && all_identical;
    ...
    std::vector<std::string> m_pilot_domains{"plugin"};
