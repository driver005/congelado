...
export module cc_tmp:types_numeric_op;
...
export {
    ...
    namespace tensorflow {
        ...
        {
        public:
            explicit UnaryOp(OpKernelConstruction* context) :
                OpKernel(context)
            {
        ...
        {
        public:
            explicit BinaryOp(OpKernelConstruction* context) :
                OpKernel(context)
            {
                const DataType dt = DataTypeToEnum<T>::v();
        ...
        public:
            ...
            void Compute(OpKernelContext* context) override
            {
                ...
                OP_REQUIRES_OK(
                    context,
                    context->forward_input_or_allocate_output({0}, 0, input.shape(), &output)
                );
        ...
        public:
            ...
            void Compute(OpKernelContext* context) override
            {
                ...
                OP_REQUIRES_OK(
                    context,
                    context->forward_input_or_allocate_output({0, 1}, 0, a.shape(), &output)
                );
