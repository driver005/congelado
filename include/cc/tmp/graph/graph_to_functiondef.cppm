...
export module cc_tmp:graph_graph_to_functiondef;
...
export {
    ...
    namespace tensorflow {
        ...
        Status GraphToFunctionDef(const Graph& graph, const string& name, FunctionDef* fdef);
    ...
    // ==================================================================
    // Implementation: graph_to_functiondef.cc
    // ==================================================================
    ...
    namespace tensorflow {
        namespace {
            ...
            std::string NodeNameMapping::UniquifyHelper(const std::string& name)
            {
                ...
                while (true) {
                    ...
                    it.first->second++;
                    if (used_names_.emplace(candidate, 0).second) {
                        return candidate;
                    }
                }
            }
            ...
            )
            {
                ...
                for (int r = 0; r < fdef->signature().output_arg_size(); ++r) {
                    ...
                    if (outputs[r].node->IsRetval()) {
                        ...
                        return_value = absl::StrCat(outputs[r].node->name(), ":", outputs[r].index);
                    }
                ...
                for (int i = 0; i < control_outputs.size(); ++i) {
                    ...
                    if (signature_name.empty()) {
                        return absl::InvalidArgumentError("Control output name must be not empty");
                    }
        ...
        absl::Status
        GraphToFunctionDef(const Graph& graph, const std::string& name, FunctionDef* fdef)
        {
