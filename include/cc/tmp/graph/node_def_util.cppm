...
export {
    ...
    namespace tensorflow {
        ...
        std::string SummarizeNodeDef(const NodeDef& node_def, int max_inputs_in_summary = -1);
        ...
        void AddNodeAttr(absl::string_view name, const AttrValue& value, NodeDef* node_def);
        void AddNodeAttr(absl::string_view name, AttrValue&& value, NodeDef* node_def);
        void AddNodeAttr(absl::string_view name, absl::string_view value, NodeDef* node_def);
        ...
        template<class T>
        void AddNodeAttr(absl::string_view name, std::initializer_list<T> value, NodeDef* node_def)
        {
        ...
        public:
            ...
            // NotFound status.
            absl::Status Find(absl::string_view attr_name, const AttrValue** attr_value) const;
            absl::Status
            FindByString(const std::string& attr_name, const AttrValue** attr_value) const;
        ...
        private:
            ...
            absl::Status CheckFind(absl::string_view attr_name, const AttrValue* attr_value) const;
        ...
        absl::Status GetNodeAttr(
            const AttrSlice& attrs,
            absl::string_view attr_name,
            std::vector<TensorShapeProto>* value
        ); // type "list(shape)"
        absl::Status GetNodeAttr(
            const AttrSlice& attrs,
            absl::string_view attr_name,
            std::vector<TensorShape>* value
        ); // type "list(shape)"
        absl::Status GetNodeAttr(
            const AttrSlice& attrs,
            absl::string_view attr_name,
            std::vector<PartialTensorShape>* value
        ); // type "list(shape)"
        absl::Status GetNodeAttr(
            const AttrSlice& attrs,
            absl::string_view attr_name,
        ...
        const std::string& GetNodeAttrString(const AttrSlice& attrs, absl::string_view attr_name);

        // Specialization to parse an attribute directly into a Padding enum.
        absl::Status
        GetNodeAttr(const AttrSlice& attrs, absl::string_view attr_name, Padding* value);
        ...
        // REQUIRES: ValidateOpDef(op_def).ok()
        absl::Status
        InputTypesForNode(const NodeDef& node_def, const OpDef& op_def, DataTypeVector* inputs);
        ...
        // REQUIRES: ValidateOpDef(op_def).ok()
        absl::Status
        OutputTypesForNode(const NodeDef& node_def, const OpDef& op_def, DataTypeVector* outputs);
        absl::Status
        OutputTypesForNode(const AttrSlice& attrs, const OpDef& op_def, DataTypeVector* outputs);

        // Computes the input and output types for a specific node.
        // REQUIRES: ValidateOpDef(op_def).ok()
        absl::Status InOutTypesForNode(
        ...
        // REQUIRES: ValidateOpDef(op_def).ok()
        absl::Status
        NumOutputsForNode(const NodeDef& node_def, const OpDef& op_def, int* num_outputs);
        ...
        absl::Status AddPrefixAndSuffixToNode(
        ...
        absl::Status MaybeAddPrefixToColocationConstraints(
            const std::unordered_set<std::string>& match,
            absl::string_view prefix,
            NodeDef* node_def
        );
    ...
    namespace tensorflow {
        ...
        AttrSlice::AttrSlice(const AttrValueMap* a) :
            ndef_(nullptr),
            attrs_(a)
        {
        }
        ...
        std::string SummarizeAttrsHelper(AttrSlice attrs, absl::string_view device)
        {
            ...
            for (const std::string& attr_name: attr_names) {
                ...
                first = false;
                absl::StrAppend(&ret, attr_name, "=", SummarizeAttrValue(*attrs.Find(attr_name)));
            }
        ...
        std::string AttrSlice::SummarizeNode() const
        {
            return ndef_ ? SummarizeNodeDef(*ndef_)
                         : absl::StrCat("[", SummarizeAttrsHelper(*this, absl::string_view()), "]");
        }
        ...
        std::string AttrSlice::DebugString() const
        {
            ...
            for (const auto& it: *this) {
                ...
                attr_key_vals.push_back(absl::StrCat(name, "=", SummarizeAttrValue(attr_value)));
            }
        ...
        absl::Status
        AttrSlice::CheckFind(absl::string_view attr_name, const AttrValue* attr_value) const
        {
            if (attr_value != nullptr) {
                return absl::OkStatus();
            }
            absl::Status s =
                absl::NotFoundError(absl::StrCat("No attr named '", attr_name, "' in NodeDef:"));
        ...
        absl::Status
        AttrSlice::Find(absl::string_view attr_name, const AttrValue** attr_value) const
        {
        ...
        absl::Status
        AttrSlice::FindByString(const std::string& attr_name, const AttrValue** attr_value) const
        {
...
#define DEFINE_GET_ATTR(TYPE, FIELD, ATTR_TYPE, APPEND_OP, CAST, ...)                              \
    Status GetNodeAttr(const AttrSlice& attrs, StringPiece attr_name, TYPE* value)                 \
    {                                                                                              \
    ...
    Status GetNodeAttr(const AttrSlice& attrs, StringPiece attr_name, std::vector<TYPE>* value)    \
    {                                                                                              \
        const AttrValue* attr_value;                                                               \
        TF_RETURN_IF_ERROR(attrs.Find(attr_name, &attr_value));                                    \
...
#define DEFINE_TRY_GET_ATTR(TYPE, FIELD, ATTR_TYPE, APPEND_OP, CAST, ...)                          \
    bool TryGetNodeAttr(const AttrSlice& attrs, StringPiece attr_name, TYPE* value)                \
    {                                                                                              \
    ...
    bool TryGetNodeAttr(const AttrSlice& attrs, StringPiece attr_name, std::vector<TYPE>* value)   \
    {                                                                                              \
        const AttrValue* attr_value = attrs.Find(attr_name);                                       \
        if (attr_value == nullptr) {                                                               \
            return false;                                                                          \
        ...
        DEFINE_GET_ATTR(
            ...
            if (static_cast<int64_t>(static_cast<int32_t>(v)) != v) {
                return absl::InvalidArgumentError(
                    absl::StrCat("Attr ", attr_name, " has value ", v, " out of range for an int32")
                );
            }
        )
        DEFINE_TRY_GET_ATTR(
        ...
        DEFINE_TRY_GET_ATTR(DataType, type, "type", emplace_back, static_cast<DataType>(v), ;)
...
#undef DEFINE_GET_ATTR
        ...
        absl::Status
        GetNodeAttr(const AttrSlice& attrs, absl::string_view attr_name, DataTypeVector* value)
        {
            const AttrValue* attr_value;
            TF_RETURN_IF_ERROR(attrs.Find(attr_name, &attr_value));
        ...
        absl::Status
        GetNodeAttr(const AttrSlice& attrs, absl::string_view attr_name, const TensorProto** value)
        {
            const AttrValue* attr_value;
            TF_RETURN_IF_ERROR(attrs.Find(attr_name, &attr_value));
        ...
        absl::Status
        GetNodeAttr(const AttrSlice& attrs, absl::string_view attr_name, const NameAttrList** value)
        {
            const AttrValue* attr_value;
            TF_RETURN_IF_ERROR(attrs.Find(attr_name, &attr_value));
        ...
        namespace { // Helper for InOutTypesForNode().
            ...
            absl::Status AddArgToSig(
                const NodeDefOrAttrSlice& node_or_attrs,
            ...
            )
            {
                ...
                if (!arg_def.number_attr().empty()) {
                    ...
                    int64_t repeats = -1;
                    TF_RETURN_IF_ERROR(GetNodeAttr(node_or_attrs, arg_def.number_attr(), &repeats));
                    ...
                    if (!arg_def.type_attr().empty()) {
                        DataType dtype;
                        TF_RETURN_IF_ERROR(GetNodeAttr(node_or_attrs, arg_def.type_attr(), &dtype));
                    ...
                    const AttrValue* attr_value;
                    TF_RETURN_IF_ERROR(
                        AttrSlice(node_or_attrs).FindByString(arg_def.type_attr(), &attr_value)
                    );
                    sig->push_back(attr_value->type());
                } else if (!arg_def.type_list_attr().empty()) {
                    const AttrValue* attr_value;
                    TF_RETURN_IF_ERROR(
                        AttrSlice(node_or_attrs).FindByString(arg_def.type_list_attr(), &attr_value)
                    );
        ...
        )
        {
            ...
            return absl::InvalidArgumentError(
                absl::StrCat("Input ", input_port, " not found for node ", node_def.name())
            );
        }
        ...
        )
        {
            ...
            return absl::InvalidArgumentError(
                absl::StrCat("Output ", output_port, " not found for node ", node_def.name())
            );
        }
        ...
        {
            if (inputs != nullptr) {
                TF_RETURN_IF_ERROR(NameRangesHelper(attrs, op_def.input_arg(), op_def, inputs));
            }
            if (outputs != nullptr) {
        ...
        namespace {
            ...
            const absl::string_view kColocationGroupPrefixStringPiece(kColocationGroupPrefix);

        } // namespace

        absl::Status ValidateOpInput(const std::string& input_name, bool* is_control_input)
        {
        ...
        )
        {
            ...
            if (!allow_multiple_formatted_node && absl::StrContains(status.message(), "{{node ")) {
        ...
        void AddNodeAttr(absl::string_view name, const AttrValue& value, NodeDef* node_def)
        {
            node_def->mutable_attr()->insert(AttrValueMap::value_type(std::string(name), value));
        }
