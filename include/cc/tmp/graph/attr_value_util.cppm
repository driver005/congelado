...
export module cc_tmp:graph_attr_value_util;
...
export {
    ...
    namespace tensorflow {
        ...
        absl::Status AttrValueHasType(const AttrValue& attr_value, absl::string_view type);
        ...
        bool ParseAttrValue(absl::string_view type, absl::string_view text, AttrValue* out);
    ...
    // ==================================================================
    // Implementation: attr_value_util.cc
    // ==================================================================
    ...
    namespace tensorflow {
        ...
        namespace attr_value_util_internal {
            ...
            int64_t TensorByteSize(const TensorProto& t)
            {
                ...
                if (tensor_byte_size < 0) {
                    VLOG(1) << "Overflow encountered when computing tensor byte size, multiplying "
        ...
        namespace {
            ...
            constexpr int kMaxAttrValueTensorByteSize = 32 * 1'024 * 1'024; // 32mb
            ...
            {
                if (attr_value_util_internal::TensorByteSize(tp) > kMaxAttrValueTensorByteSize) {
                    return DeterministicProtoHash64(tp);
                } else {
            ...
            )
            {
                ...
                // different.
                const int64_t lhs_tensor_bytes = attr_value_util_internal::TensorByteSize(lhs);
                const int64_t rhs_tensor_bytes = attr_value_util_internal::TensorByteSize(rhs);
                ...
                const bool large_expansion = (lhs_proto_bytes < 512 && lhs_tensor_bytes > 4'096);
            ...
            uint64_t AttrValueHash(const AttrValue& a, const TensorProtoHasher& tensor_hash)
            {
                if (a.has_tensor()) {
                    return tensor_hash(a.tensor());
                }

                if (a.has_func()) {
                    const NameAttrList& func = a.func();
                    uint64_t h = Hash64(func.name());
                    std::map<std::string, AttrValue> map(func.attr().begin(), func.attr().end());
            ...
            {
                Tensor t;
                int64_t tensor_byte_size = attr_value_util_internal::TensorByteSize(tensor_proto);
                if (
                    tensor_byte_size > kMaxAttrValueTensorByteSize ||
            ...
            bool ParseAttrValueHelper_TensorNestsUnderLimit(int limit, std::string to_parse)
            {
                int nests = 0;
...
#undef VALIDATE_FIELD
            ...
            if (absl::StartsWith(type, "list(") && !attr_value.has_list()) {
                if (num_set) {
                    return absl::InvalidArgumentError(
                        absl::StrCat("AttrValue missing value with expected type '", type, "'")
                    );
                } else {
            ...
            if (type == "type") {
                if (!DataType_IsValid(attr_value.type())) {
                    return absl::InvalidArgumentError(
                        absl::StrCat("AttrValue has invalid DataType enum: ", attr_value.type())
                    );
                }
                ...
                for (auto as_int: attr_value.list().type()) {
                    ...
                    if (dtype == DT_INVALID) {
                        return absl::InvalidArgumentError("AttrValue contains invalid DataType");
                    }
                }
            }
        ...
        bool ParseAttrValue(absl::string_view type, absl::string_view text, AttrValue* out)
        {
            ...
            if (field_name == "tensor") {
                if (!ParseAttrValueHelper_TensorNestsUnderLimit(kMaxTensorNestDepth, to_parse)) {
                    return false;
                }
            }
...
#define DEFINE_SET_ATTR_VALUE_BOTH(ARG_TYPE, FIELD)                                                \
    ...
    DEFINE_SET_ATTR_VALUE_LIST(gtl::ArraySlice<ARG_TYPE>, FIELD)
        ...
        void SetAttrValue(const absl::Span<const absl::string_view> value, AttrValue* out)
        {
            out->mutable_list()->Clear(); // Create list() even if value empty.
            for (const auto& v: value) {
                out->mutable_list()->add_s(v.data(), v.size());
        ...
        void SetAttrValue(const absl::Span<const PartialTensorShape> value, AttrValue* out)
        {
            out->mutable_list()->Clear(); // Create list() even if value empty.
            for (const auto& v: value) {
                v.AsProto(out->mutable_list()->add_shape());
        ...
        bool AreAttrValuesEqual(const AttrValue& a, const AttrValue& b, bool allow_false_negatives)
        {
            ...
            if (a.has_func() != b.has_func()) {
                ...
                std::unordered_map<std::string, AttrValue> am(af.attr().begin(), af.attr().end());
                for (const auto& bm_pair: bf.attr()) {
                    const auto& iter = am.find(bm_pair.first);
                    if (iter == am.end()) {
                        return false;
                    }
                    if (!AreAttrValuesEqual(iter->second, bm_pair.second, allow_false_negatives)) {
                        return false;
                    }
                    am.erase(iter);
                }
