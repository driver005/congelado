...
export module cc_tmp:ops_op_gen_lib;
...
export {
    ...
    namespace tensorflow {
        ...
        std::string WordWrap(absl::string_view prefix, absl::string_view str, int width);
        ...
        public:
            ...
            absl::Status LoadFileList(Env* env, const std::vector<std::string>& filenames);
    ...
    namespace tensorflow {
        ...
        std::string WordWrap(absl::string_view prefix, absl::string_view str, int width)
        {
            ...
            while (!str.empty()) {
                ...
                if (!str.empty()) {
                    absl::StrAppend(&result, indent_next_line);
                }
            }
        ...
        bool ConsumeEquals(absl::string_view* description)
        {
            if (absl::ConsumePrefix(description, "=")) {
                while (absl::ConsumePrefix(description, " ")) { // Also remove spaces after "=".
                }
                return true;
            }
            return false;
        ...
        // first `split_ch`.
        static bool SplitAt(char split_ch, absl::string_view* orig, absl::string_view* before_split)
        {
        ...
        std::string
        PBTxtToMultiline(absl::string_view pbtxt, const std::vector<std::string>& multi_line_fields)
        {
            std::string ml;
        ...
        static bool FindMultiline(absl::string_view line, size_t colon, std::string* end)
        {
        ...
        std::string PBTxtFromMultiline(absl::string_view multiline_pbtxt)
        {
            ...
            while (!multiline_pbtxt.empty()) {
                ...
                while (!multiline_pbtxt.empty()) {
                    SplitAt('\n', &multiline_pbtxt, &line);
                    if (absl::ConsumePrefix(&line, end)) {
                        break;
                    }
                    if (first) {
                        first = false;
        ...
        static void StringReplace(const std::string& from, const std::string& to, std::string* s)
        {
        ...
        static void RenameInDocs(const std::string& from, const std::string& to, ApiDef* api_def)
        {
        ...
        namespace {
            ...
            absl::Status MergeApiDefs(ApiDef* base_api_def, const ApiDef& new_api_def)
            {
                ...
                // Merge description
                auto description = new_api_def.description().empty() ? base_api_def->description()
                                                                     : new_api_def.description();

                if (!new_api_def.description_prefix().empty()) {
                    description = absl::StrCat(new_api_def.description_prefix(), "\n", description);
                }
                if (!new_api_def.description_suffix().empty()) {
                    description = absl::StrCat(description, "\n", new_api_def.description_suffix());
                }
        ...
        absl::Status ApiDefMap::LoadFileList(Env* env, const std::vector<std::string>& filenames)
        {
        ...
        absl::Status ApiDefMap::LoadFile(Env* env, const std::string& filename)
        {
            ...
            if (!status.ok()) {
                ...
                return errors::CreateWithUpdatedMessage(
                    status,
                    absl::StrCat("Error parsing ApiDef file ", filename, ": ", status.message())
                );
            }
            return absl::OkStatus();
        }
        ...
        absl::Status ApiDefMap::LoadApiDef(const std::string& api_def_file_contents)
        {
            ...
            ApiDefs api_defs;
            TF_RETURN_IF_ERROR(proto_utils::ParseTextFormatFromString(contents, &api_defs));
