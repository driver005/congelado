...
export module cc_tmp:ops_op_def_util;
...
#    define TENSORFLOW_CORE_FRAMEWORK_OP_DEF_UTIL_H_
    ...
    namespace tensorflow {
        ...
        // not raise an error.
        absl::Status OpDefAttrDefaultsUnchanged(const OpDef& old_op, const OpDef& new_op);
        ...
        uint64_t RepeatedAttrDefHash(const protobuf::RepeatedPtrField<OpDef::AttrDef>& a);
        ...
        namespace { // ------ Helper functions ------
            ...
            absl::Status AllowedTypeValue(DataType dt, const OpDef::AttrDef& attr)
            {
                ...
                for (int i = 0; i < allowed_values.list().type_size(); ++i) {
                    ...
                    absl::StrAppend(&allowed_str, DataTypeString(allowed_values.list().type(i)));
                }
...
#define VALIDATE(EXPR, ...)                                                                        \
    do {                                                                                           \
        ...
        static absl::Status ValidateArg(
        ...
        absl::Status ValidateOpDef(const OpDef& op_def)
        {
            ...
            absl::flat_hash_set<absl::string_view> names; // for detecting duplicate names
            for (const auto& attr: op_def.attr()) {
                ...
                bool found = false;
                for (absl::string_view valid:
                     {"string", "int", "float", "bool", "type", "shape", "tensor", "func"}) {
...
#undef VALIDATE
        ...
        namespace {
            ...
            std::string SummarizeArgs(const protobuf::RepeatedPtrField<OpDef::ArgDef>& args)
            {
                std::string ret;
        ...
        namespace {
            ...
            {
                // Anything -> no restriction : not more restrictive.
                if (!new_attr.has_minimum()) {
                    return false;
                }
                // No restriction -> restriction : more restrictive.
                if (!old_attr.has_minimum()) {
                    return true;
                }
                // If anything that was previously allowed is no longer allowed:
                // more restrictive.
    ...
    if (!(CONDITION)) {                                                                            \
        ...
        bool AttrDefEqual(const OpDef::AttrDef& a1, const OpDef::AttrDef& a2)
        {
            if (std::is_base_of<protobuf::Message, OpDef::AttrDef>()) {
                DCHECK_EQ(
                    7,
                    reinterpret_cast<const protobuf::Message*>(&a1)->GetDescriptor()->field_count()
            ...
            if (a1.has_minimum() != a2.has_minimum()) {
                return false;
            }
            if (a1.has_minimum() && a1.minimum() != a2.minimum()) {
                return false;
            }
            if (!AreAttrValuesEqual(a1.default_value(), a2.default_value())) {
                return false;
            }
        ...
        )
        {
            ...
            for (const OpDef::AttrDef& def: a2) {
                ...
                if (!AttrDefEqual(*iter->second, def)) {
                    return false;
                }
                a1_set.erase(iter);
            }
        ...
        bool OpDefEqual(const OpDef& o1, const OpDef& o2)
        {
            ...
            if (!RepeatedAttrDefEqual(o1.attr(), o2.attr())) {
                return false;
            }
        ...
        uint64_t OpDefHash(const OpDef& o)
        {
            ...
            for (const auto& co: control_output) {
                h = Hash64Combine(h, Hash64(co));
            }
