...
export {
    ...
    namespace tensorflow {
        namespace test {
            ...
            namespace internal_test {
                ...
                IsClose(Eigen::half x, Eigen::half y, double atol = -1.0, double rtol = -1.0);
                ::testing::AssertionResult
                IsClose(float x, float y, double atol = -1.0, double rtol = -1.0);
                ::testing::AssertionResult
                IsClose(double x, double y, double atol = -1.0, double rtol = -1.0);
            } // namespace internal_test
    ...
    namespace tensorflow {
        namespace test {
            ...
            {
                if (x.dtype() != y.dtype()) {
                    return ::testing::AssertionFailure() << "Tensors have different dtypes ("
                                                         << x.dtype() << " vs " << y.dtype() << ")";
                }
            ...
            ::testing::AssertionResult IsSameShape(const Tensor& x, const Tensor& y)
            {
                if (!x.IsSameSize(y)) {
                    return ::testing::AssertionFailure()
                           << "Tensors have different shapes (" << x.shape().DebugString() << " vs "
                           << y.shape().DebugString() << ")";
                }
                return ::testing::AssertionSuccess();
            }
            ...
            template<>
            ::testing::AssertionResult EqualFailure<int8_t>(const int8_t& x, const int8_t& y)
            {
            ...
            static ::testing::AssertionResult IsEqual(Eigen::half x, Eigen::half y, Tolerance t)
            {
                // We consider NaNs equal for testing.
                if (Eigen::numext::isnan(x) && Eigen::numext::isnan(y)) {
                    return ::testing::AssertionSuccess();
            ...
            static ::testing::AssertionResult IsEqual(tsl::bfloat16 x, tsl::bfloat16 y, Tolerance t)
            {
                // We consider NaNs equal for testing.
                if (Eigen::numext::isnan(x) && Eigen::numext::isnan(y)) {
                    return ::testing::AssertionSuccess();
            ...
            IsEqual(const std::complex<T>& x, const std::complex<T>& y, Tolerance t)
            {
            ...
            template<typename T>
            static void
            ExpectEqual(const Tensor& x, const Tensor& y, Tolerance t = Tolerance::kDefault)
            {
                const T* Tx = x.unaligned_flat<T>().data();
                const T* Ty = y.unaligned_flat<T>().data();
                auto size = x.NumElements();
            ...
            template<typename T>
            static ::testing::AssertionResult
            IsClose(const T& x, const T& y, const T& atol, const T& rtol)
            {
                // We consider NaNs equal for testing.
                if (Eigen::numext::isnan(x) && Eigen::numext::isnan(y)) {
                    return ::testing::AssertionSuccess();
                ...
                if (x == y) {
                    return ::testing::AssertionSuccess(); // Handle infinity.
                }
            ...
            template<typename T>
            static void ExpectClose(const Tensor& x, const Tensor& y, double atol, double rtol)
            {
                ...
                for (decltype(size) i = 0; i < size; ++i) {
                    EXPECT_TRUE(IsClose(Tx[i], Ty[i], typed_atol, typed_rtol))
                        ...
                        << " Ty[i] = " << Ty[i];
                    ASSERT_LT(num_failures, max_failures) << "Too many mismatches (atol = " << atol
                                                          << " rtol = " << rtol << "), giving up.";
                }
            ...
            ::testing::AssertionResult
            internal_test::IsClose(float x, float y, double atol, double rtol)
            {
                return test::IsClose(x, y, GetTolerance<float>(atol), GetTolerance<float>(rtol));
            }

            ::testing::AssertionResult
            internal_test::IsClose(double x, double y, double atol, double rtol)
            {
                return test::IsClose(x, y, GetTolerance<double>(atol), GetTolerance<double>(rtol));
            }
