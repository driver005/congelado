...
#define DEFINE_PROTO_FIELD_HELPER(TYPE, FIELDNAME)                                                 \
    ...
    {                                                                                              \
    public:                                                                                        \
        typedef decltype(std::declval<TensorProto>().FIELDNAME##_val(0)) FieldType;                \
        typedef decltype(std::declval<TensorProto>().FIELDNAME##_val()) RepeatedFieldType;         \
        typedef decltype(std::declval<TensorProto>()                                               \
                             .mutable_##FIELDNAME##_val()) MutableRepeatedFieldType;               \
...
#undef DEFINE_PROTO_HELPER
                ...
                public:
                    ...
                    template<typename IterType>
                    static void
                    CopyToTensorContent(IterType begin, IterType end, TensorProto* proto)
                    {
            ...
            template<typename Type>
            typename std::enable_if<internal::TensorProtoHelper<Type>::value, TensorProto>::type
            CreateTensorProto(const std::vector<Type>& values, const absl::Span<const size_t> shape)
            {
            ...
            namespace internal {
                ...
                )
                {
                    ...
                    if constexpr (sizeof(FieldType) == sizeof(T)) {
                        ...
                        if (new_num_values >= tmp.max_size()) {
                            return false;
                        }
...
#undef HANDLE_COMPRESS_CASE
            ...
            absl::Status MakeShape(const Tensor& shape, TensorShape* out)
            {
                ...
                if (shape.dtype() == DataType::DT_INT32) {
                    ...
                    return absl::InvalidArgumentError("shape must be a vector of {int32,int64}.");
                }
            }
