...
export module cc_tmp:shape_shape_inference_testutil;
...
export {
    ...
    namespace tensorflow {
        ...
        struct ShapeInferenceTestOp
        {
            ...
            explicit ShapeInferenceTestOp(StringPiece name) :
                name(string(name))
            {
            }
            ...
            std::vector<std::vector<ShapeAndType>*> input_resource_handle_shapes_and_types;
...
#define INFER_ERROR(error_substring, op, i)                                                        \
    {                                                                                              \
        std::string error_message =                                                                \
            ::tensorflow::shape_inference::ShapeInferenceTestutil::InferShapes(op, i, "e")         \
                .error_message();                                                                  \
    ...
    // ==================================================================
    // Implementation: shape_inference_testutil.cc
    // ==================================================================
    ...
    namespace tensorflow {
        namespace shape_inference {
            ...
            )
            {
                ...
                std::vector<std::string> expected_outs_v = str_util::Split(expected_outs, ';');
                            ...
                            if (in_dim_idx.first != -1) {
                                return absl::UnknownError(
                                    absl::StrCat(
                                        ...
                                        "DimensionHandle "
                                        "from the input to the output, but did. Note that even if "
                                        "the "
                                        "passed through output has the same dimension value as the "
                                        "expected value, this is considered a failure for the "
                                        "test; "
            ...
            )
            {
                ...
                while (scanner.Peek() != ']') {
                    ...
                    if (scanner.Peek() == ',') {
                        ...
                        return absl::InvalidArgumentError(
                            absl::StrCat("Invalid input spec (] not found in dim shape): ", spec)
                        );
                    }
                }
                if (!scanner.OneLiteral("]").Eos().GetResult()) {
                    return absl::InvalidArgumentError("Malformed shape spec: did not end in ']'.");
                }
