...
export module cc_tmp:runtime_run_handler;
...
export {
    ...
    namespace tensorflow {
        ...
        public:
            ...
            std::unique_ptr<RunHandler>
            Get(int64_t step_id = 0,
        ...
        namespace internal {
            ...
            typedef Eigen::RunQueue<Task, 1'024> Queue;
            ...
            public:
                ...
                Task FindTask(
                    int searching_range_start,
                    int searching_range_end,
                    int thread_id,
                    int sub_thread_pool_id,
                    int max_blocking_inflight,
                ...
                void WaitForWork(bool is_blocking, int thread_id, int32_t max_blocking_inflight);
    ...
    namespace tensorflow {
        ...
        namespace internal {
            ...
            void RunHandlerEnvironment::ExecuteTask(const Task& t)
            {
                ...
                tracing::ScopedRegion region(tracing::EventCategory::kRunClosure, t.f->trace_id);
                t.f->f();
            }
            ...
            ThreadWorkSource::ThreadWorkSource() :
                non_blocking_work_sharding_factor_(
                    static_cast<int32>(
                        ParamFromEnvWithDefault("TF_RUN_HANDLER_NUM_OF_NON_BLOCKING_QUEUES", 1)
                    )
                ),
            ...
            Task ThreadWorkSource::PopNonBlockingTask(int start_index, bool search_from_all_queue)
            {
                Task t;
            ...
            void RunHandlerThreadPool::WorkerLoop(int thread_id, bool may_steal_blocking_work)
            {
        ...
        public:
            explicit Impl(int num_inter_op_threads, int num_intra_op_threads) :
                ...
                queue_waiters_(ParamFromEnvWithDefault("TF_RUN_HANDLER_NUM_SUB_THREAD_POOL", 2)),
            ...
            {
                thread_local std::unique_ptr<Eigen::MaxSizeVector<internal::ThreadWorkSource*>>
                    thread_work_sources =
                ...
                RunHandler::Impl* handler_impl;
                {
                    ...
                    if (!has_free_handler()) {
                        ...
                        TRACESTRING(
                            strings::StrCat(
                                "RunHandlerPool::Impl::Get waiting for a handler "
                        ...
                        if (timeout_in_ms == 0) {
                            mu_.Await(Condition(this, &Impl::has_free_handler));
                                       ...
                                       EnvTime::NowNanos() + timeout_in_ms * 1'000 * 1'000
                                   )) {
                            return nullptr;
                        }
                    }
            ...
            std::vector<int64_t> GetActiveHandlerPrioritiesForTesting() TF_LOCKS_EXCLUDED(mu_)
            {
                mutex_lock l(mu_);
        ...
        private:
            void RecomputePoolStats(
                int num_active_requests,
                uint64 version,
                const Eigen::MaxSizeVector<internal::ThreadWorkSource*>& thread_work_sources
            );
        ...
        )
        {
            ...
            for (int i = 0; i < num_active_requests; ++i) {
                while (sub_thread_pool_id < sub_thread_pool_end_request_percentage_.size() - 1 &&
        ...
        {
            if (iterations_++ % 50'000 == 10 && VLOG_IS_ON(1)) {
                ...
                for (int i = 0; i < num_active_requests; ++i) {
                    ...
                    times_str += strings::StrCat((now - (*it)->start_time_us()) / 1000.0, " ms.");
        ...
        {
            return run_handler_impl_->pool_impl_->run_handler_thread_pool()->CurrentThreadId();
        }

        void RunHandler::Impl::ThreadPoolInterfaceWrapper::Schedule(std::function<void()> fn)
        {
        ...
        void RunHandler::Impl::ScheduleInterOpClosure(std::function<void()> fn)
        {
            ...
            pool_impl_->run_handler_thread_pool()->AddWorkToQueue(tws(), true, std::move(fn));
        }

        void RunHandler::Impl::ScheduleIntraOpClosure(std::function<void()> fn)
        {
            VLOG(3) << "Scheduling intra work for " << tws()->GetTracemeId();
            pool_impl_->run_handler_thread_pool()->AddWorkToQueue(tws(), false, std::move(fn));
        }
        ...
        RunHandlerPool::RunHandlerPool(int num_inter_op_threads, int num_intra_op_threads) :
        ...
        std::vector<int64_t> RunHandlerPool::GetActiveHandlerPrioritiesForTesting() const
        {
            return impl_->GetActiveHandlerPrioritiesForTesting();
        }

        RunHandler::RunHandler(Impl* impl) :
            impl_(impl)
        {
        }
