...
export module cc_tmp:runtime_local_rendezvous;
...
export {
    ...
    namespace tensorflow {
        ...
        private:
            ...
            static std::vector<tsl::core::RefCountPtr<Rendezvous>>&
                aborted_rendezs_ TF_GUARDED_BY(aborted_rendezs_mu_);
    ...
    // ==================================================================
    // Implementation: local_rendezvous.cc
    // ==================================================================
    ...
    namespace tensorflow {
        ...
        struct LocalRendezvous::Item
        {
            ...
            Item(
                ...
                Rendezvous::DoneCallback waiter,
            ...
            for (int i = 0; i < num_buckets_; ++i) {
                auto& bucket = table_buckets_[i];
                {
                    ...
                    while (bucket.pending_callback_counter != 0) {
                        bucket.pending_callback_cond_var.wait_for(l, std::chrono::milliseconds(50));
                    }
                }
        ...
        absl::Status LocalRendezvous::Send(
            const Rendezvous::ParsedKey& key,
            const Rendezvous::Args& send_args,
            const Tensor& val,
            const bool is_dead
        )
        {
            ...
            DVLOG(2) << "Send " << this << " " << key_hash.ToString() << " " << key.FullKey();
            ...
            (*item->recv_state.waiter)(absl::OkStatus(), send_args, item->args, val, is_dead);
            {
                mutex_lock l(bucket.mu);
                bucket.pending_callback_counter--;
        ...
        void LocalRendezvous::RecvAsync(
            const Rendezvous::ParsedKey& key,
            const Rendezvous::Args& recv_args,
            Rendezvous::DoneCallback done
        )
        {
            KeyHash key_hash = KeyHash(key.FullKey());
            DVLOG(2) << "Recv " << this << " " << key_hash.ToString() << " " << key.FullKey();
                        ...
                        !cm->RegisterCallback(token, [this, token, key_hash, &bucket] {
                            ...
                            Item* item = nullptr;
                            {
                                ...
                                if (it != bucket.table.end()) {
                                    ItemQueue* queue = &it->second;
                                    // Find an item in the queue with a cancellation token that
                                    // matches `token`, and remove it.
                                        ...
                                        for (Item *prev = nullptr, *curr = queue->head;
                                            ...
                                            if (curr->recv_state.cancellation_token == token) {
                                                ...
                                                if (queue->head->next == nullptr) {
                                                    // We have a single-element queue, so we can
        ...
        std::vector<tsl::core::RefCountPtr<Rendezvous>>& LocalRendezvous::aborted_rendezs_ =
        ...
        void LocalRendezvous::DoAbort(const absl::Status& status)
        {
            ...
            if (status.code() != absl::StatusCode::kOutOfRange) {
                LOG_EVERY_POW_2(WARNING) << "Local rendezvous is aborting with status: " << status;
            }
