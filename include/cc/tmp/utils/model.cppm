            ...
            namespace model {
                ...
                struct Parameter
                {
                    Parameter(
                        ...
                        value(state == nullptr || state->value == kAutotune ? min : state->value),
                        min(min),
                        max(max),
                ...
                {
                public:
                    explicit RamBudgetManager(int64_t budget) :
                        budget_(budget)
                    {
                        if (budget <= 0) {
                            LOG(WARNING)
                                << "RAM budget is " << budget
                ...
                public:
                    ...
                    int64_t id() const TF_LOCKS_EXCLUDED(mu_)
                    {
                        return id_;
                    }
                    ...
                    int64_t num_elements() const TF_LOCKS_EXCLUDED(mu_)
                    {
                    ...
                    std::shared_ptr<Node> output_shared()
                    {
                        return output_weak_ptr_.lock();
                    }
                    ...
                    bool is_recording() TF_LOCKS_EXCLUDED(mu_)
                    {
                        return work_start_ > 0;
                    }
                    ...
                    ParameterValue(const std::string& parameter_name) const TF_LOCKS_EXCLUDED(mu_)
                    {
                        tf_shared_lock l(mu_);
                    ...
                    double TotalProcessingTime(NodeValues* processing_times) TF_LOCKS_EXCLUDED(mu_);
                    ...
                    static absl::Status FromProto(
                        ModelProto::Node node_proto,
                        std::shared_ptr<Node> output,
                ...
                protected:
                        ...
                        if (previous_processing_time_ == 0) {
                            ...
                            processing_time_ema_ =
                                ...
                                kProcessingTimeEmaWeight *
                                    static_cast<double>(
                                        processing_time_ - previous_processing_time_
                                    );
                        }
                    ...
                    Clone(std::shared_ptr<Node> output) const TF_SHARED_LOCKS_REQUIRED(mu_) = 0;
                    ...
                    double OutputTimeGradientsForInputs(
                        const NodeValues& output_time_gradients
                    ) const TF_SHARED_LOCKS_REQUIRED(mu_);

                    // Computes the input time for this node and stores it in `input_times`.
                    virtual void InputTimeLocked(
                    ...
                    virtual void OutputTimeLocked(
                        const NodeValues& input_times,
                        ParameterGradients* gradients,
                        NodeValues* output_times,
                        NodeValues* output_time_gradients
                    ) const TF_SHARED_LOCKS_REQUIRED(mu_) = 0;
                    ...
                    double TotalProcessingTimeForInputs(
                        const NodeValues& total_processing_times
                    ...
                    // mutex locked.
                    ModelParameters
                    CollectTunableParametersLocked() const TF_SHARED_LOCKS_REQUIRED(mu_);
                    ...
                    static absl::Status
                    FromProtoHelper(ModelProto::Node node_proto, std::shared_ptr<Node> node);
                    ...
                    std::optional<int64_t>
                        estimated_element_size_ TF_GUARDED_BY(mu_) = std::nullopt;
                };
                ...
                public:
                    ...
                    static absl::Status
                    FromProto(ModelProto model_proto, std::unique_ptr<Model>* model);
                ...
                private:
                    ...
                    // upsized.
                    absl::flat_hash_map<Node*, Parameter*>
                    CollectBufferParametersToUpsize(std::shared_ptr<Node> snapshot);
                ...
                public:
                    ...
                    GetStageNodes(std::shared_ptr<Node> stage_root) const;
        ...
        namespace data {
            namespace model {
                ...
                namespace {
                    ...
                    {
                    public:
                        explicit TargetTimeCalculator(
                            const std::vector<uint64_t>& points_usec,
                    ...
                    private:
                        // Returns the remaining points after removing outliers from the original
                        // set of points.
                        ...
                        {
                            uint64_t sum =
                                std::accumulate(points_usec.begin(), points_usec.end(), 0);
                            *mean =
                                static_cast<double>(sum) / static_cast<double>(points_usec.size());
                            double accum = 0.0;
                            for (auto point: points_usec) {
                    ...
                    )
                    {
                        ...
                        // transformations's processing time is greater than essential rate times
                        // the average transformation self processing time.
                    ...
                    )
                    {
                        ...
                        for (auto& pair: *parameters) {
                            if (std::round(pair.second->value) != pair.second->max) {
                                ...
                                if (gradient) {
                                    max_abs_derivative =
                                        std::max(max_abs_derivative, std::abs(*gradient));
                                }
                            }
                        }
                        for (auto& pair: *parameters) {
                            ...
                            if (gradient) {
                                new_value = pair.second->value -
                                            kDescentStep * (*gradient) / max_abs_derivative;
                    ...
                    absl::Status ModelToProtoHelper(std::shared_ptr<Node> output, ModelProto* model)
                    {
                    ...
                    absl::Status
                    ModelFromProtoHelper(ModelProto model, std::shared_ptr<Node>* output)
                    {
                    ...
                    public:
                        ...
                        // The ratio of an InterleaveMany node is `1/cycle_length`. If cycle length
                        // is not available, we approximate it by `1/input_size`. The input size
                        // does not include the original input dataset that generates other input
                        // datasets of interleave nodes.
                        double Ratio() const override
                        {
                            ...
                            // After cl/436244658, `cycle_length` can not be `nullptr`. The
                            // remaining part of this function is used to approximate `Ratio()` of
                            // this node for model proto that was created before the CL.
                            ...
                            std::size_t input_size = 1;
                            {
                                ...
                                if (inputs_.size() >= 2) {
                                    ...
                                    // Some interleave datasets have 2 different inputs: the
                                    // original input dataset and the generated input datasets when
                                    // interleave is iterated, and some do not.
                    ...
                    protected:
                        ...
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            ...
                            // Here `inherited_input_time + SelfProcessingTimeLocked()` is the
                            // average input time for InterleaveMany node to call one of the
                            ...
                            // node, the average input time for any of the `(num_inputs() - 1)`
                            // input nodes to be called is computed as:
                            double input_time =
                                (inherited_input_time + SelfProcessingTimeLocked()) *
                        ...
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            ...
                            if (gradients) {
                                ...
                                // Set derivatives w.r.t. tunable parameters of the subtree rooted
                                // in the first input equal to 0 since its output time is excluded
                                // from computations.
                            ...
                            (*output_times)[long_name()] =
                                self_processing_time + inputs_output_time;
                        }
                    ...
                    public:
                        ...
                        double Ratio() const override
                        {
                            ...
                            // Cycle length is not available, use 1/min(input_size, parallelism) as
                            // the ratio.
                        ...
                        {
                            double parallelism = 1.0;
                            auto* parallelism_parameter =
                                gtl::FindOrNull(parameters_, kParallelism);
                    ...
                    protected:
                        ...
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            ...
                            // Here `inherited_input_time + SelfProcessingTimeLocked()` is the
                            // average input time for AsyncInterleaveMany node to call one of the
                            // `(num_inputs()
                            // - 1)` input nodes (except first input) to return an element.
                            // Regardless of the `block_length` parameter of AsyncInterleaveMany
                            // node, the average input time for any of the `(num_inputs() - 1)`
                            // input nodes to be called is computed as:
                            double input_time =
                                (inherited_input_time + SelfProcessingTimeLocked()) *
                            ...
                            if (gradients) {
                                ...
                                if (parameter && (*parameter)->state->tunable) {
                                    (*gradients)[std::make_pair(long_name(), (*parameter)->name)] =
                                        buffer_size_der -
                                        producer_time_der * producer_time / parallelism;
                                }
                            } else {
                                wait_time = ComputeWaitTime(
                    ...
                    {
                    public:
                        KnownRatio(Node::Args args, double ratio) :
                            Node(args),
                            ratio_(ratio)
                        {
                        }
                    ...
                    protected:
                        ...
                        // The input time is the sum of inherited input time and self processing
                        // time, divided by `ratio_`.
                        void InputTimeLocked(
                            NodeValues* input_times
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                    ...
                    protected:
                        ...
                        // The input time is the sum of inherited input time and parallelism
                        // adjusted self processing time, divided by `Ratio()`.
                        void InputTimeLocked(
                            NodeValues* input_times
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                        ...
                        // The output time is the sum of parallelism adjusted self processing time
                        // and expected wait time from the buffer model estimated using
                        // `ComputeWaitTime(producer_time, consumer_time, parallelism, ...)`, where
                        // `producer_time` is the product of `Ratio()` and the sum of output times
                        // of inputs, `consumer_time` is the product of `Ratio()` and the
                        // `input_time` specified through `input_times` (since for each element
                        // stored in the buffer, the inputs need to be called `Ratio()` times), and
                        // if the node has parallelism parameter, then `buffer_size` is derived from
                        // `parallelism`.
                        //
                        // Current implementation assumes that there is at most 1 parameter per
                        // node.
                        void OutputTimeLocked(
                            const NodeValues& input_times,
                            ParameterGradients* gradients,
                        ...
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            ...
                            if (parallelism_parameter) {
                                ...
                                if (ratio == 0.0) {
                                    ...
                                    // AsyncKnownRatio nodes with ratio >= 1. For MapAndBatch, we
                                    // create `parallelism` threads to apply the function on
                                    // elements from input dataset, while one element in the buffer
                                    // actually corresponds to `Ratio()` elements from input
                                    // dataset. So we adjust the `buffer_size` by dividing
                                    // `Ratio()`.
                        ...
                        double MaximumBufferedBytes() const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            if (is_legacy_prefetch_autotuned_) {
                                // This node's memory is managed by a legacy prefetch autotuner
                            ...
                            if (parameter) {
                                if (memory_ratio_ == 0) {
                                    result +=
                                        (*parameter)->value * AverageBufferedElementSizeLocked();
                                } else {
                                    // The estimation is currently not accurate for
                                    // MapAndBatchDataset for the maximum buffer size does not match
                                    // `num_parallel_calls` parameter.
                    ...
                    private:
                        ...
                        // introduced by one buffered element. The value is defined to correctly
                        // estimate RAM budget bound with given num_parallel_calls (or buffer_size)
                        // combined with the estimated average size of buffered elements.
                    ...
                    protected:
                        ...
                        {
                            return std::make_shared<UnknownRatio>(
                                Args{id_, name_, std::move(output)}
                            );
                        }

                        // The input time is the sum of inherited input time and self processing
                        // time, divided by the ratio estimate.
                        void InputTimeLocked(
                            NodeValues* input_times
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                        ...
                        ) const override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            ...
                            // TODO(jsimsa): The current implementation assumes that the number of
                            // input elements consumed per output is the same across all inputs.
                        ...
                        // The processing time is the sum of the self processing time and the
                        // product of the ratio estimate and the sum of processing times of inputs.
                        void TotalProcessingTimeLocked(
                            absl::flat_hash_map<std::string, double>* processing_times,
                            absl::flat_hash_map<std::string, double>* total_processing_times
                        ) override TF_SHARED_LOCKS_REQUIRED(mu_)
                        {
                            double self_processing_time = SelfProcessingTimeLocked();
                            if (processing_times) {
                    ...
                    protected:
                        ...
                        absl::Status ToProto(ModelProto::Node* node_proto) const override
                        {
                            ...
                            if (is_legacy_prefetch_autotuned_) {
                                // Update buffer_size parameter to make sense from a user
                                // perspective.
                                if (node_proto->parameters_size() != 1) {
                                    return absl::InternalError(
                                        absl::StrCat(
                                            "Expected prefetch node to have one parameter, but it "
                                            "has ",
                                ...
                                // Legacy autotuner only modifies the state_value, not the model
                                // value.
                ...
                double Node::ComputeWaitTime(
                    const double producer_time,
                ...
                )
                {
                    ...
                    if (producer_time == 0) {
                        if (producer_time_derivative) {
                            // Note a common error is `*producer_time_derivative = 0` since p=0 on
                            // the line y=0 doesn't imply dp/dy = 0 there. Actually to compute dp/dy
                            // at (x,0), we need to consider lim_{dy->0+} [p(x,dy)-p(x,0)] / dy,
                            // where p(x,0)=0 and p(x,dy) = [1 - x/dy] / [1 - power(x/dy, n+1)].
                    ...
                    if (consumer_time == 0) {
                        ...
                        if (consumer_time_derivative) {
                            // Note a common error is `*consumer_time_derivative = 0` since p=1 on
                            // the line x=0 doesn't imply dp/dx = 0 there. Actually to compute dp/dx
                            // at (0,y), we need to consider lim_{dx->0+} [p(dx,y)-p(0,y)] / dx,
                            // where p(0,y)=1, p(dx,y) = [1 - dx/y] / [1 - power(dx/y, n+1)] if
                            // y!=0.
                            if (buffer_size == 0) {
                                *consumer_time_derivative = 0.0L;
                            } else {
                    ...
                    if (consumer_time == producer_time) {
                        ...
                        if (producer_time_derivative) {
                            ...
                            // since p=1/(n+1) on the line x=y doesn't imply dp/dy = 0 there.
                            // Actually to compute dp/dy at (y,y), we need to consider lim_{dy->0}
                        ...
                        if (consumer_time_derivative) {
                            ...
                            // compute dp/dx at (x,x), we need to consider lim_{dx->0}
                            // [p(x+dx,x)-p(x,x)] / dx, where p(x,x)=1/(n+1), p(x+dx,x) = [1 -
                            // (x+dx)/x] / [1 - power((x+dx)/x, n+1)].
                ...
                double Node::AverageBufferedElementSizeLocked() const
                {
                    ...
                    if (num_elements_ <= 0) {
                        if (buffered_elements_ <= 0) {
                            ...
                            // If there are no produced elements or buffered elements recorded,
                            // return 0.
                            return 0;
                        }
                ...
                double Node::OutputTimeGradientsForInputs(
                    const Node::NodeValues& output_time_gradients
                ) const
                {
                    double sum = 0;
                    for (auto& input: inputs_) {
                ...
                double
                Node::TotalProcessingTimeForInputs(const Node::NodeValues& total_processing_times)
                {
                        ...
                        if (input->autotune()) {
                            ...
                            if (num_elements < kNumElementsThreshold) {
                                if (input_processing_time_count_ < kCountThreshold) {
                                    ...
                                    // The fewer elements the input has produced so far, the more
                                    // weight is assigned to the prior to reduce volatility.
                                    double prior_weight =
                                        1.0L / static_cast<double>(2 << num_elements);
                                    double prior =
                    ...
                    }
                    {
                        ...
                        for (auto& [node_name, parameter]: buffer_size_parameters) {
                            if (buffered_elements_low_ > 0 &&
                                ...
                                // By default, we increase the buffer sizes by
                                // `kBufferUpsizeMultiplier` if there is enough RAM in upsize. We
                                // cap the downsize by `kBufferDownsizeMultipliter` of the current
                                // size.
                ...
                absl::Status
                Node::FromProtoHelper(ModelProto::Node node_proto, std::shared_ptr<Node> node)
                {
                    {
                        tf_shared_lock l(node->mu_);
                        ...
                        if (node_proto.buffered_elements() == 0) {
                            ...
                            node->buffered_elements_high_.store(
                            ...
                            for (const auto& root: bfs_stage_roots) {
                                ...
                                if (root_timing == nullptr) {
                                    ...
                                    LOG_EVERY_N_SEC(ERROR, TEN_MINUTES)
                                        << "Encounter an error when computing the pipeline "
                                           "processing "
                                           "time for "
                ...
                Model::ModelParameters Model::CollectTunableParameters(std::shared_ptr<Node> node)
                {
                ...
                {
                    Node::NodeVector nodes = snapshot->CollectNodes(TraversalOrder::BFS, IsAnyNode);
                ...
                absl::flat_hash_map<Node*, Parameter*>
                Model::CollectBufferParametersToUpsize(std::shared_ptr<Node> snapshot)
                {
                    Node::NodeVector nodes = snapshot->CollectNodes(TraversalOrder::BFS, IsAnyNode);
                    absl::flat_hash_map<Node*, Parameter*> node_parameters;
                    if (snapshot->IsAsync()) {
                ...
                absl::Status Model::OptimizeLoop(
                    AutotuneAlgorithm algorithm,
                    std::function<int64_t()> cpu_budget_func,
                    double ram_budget_share,
                    std::optional<int64_t> fixed_ram_budget,
                    ...
                    while (true) {
                        {
                            ...
                            while (!cancellation_manager->IsCancelled() &&
                                   last_optimization_ms + optimization_period_ms_ >
                                       current_time_ms) {
                                auto wait_ms = last_optimization_ms + optimization_period_ms_ -
                                               current_time_ms;
                ...
                )
                {
                    ...
                    constexpr int64_t kMaxIterations = 1'000;
                ...
                )
                {
                    ...
                    while (!cancellation_manager->IsCancelled()) {
                        ...
                        if (!best_parameter) {
                            ...
                            VLOG(2)
                                << "Failed to find a tunable parameter that would further "
                ...
                )
                {
                    ...
                    while (!cancellation_manager->IsCancelled()) {
                        ...
                        if (TotalMaximumBufferedBytes(snapshot) >
                            ...
                            // Increasing the parallelism by 1 exceeded ram budget. Reduce it back
                            // and stop optimization because we cannot improve the most critical
                            // stage. There is also a decent chance that the current optimization
                            // iteration is under-optimized. For that reason, return immediately
                            // without updating the parameter state values.
                            parallelism_parameter->value -= 1.0;
                ...
                )
                {
                    ...
                    while (critical_root.first > target_time_nsec) {
                        ...
                        if (cancellation_manager->IsCancelled() ||
                            ...
                            // parallelism by 1 exceeded ram budget. There is a decent chance that
                            // the current optimization iteration is under-optimized. For that
                            // reason, return immediately without updating the parameter state
                            // values after recording the stopping criteria.
                ...
                void Model::OptimizeBuffers(std::shared_ptr<Node> snapshot, int64_t ram_budget)
                {
                ...
                {
                    Node::NodeVector nodes = output()->CollectNodes(TraversalOrder::BFS, IsAnyNode);
                ...
                absl::Status Model::FromProto(ModelProto model_proto, std::unique_ptr<Model>* model)
                {
                    std::unique_ptr<Model> restored_model = std::make_unique<Model>();
                    mutex_lock l(restored_model->mu_);
                    TF_RETURN_IF_ERROR(ModelFromProtoHelper(model_proto, &restored_model->output_));
                    restored_model->id_counter_ = model_proto.id_counter();
                    *model = std::move(restored_model);
                    return absl::OkStatus();
                }

                absl::Status Model::Save(
                    const std::string& fname,
                    std::shared_ptr<Node> snapshot,
                    const OptimizationParams& optimization_params
                )
                ...
                {
                    ModelProto model_proto;
                    TF_RETURN_IF_ERROR(ReadTextOrBinaryProto(Env::Default(), fname, &model_proto));
                ...
                ModelTiming::ModelTiming(std::shared_ptr<Node> root) :
                    root_(root)
                {
                    ...
                    for (const auto& node: bfs_nodes) {
                        ...
                        if (node->output() != nullptr || timing_nodes_.contains(node->output())) {
                            ...
                            auto should_estimate_first_input_ratio = [node]() {
                                ...
                                // reason, the ratio of such inputs is not known statically and is
                                // instead estimated as the ratio of `num_elements` it produces and
                                // `num_elements` its output produces.
                ...
                double
                ModelTiming::ComputeAsyncInterleaveManyInterleavedInputsTotalTime(const Node& node)
                {
                    DCHECK(timing_nodes_.contains(&node));
                ...
                std::vector<std::shared_ptr<Node>>
                ModelTiming::GetStageNodes(std::shared_ptr<Node> stage_root) const
                {
