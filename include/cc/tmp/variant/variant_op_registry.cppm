...
export module cc_tmp:variant_variant_op_registry;
...
#define EIGEN_USE_THREADS
    ...
    namespace tensorflow {
        ...
        public:
            ...
            typedef std::function<absl::Status(OpKernelContext*, const Variant&, Variant*)>
                VariantUnaryOpFn;
            typedef std::function<
                absl::Status(OpKernelContext*, const Variant&, const Variant&, Variant*)>
                VariantBinaryOpFn;
            ...
            typedef std::function<
                absl::Status(const Variant& from, Variant* to, AsyncTensorDeviceCopyFn copy_fn)>
                AsyncVariantDeviceCopyFn;

            // Add a decode function to the registry.
            void RegisterDecodeFn(const std::string& type_name, const VariantDecodeFn& decode_fn);
            ...
            void RegisterDeviceCopyFn(
                const VariantDeviceCopyDirection direction,
            ...
            AsyncVariantDeviceCopyFn*
            GetDeviceCopyFn(const VariantDeviceCopyDirection direction, const TypeIndex& type_index)
            {
                auto found = device_copy_fns.find(std::make_pair(direction, type_index));
                if (found == device_copy_fns.end()) {
                    return nullptr;
                }
                return &found->second;
            }
            ...
            )
            {
                ...
                CHECK_EQ(existing, nullptr) << "Unary VariantUnaryOpFn for type_index: "
            ...
            VariantUnaryOpFn*
            GetUnaryOpFn(VariantUnaryOp op, absl::string_view device, const TypeIndex& type_index)
            {
                auto found = unary_op_fns.find({op, device, type_index});
                if (found == unary_op_fns.end()) {
                    return nullptr;
                }
                return &found->second;
            }
            ...
            )
            {
                ...
                CHECK_EQ(existing, nullptr) << "Unary VariantBinaryOpFn for type_index: "
                                            << port::MaybeAbiDemangle(type_index.name())
                                            << " already registered for device type: " << device;
            ...
            // op, device and TypeName.
            VariantBinaryOpFn*
            GetBinaryOpFn(VariantBinaryOp op, absl::string_view device, const TypeIndex& type_index)
            {
                auto found = binary_op_fns.find({op, device, type_index});
                if (found == binary_op_fns.end()) {
                    return nullptr;
                }
                return &found->second;
            }
        ...
        private:
            ...
            {
                std::size_t operator()(const TypeIndex& x) const
                {
                    return x.hash_code();
                }
            };

            gtl::FlatMap<absl::string_view, VariantDecodeFn, StringPieceHasher> decode_fns;
            ...
            struct FuncTuple
            {
                FuncTuple(const Op& op, const absl::string_view& dev, const TypeIndex& type_index) :
                    op_type_(op),
                    device_(dev),
            ...
            {
                template<typename Op>
                std::size_t operator()(const std::tuple<Op, absl::string_view, TypeIndex>& x) const
                {
                    // The hash of an enum is just its value as a std::size_t.
                    std::size_t ret = static_cast<std::size_t>(std::get<0>(x));
            ...
            gtl::FlatMap<FuncTuple<VariantUnaryOp>, VariantUnaryOpFn, TupleHash> unary_op_fns;
            gtl::FlatMap<FuncTuple<VariantBinaryOp>, VariantBinaryOpFn, TupleHash> binary_op_fns;
        ...
        inline bool operator==(
            const UnaryVariantOpRegistry::FuncTuple<Op>& lhs,
        ...
        absl::Status
        UnaryOpVariant(OpKernelContext* ctx, VariantUnaryOp op, const Variant& v, Variant* v_out)
        {
        ...
        namespace variant_op_registry_fn_registration {
            ...
            {
                typedef std::function<absl::Status(OpKernelContext* ctx, const T& t, T* t_out)>
                    LocalVariantUnaryOpFn;

            public:
                UnaryVariantUnaryOpRegistration(
                    VariantUnaryOp op,
                    const std::string& device,
                    const TypeIndex& type_index,
            ...
            {
                typedef std::function<
                    absl::Status(OpKernelContext* ctx, const T& a, const T& b, T* out)>
                    LocalVariantBinaryOpFn;

            public:
                UnaryVariantBinaryOpRegistration(
                    VariantBinaryOp op,
                    const std::string& device,
                    const TypeIndex& type_index,
                        ...
                        [type_index_name, binary_op_fn](
                            ...
                            *out = T();
                            if (a.get<T>() == nullptr) {
                                return absl::InternalError(
                                    absl::StrCat(
                                        "VariantBinaryOpFn: Could not access object 'a', "
                                        "type_index: ",
                                        type_index_name
                                    )
                                );
                            ...
                            if (b.get<T>() == nullptr) {
                                return absl::InternalError(
                                    absl::StrCat(
                                        "VariantBinaryOpFn: Could not access object 'b', "
                                        "type_index: ",
                                        type_index_name
                                    )
                                );
...
#define REGISTER_UNARY_VARIANT_DECODE_FUNCTION_UNIQ(ctr, T, type_name)                             \
    static ::tensorflow::variant_op_registry_fn_registration::UnaryVariantDecodeRegistration<T>    \
...
#define REGISTER_UNARY_VARIANT_BINARY_OP_FUNCTION_UNIQ(                                            \
    ...
    // ==================================================================
    // Implementation: variant_op_registry.cc
    // ==================================================================
    ...
    namespace tensorflow {
        namespace {
            ...
            {
                explicit VariantDecodeDepthGuard(int* depth) :
                    depth_(depth)
                {
                    ++(*depth_);
                }

                ~VariantDecodeDepthGuard()
                {
                    --(*depth_);
                }
        ...
        std::unordered_set<std::string>* UnaryVariantOpRegistry::PersistentStringStorage()
        {
        ...
        UnaryVariantOpRegistry::VariantDecodeFn*
        UnaryVariantOpRegistry::GetDecodeFn(absl::string_view type_name)
        {
            auto found = decode_fns.find(type_name);
            if (found == decode_fns.end()) {
                return nullptr;
            }
            return &found->second;
        }
        ...
        bool DecodeUnaryVariant(Variant* variant)
        {
            ...
            if (variant->TypeName() != type_name) {
                LOG(ERROR) << "DecodeUnaryVariant: Variant type_name before decoding was: "
                           << type_name << " but after decoding was: " << variant->TypeName()
...
#define REGISTER_VARIANT_DECODE_TYPE(T) REGISTER_UNARY_VARIANT_DECODE_FUNCTION(T, TF_STR(T));
...
#undef REGISTER_VARIANT_DECODE_TYPE
        ...
        )
        {
            UnaryVariantOpRegistry::AsyncVariantDeviceCopyFn* device_copy_fn =
                UnaryVariantOpRegistry::Global()->GetDeviceCopyFn(direction, from.TypeId());
...
#undef REGISTER_VARIANT_DEVICE_COPY_TYPE
        ...
        namespace {
            template<typename T>
            absl::Status ZerosLikeVariantPrimitiveType(OpKernelContext* ctx, const T& t, T* t_out)
            {
                *t_out = T(0);
                return absl::OkStatus();
            }
...
#undef REGISTER_VARIANT_ZEROS_LIKE_TYPE
        ...
        namespace {
            template<typename T>
            absl::Status
            AddVariantPrimitiveType(OpKernelContext* ctx, const T& a, const T& b, T* out)
            {
                *out = a + b;
                return absl::OkStatus();
            }
