    ...
    namespace tensorflow {
        ...
        namespace data {
            ...
            public:
                ...
                virtual bool Contains(absl::string_view name, absl::string_view key) const = 0;

                // Reads an integer for the given key.
                virtual absl::Status ReadScalar(absl::string_view key, int64_t* val) const = 0;
                virtual absl::Status
                ReadScalar(absl::string_view name, absl::string_view key, int64_t* val) const = 0;

                // Reads a string for the given key.
                virtual absl::Status ReadScalar(absl::string_view key, tstring* val) const = 0;
                virtual absl::Status
                ReadScalar(absl::string_view name, absl::string_view key, tstring* val) const = 0;
            ...
            public:
                ...
                virtual absl::Status WriteScalar(absl::string_view key, const int64_t val) = 0;
                virtual absl::Status
                WriteScalar(absl::string_view name, absl::string_view key, const int64_t val) = 0;

                // Writes a string for the given key.
                virtual absl::Status WriteScalar(absl::string_view key, const tstring& val) = 0;
                virtual absl::Status
                WriteScalar(absl::string_view name, absl::string_view key, const tstring& val) = 0;

                // Writes a tensor for the given key.
                virtual absl::Status WriteTensor(absl::string_view key, const Tensor& val) = 0;
                virtual absl::Status
                WriteTensor(absl::string_view name, absl::string_view key, const Tensor& val) = 0;
            ...
            public:
                ...
                virtual absl::Status
                Save(SerializationContext* ctx, IteratorStateWriter* writer) = 0;
                virtual absl::Status Restore(IteratorContext* ctx, IteratorStateReader* reader) = 0;
            };
            ...
            {
            public:
                explicit GraphDefBuilderWrapper(GraphDefBuilder* b) :
                    b_(b)
                {
                }
            ...
            private:
                ...
                bool HasAttr(const std::string& op_type_name, const std::string& attr_name) const;
            ...
            public:
                ...
                static MemoryCheckpoint CreateRootCheckpoint(std::shared_ptr<IdRegistry> registry)
                {
                ...
                absl::Status
                WriteScalar(absl::string_view name, absl::string_view key, int64_t val) override
                {
            ...
            public:
                    ...
                    switch (params_.external_state_policy) {
                        ...
                        default:
                            return absl::InvalidArgumentError(
                                absl::StrCat(
                                    "Unexpected value of external policy: ",
                ...
                struct Params
                {
                    ...
                    std::vector<std::pair<std::string, Tensor>>* input_list = nullptr; // Not owned.

                    // Indicates what to do if the dataset depends on external state.
                    ExternalStatePolicy external_state_policy = ExternalStatePolicy::POLICY_WARN;
                ...
                explicit SerializationContext(Params params) :
                    params_(params)
                {
                }
                ...
                {
                    return params_.symbolic_checkpoint;
                }
            ...
            public:
                ...
                const std::shared_ptr<model::Model>& model() const
                {
                ...
                bool warm_start()
                {
                    return params_.warm_start;
                }

                RunMode run_mode()
                {
                    return params_.run_mode;
                }

                IndexMapperFn index_mapper() const
                {
                ...
                std::string data_service_address()
                {
                    return params_.data_service_address;
                }
                ...
                CreateThreadPool(const std::string& name, int num_threads)
                {
                ...
                StartThread(const std::string& name, std::function<void()> fn)
                {
            ...
            public:
                ...
                virtual IndexMapperFn GetIndexMapper(IndexMapperFn parent_index_mapper) const
                {
                ...
                virtual bool SymbolicCheckpointCompatible() const
                {
                    return false;
                }
                ...
                absl::Status Save(SerializationContext* ctx, IteratorStateWriter* writer) override
                {
                    int64_t start_us = EnvTime::NowMicros();
                    TF_RETURN_IF_ERROR(SaveInternal(ctx, writer));
                    VLOG(1) << "Saved " << prefix() << " in " << (EnvTime::NowMicros() - start_us)
                            << "us";
                    return absl::OkStatus();
                }

                // Restores the state of this iterator.
                absl::Status Restore(IteratorContext* ctx, IteratorStateReader* reader) override
                {
                    int64_t start_us = EnvTime::NowMicros();
                ...
                {
                    if (node_) {
                        return node_->TotalBufferedBytes();
                    }
                    return 0;
                }
            ...
            protected:
                ...
                virtual std::shared_ptr<model::Node>
                CreateNode(IteratorContext* ctx, model::Node::Args args) const = 0;

                // This is needed so that sub-classes of IteratorBase can call
                // `SaveInternal` on their input iterators.
                absl::Status SaveInput(
                    SerializationContext* ctx,
                ...
                virtual absl::Status
                SaveInternal(SerializationContext* ctx, IteratorStateWriter* writer) = 0;
                ...
                // implementations have an override.
                virtual absl::Status
                RestoreInternal(IteratorContext* ctx, IteratorStateReader* reader) = 0;
                ...
                // iterator.
                std::shared_ptr<model::Node> model_node() const
                {
                    return node_;
                }
            ...
            public:
                ...
                explicit DatasetContext(Params params) :
                    params_(std::move(params))
                {
                }
            ...
            absl::Status
            GetDatasetFromVariantTensor(const Tensor& tensor, DatasetBase** out_dataset);
            ...
            public:
                ...
                virtual int64_t CardinalityInternal(
                    CardinalityOptions options
                ) const TF_EXCLUSIVE_LOCKS_REQUIRED(cardinality_mu_)
                {
                    return kUnknownCardinality;
                }
                ...
                // dataset.
                virtual absl::Status InputDatasets(std::vector<const DatasetBase*>* inputs) const;
                ...
                virtual absl::Status
                Get(OpKernelContext* ctx, int64_t index, std::vector<Tensor>* out_tensors) const;
                ...
                virtual absl::Status
                Get(AnyContext ctx, int64_t index, std::vector<Tensor>* out_tensors) const;
            ...
            protected:
                ...
                MakeIteratorInternal(const std::string& prefix) const = 0;
            ...
            private:
                ...
                mutable int64_t cardinality_ TF_GUARDED_BY(cardinality_mu_) = kUnknownCardinality;
            };
            ...
            public:
                ...
                {
                    VLOG(2) << "Attempting to save checkpoints on iterator (prefix: " << prefix()
                            << ") from " << dataset()->DebugString();
            ...
            protected:
                absl::Status Restore(IteratorContext* ctx, IteratorStateReader* reader) final
                {
                    VLOG(2) << "Attempting to restore checkpoints on iterator (prefix: " << prefix()
                            << ") from " << dataset()->DebugString();
                ...
                CreateNode(IteratorContext* ctx, model::Node::Args args) const override
                {
                ...
                void RecordBufferDequeue(IteratorContext* ctx, const std::vector<Tensor>& element)
                {
                ...
                void RecordBufferEnqueue(IteratorContext* ctx, const std::vector<Tensor>& element)
                {
                    if (collect_resource_usage(ctx)) {
                        node_->record_buffer_event(GetAllocatedBytes(element), 1);
                    }
            ...
            {
            public:
                explicit DatasetOpKernel(OpKernelConstruction* ctx) :
                    OpKernel(ctx)
                {
            ...
            protected:
                ...
                virtual void
                MakeDataset(OpKernelContext* ctx, DatasetBase* input, DatasetBase** output) = 0;
            };
        ...
        namespace data {
            namespace {
                ...
                public:
                    ...
                    explicit DatasetVariantWrapper(DatasetBase* dataset) :
                        dataset_(dataset)
                    {
                    }
                ...
                {
                public:
                    explicit WrapDatasetVariantOp(OpKernelConstruction* ctx) :
                        OpKernel(ctx)
                    {
                    }
                ...
                {
                public:
                    explicit UnwrapDatasetVariantOp(OpKernelConstruction* ctx) :
                        OpKernel(ctx)
                    {
                    }
                ...
                static absl::Status WrappedDatasetVariantDeviceCopy(
                    const WrappedDatasetVariantWrapper& from,
                    WrappedDatasetVariantWrapper* to,
        ...
        WrappedDatasetVariantWrapper, DIRECTION, WrappedDatasetVariantDeviceCopy                   \
            ...
            )
            {
                ...
                if (opts->HaveError()) {
                    return absl::InternalError(
                        absl::StrCat(
                            "AddDataset: Failed to build Options with error ",
                            opts->StatusToString()
                        )
                    );
                }
                ...
                );
                {
                    ...
                    for (int i = 0; i < total_size; i++) {
                        if (inputs_iter != inputs.end() && inputs_iter->first == i) {
                            ...
                            list_inputs_iter != list_inputs.end() && list_inputs_iter->first == i
                        ) {
            ...
            void GraphDefBuilderWrapper::AddPlaceholderInternal(const Tensor& val, Node** output)
            {
                *output = ops::SourceOp(
                    "Placeholder",
            ...
            int32_t GetRunnerThreadpoolSizeFromOpKernelContext(OpKernelContext* ctx)
            {
                thread::ThreadPool* thread_pool = ctx->device()->tensorflow_device_thread_pool();
                if (thread_pool) {
            ...
            int64_t
            MemoryCheckpoint::IdRegistry::Add(const std::string& prefix, const std::string& key)
            {
                mutex_lock l(mu_);
            ...
            std::vector<int64_t>
            MemoryCheckpoint::IdRegistry::GetMatchingIds(const std::string& prefix_to_match)
            {
                mutex_lock l(mu_);
            ...
            std::pair<std::string, std::string> MemoryCheckpoint::IdRegistry::Get(int64_t id)
            {
                mutex_lock l(mu_);
            ...
            absl::Status
            IteratorBase::InitializeBase(IteratorContext* ctx, const IteratorBase* parent)
            {
                parent_ = parent;
            ...
            )
            {
                ...
                const CompressedElement* compressed_element = variant.get<CompressedElement>();
...
#define WARN_PROTO_FIELD_CONFLICT(reflection, field, field_type, src, dst)                         \
    {                                                                                              \
        ...
        if (source_value != destination_value) {                                                   \
            LOG(WARNING) << "Changing the value of option field " << field->name() << " from "     \
                         << destination_value << " to " << source_value;                           \
        }                                                                                          \
    }
...
#define WARN_PROTO_ENUM_FIELD_CONFLICT(reflection, field, src, dst)                                \
    {                                                                                              \
        ...
        if (source_value != destination_value) {                                                   \
            LOG(WARNING) << "Changing the value of option enum field " << field->name()            \
                         << " from " << destination_value->full_name() << " to "                   \
                ...
                void WarnProtoConflicts(const protobuf::Message& src, protobuf::Message* dst)
                {
                    ...
                    for (auto field: in_both) {
                        ...
                        if (field->type() == protobuf::FieldDescriptor::TYPE_MESSAGE) {
                            WarnProtoConflicts(
                                reflection->GetMessage(src, field),
...
#undef WARN_PROTO_FIELD_CONFLICT
            ...
            absl::Status DatasetBase::CheckRandomAccessCompatible(const int64_t index) const
            {
                CardinalityOptions options;
            ...
            ) const
            {
                return absl::UnimplementedError(
                    absl::StrCat("Random access is not implemented for dataset ", DebugString())
                );
            }

            absl::Status
            DatasetBase::Get(AnyContext ctx, int64_t index, std::vector<Tensor>* out_tensors) const
            {
                return absl::UnimplementedError(
                    absl::StrCat("Random access is not implemented for dataset ", DebugString())
                );
            ...
            absl::Status DatasetBase::InputDatasets(std::vector<const DatasetBase*>* inputs) const
            {
                return absl::UnimplementedError(
                    absl::StrCat(
                        "Cannot compute input sources for dataset of type ", type_string(),
            ...
            )
            {
                ...
                NodeBuilder node_builder(opts.GetNameForOp(op_name), op_name, opts.op_registry());
            ...
            std::string DatasetOpKernel::TraceString(const OpKernelContext& ctx, bool verbose) const
            {
            ...
            bool DatasetOpKernel::IsDatasetOp(const OpDef& op_def)
            {
                ...
                if (op_def.output_arg(0).type() != DT_VARIANT) {
                    return false;
                }
                ...
                if (op_name == "DatasetFromGraph") {
                    return true;
                }
                if (absl::EndsWith(op_name, "Dataset")) {
                    return true;
                }
                ...
                if (index < kPrefixLength - 1 || index == op_name.length() - 1) {
                    return false;
                }
                return op_name.substr(index - kPrefixLength + 1, kPrefixLength) == kDatasetPrefix;
            }

            void UnaryDatasetOpKernel::MakeDataset(OpKernelContext* ctx, DatasetBase** output)
            {
                DatasetBase* input;
            ...
            void BinaryDatasetOpKernel::MakeDataset(OpKernelContext* ctx, DatasetBase** output)
            {
                DatasetBase* input;
                OP_REQUIRES_OK(ctx, GetDatasetFromVariantTensor(ctx->input(0), &input));
                DatasetBase* another_input;
                OP_REQUIRES_OK(ctx, GetDatasetFromVariantTensor(ctx->input(1), &another_input));
