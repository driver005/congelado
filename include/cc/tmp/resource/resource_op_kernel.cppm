...
export module cc_tmp:resource_resource_op_kernel;
...
export {
    ...
    namespace tensorflow {
        ...
        private:
            core::WeakPtr<T> weak_resource_ TF_GUARDED_BY(mu_) = core::WeakPtr<T>(nullptr);

            // Must return a T descendant allocated with new that ResourceOpKernel will
            // take ownership of.
            virtual absl::Status CreateResource(T** resource) TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) = 0;
            ...
            virtual absl::Status VerifyResource(T* resource)
            {
