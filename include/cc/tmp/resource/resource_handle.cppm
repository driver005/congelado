...
export {
    ...
    namespace tensorflow {
        ...
        public:
            ...
            static absl::Status
            BuildResourceHandle(const ResourceHandleProto& proto, ResourceHandle* out);
            ...
            void set_hash_code(uint64_t hash_code)
            {
    ...
    namespace tensorflow {
        ...
        namespace {
            std::string
            DtypeAndShapesToString(const std::vector<DtypeAndPartialTensorShape>& dtype_and_shapes)
            {
        ...
        absl::Status
        ResourceHandle::BuildResourceHandle(const ResourceHandleProto& proto, ResourceHandle* out)
        {
            if (out == nullptr) {
        ...
        absl::Status ResourceHandle::FromProto(const ResourceHandleProto& proto)
        {
            ...
            for (const auto& dtype_and_shape: proto.dtypes_and_shapes()) {
                ...
                absl::Status s =
                    PartialTensorShape::BuildPartialTensorShape(dtype_and_shape.shape(), &shape);
                if (!s.ok()) {
                    return s;
                }
        ...
        )
        {
            ...
            if (!d->ReadSizes(&sizes)) {
                return false;
            }

            ResourceHandleProto proto;
            for (int i = 0; i < n; ++i) {
                if (!proto.ParseFromString(absl::string_view(d->Data(sizes[i]), sizes[i]))) {
                    return false;
                }
