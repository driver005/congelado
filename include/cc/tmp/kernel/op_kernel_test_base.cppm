...
export module cc_tmp:kernel_op_kernel_test_base;
...
export {
    ...
    namespace tensorflow {
        ...
        protected:
            ...
            NodeDef CreateNodeDef(const std::string& op_type, const std::vector<std::string>& attrs)
            {
                NodeDef node_def;
                ...
                for (const std::string& attr_desc: attrs) {
                    ...
                    node_def.mutable_attr()->insert(AttrValueMap::value_type(parts[0], attr_value));
                }
                return node_def;
            }

            std::unique_ptr<OpKernel> ExpectSuccess(
            ...
            )
            {
                ...
                EXPECT_TRUE(found)
                    << "Missing " << device_type << " from " << devices.size() << " devices.";
        ...
        {
        public:
            explicit BaseKernel(OpKernelConstruction* context) :
                OpKernel(context)
            {
            }
