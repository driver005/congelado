...
class Connector : public shared::HandlerBase {
  public:
    Connector(core::contract::ContractGroup<MaxCapacity>& group,
    ...
    void create_table(std::move_only_function<void(bool)> callback) noexcept {
        enqueue([this, callback = std::move(callback)]() mutable {
            active_database().query(build_create_sql(), [callback = std::move(callback)](std::string_view result) mutable {
            ...
            active_cache().get(cache_key_string,
                [this,
                 ...
                 callback = std::move(callback)](std::string_view cached_value) mutable {
                    ...
                    active_database().query(build_select_sql(owned_key),
                        [this,
                         cache_key_string,
                         callback = std::move(callback)](std::string_view db_result) mutable {
                            if (db_result.empty()) { callback(std::nullopt); return; }
                            auto decoded = rfl::json::read<T>(std::string{db_result});
                            if (!decoded) { callback(std::nullopt); return; }
                            auto encoded = rfl::json::write(*decoded);
    ...
    void find_many(std::span<const std::string_view> keys,
        ...
        enqueue([this,
                 ...
                 callback   = std::move(callback)]() mutable {
            active_database().query(build_select_many_sql(owned_keys),
                [callback = std::move(callback)](std::string_view db_result) mutable {
                    if (db_result.empty()) { callback({}); return; }
                    auto decoded = rfl::json::read<std::vector<T>>(std::string{db_result});
    ...
    void find_all(std::move_only_function<void(std::vector<T>)> callback) noexcept {
        enqueue([this, callback = std::move(callback)]() mutable {
            active_database().query(build_select_all_sql(),
                [callback = std::move(callback)](std::string_view db_result) mutable {
                    if (db_result.empty()) { callback({}); return; }
                    auto decoded = rfl::json::read<std::vector<T>>(std::string{db_result});
                    callback(decoded ? std::move(*decoded) : std::vector<T>{});
    ...
    void insert_many(std::span<const T> values,
        ...
        enqueue([this,
                 ...
                 callback     = std::move(callback)]() mutable {
            active_database().query(build_insert_many_sql(owned_values),
    ...
    void remove(std::string_view key,
        ...
        enqueue([this,
            ...
            active_database().remove(build_delete_sql(owned_key),
                [callback = std::move(callback)](std::string_view result) mutable {
                    callback(!result.empty());
                });
        });
    ...
    void remove_many(std::span<const std::string_view> keys,
        ...
        enqueue([this,
            ...
            active_database().remove(build_delete_many_sql(owned_keys),
                [callback = std::move(callback)](std::string_view result) mutable {
                    callback(!result.empty());
                });
        });
  ...
  private:
    ...
    void write_through(const T& value,
        ...
        active_database().query(sql, [callback = std::move(callback)](std::string_view result) mutable {
            callback(!result.empty());
        });
    }
    ...
    static std::string build_select_sql(std::string_view key) {
        return std::format(
            "SELECT row_to_json(row) FROM (SELECT * FROM {} WHERE id = '{}') row",
    ...
    static std::string build_insert_many_sql(const std::vector<T>& values) {
        ...
        for (std::size_t index = 0; index < values.size(); ++index) {
            if (index > 0) rows += ',';
            rows += std::format("'{}'", rfl::json::write(values[index]));
        }
        return std::format(
    ...
    static std::string build_update_sql(const T& value) {
        return std::format(
            "UPDATE {} SET data = row FROM json_populate_record(NULL::{}, '{}') row WHERE {}.id = row.id",
            serde::Serializable<T>::table_name(),
            serde::Serializable<T>::table_name(),
            rfl::json::write(value),
            serde::Serializable<T>::table_name());
    ...
    static std::string build_delete_sql(std::string_view key) {
        return std::format("DELETE FROM {} WHERE id = '{}'",
                           serde::Serializable<T>::table_name(), key);
    }
    ...
    static std::string build_delete_many_sql(const std::vector<std::string>& keys) {
        ...
        return std::format("DELETE FROM {} WHERE id IN ({})",
                           serde::Serializable<T>::table_name(), list);
    }

    static std::string pk_value(const T& value) {
        return rfl::json::write(value); // placeholder — FieldOptions.db.primary_key drives this
    }
