...
extern "C"
{
    ...
    typedef TP_Otel_Counter* (*TP_Otel_Meter_CreateCounterFn)(
        void* user_data,
        const TF_TString* name,
        const TF_TString* description,
        const TF_TString* unit
    );
    typedef TP_Otel_Histogram* (*TP_Otel_Meter_CreateHistogramFn)(
        void* user_data,
        const TF_TString* name,
        const TF_TString* description,
        const TF_TString* unit
...
#define TP_OTEL_METER_STRUCT_SIZE TF_OFFSET_OF_END(TP_Otel_Meter, create_histogram_cb)
    ...
    static inline void TP_OtelMeter_SetCreateCounterCallback(
        TP_Otel_Meter* builder, TP_Otel_Meter_CreateCounterFn create_counter_cb
    )
    {
        builder->create_counter_cb = create_counter_cb;
    }

    static inline void TP_OtelMeter_SetCreateHistogramCallback(
        TP_Otel_Meter* builder, TP_Otel_Meter_CreateHistogramFn create_histogram_cb
    )
    {
