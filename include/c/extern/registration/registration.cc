...
//  * the vtable entry points match the header's signatures exactly (register_op takes
//    const char* name, get/unregister take const TF_String* name);
...
std::string key_from_cstr(const char* type, const char* name) noexcept
{
    try {
...
std::string key_from_tstring(const char* type, const TF_String* name) noexcept
{
    try {
        std::string t{type ? type : ""};
...
void registration_register(void* plugin_context, const char* type, const char* name, void* value) noexcept
{
    try {
        std::lock_guard<std::mutex> lock{registry_mutex()};
        storage()[key_from_cstr(type, name)] = value;
    } catch (...) {
        // no exception may escape a C ABI function
    }
}

void* registration_get(void* plugin_context, const char* type, const TF_String* name) noexcept
{
    try {
        std::lock_guard<std::mutex> lock{registry_mutex()};
...
void registration_unregister(void* plugin_context, const char* type, const TF_String* name) noexcept
{
    try {
        std::lock_guard<std::mutex> lock{registry_mutex()};
