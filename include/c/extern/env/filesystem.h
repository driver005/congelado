...
#ifndef CONGELADO_C_ENV_FILESYSTEM_H_
#define CONGELADO_C_ENV_FILESYSTEM_H_
...
extern "C"
{
    ...
    // TF_WritableFileHandle and TF_StringStream are opaque: their layout is
    // private to the library. Handles are obtained from TF_NewWritableFile /
    // TF_GetChildren and released via TF_CloseWritableFile / TF_StringStreamDone.
    // No public alloc/dealloc inlines are possible here.
    ...
    //    implemented
    TF_CAPI_EXPORT extern void TF_DeleteRecursively(
        const char* dirname,
    ...
    TF_FileStat(const char* filename, TF_FileStatistics* stats, TF_Status* status);
    ...
    TF_CAPI_EXPORT extern void
    TF_NewWritableFile(const char* filename, TF_WritableFileHandle** handle, TF_Status* status);
    ...
    TF_CAPI_EXPORT extern void
    TF_CloseWritableFile(TF_WritableFileHandle* handle, TF_Status* status);
    ...
    TF_CAPI_EXPORT extern void
    TF_SyncWritableFile(TF_WritableFileHandle* handle, TF_Status* status);
    ...
    TF_CAPI_EXPORT extern void
    TF_FlushWritableFile(TF_WritableFileHandle* handle, TF_Status* status);

    // Appends the given bytes to the file. Any failure to do so is indicated in
    // status.
    TF_CAPI_EXPORT extern void TF_AppendWritableFile(
        TF_WritableFileHandle* handle, const char* data, size_t length, TF_Status* status
    );
    ...
    TF_CAPI_EXPORT extern bool TF_StringStreamNext(TF_StringStream* list, const char** result);
    ...
    TF_CAPI_EXPORT extern TF_StringStream* TF_GetChildren(const char* filename, TF_Status* status);
...
#endif // CONGELADO_C_ENV_FILESYSTEM_H_
