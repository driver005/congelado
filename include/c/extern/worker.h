...
typedef void (*TF_Worker_CompletionFn)(const TF_TString* result_json,
                                       void* user_data);

typedef const TF_TString* (*TP_Worker_GetTaskTypeFn)(void* user_data);
typedef TF_Status* (*TP_Worker_ExecuteFn)(void* user_data, const TF_TString* input_json,
                                          TF_TString** out_result_json);
typedef void (*TP_Worker_ExecuteAsyncFn)(void* user_data, const TF_TString* input_json,
...
static inline void TP_Worker_SetGetTaskTypeCallback(TP_Worker* builder, TP_Worker_GetTaskTypeFn get_task_type_cb) {
  builder->get_task_type_cb = get_task_type_cb;
}

static inline void TP_Worker_SetExecuteCallback(TP_Worker* builder, TP_Worker_ExecuteFn execute_cb) {
  builder->execute_cb = execute_cb;
}

static inline void TP_Worker_SetExecuteAsyncCallback(TP_Worker* builder, TP_Worker_ExecuteAsyncFn execute_async_cb) {
  builder->execute_async_cb = execute_async_cb;
}

static inline void TP_Worker_SetOnErrorCallback(TP_Worker* builder, TP_Worker_OnErrorFn on_error_cb) {
  builder->on_error_cb = on_error_cb;
}

static inline void TP_Worker_SetOnReleasedCallback(TP_Worker* builder, TP_Worker_OnReleasedFn on_released_cb) {
