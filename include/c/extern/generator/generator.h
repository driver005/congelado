...
extern "C"
{
    ...
    typedef struct TF_GeneratorOps
    {
        ...
        void (*get_name)(TF_Generator* generator, TF_String* out);
        ...
        TF_Generator_Module* (*get_module)(TF_Generator* generator, const TF_String* name);

        TF_Tensor* (*list_modules)(TF_Generator* generator, TF_Status* status);
...
#define TF_GENERATOR_STRUCT_SIZE TF_OFFSET_OF_END(TF_GeneratorOps, set_name)
    ...
    TF_CAPI_EXPORT void
    create_generator(TF_GeneratorOps** ops, void** plugin_context, TF_Status* status);
    ...
    static inline void init_generator(TF_GeneratorOps** ops, TF_Generator* generator, TF_Status* status)
    {
        create_generator(ops, &generator->plugin_data, status);

        TF_Generator_ModuleOps* module_ops = NULL;
        create_generator_module(&module_ops, &generator->plugin_data, status);
        generator->module_ops = module_ops;

        TF_Generator_FunctionOps* function_ops = NULL;
        create_generator_function(&function_ops, &generator->plugin_data, status);
        generator->function_ops = function_ops;

        TF_Generator_ParameterOps* parameter_ops = NULL;
        create_generator_parameter(&parameter_ops, &generator->plugin_data, status);
        generator->parameter_ops = parameter_ops;

        TF_Generator_AttributeOps* attribute_ops = NULL;
        create_generator_attribute(&attribute_ops, &generator->plugin_data, status);
        generator->attribute_ops = attribute_ops;

        TF_Generator_DefinitionOps* definition_ops = NULL;
        create_generator_definition(&definition_ops, &generator->plugin_data, status);
        generator->definition_ops = definition_ops;

        TF_Generator_BlockOps* block_ops = NULL;
        create_generator_block(&block_ops, &generator->plugin_data, status);
        generator->block_ops = block_ops;

        TF_Generator_NodeOps* node_ops = NULL;
        create_generator_node(&node_ops, &generator->plugin_data, status);
        generator->node_ops = node_ops;

        TF_TypeInfoOps* typeinfo_ops = NULL;
        create_typeinfo(&typeinfo_ops, &generator->plugin_data, status);
