...
typedef void (*TP_Orchestrator_StartWorkflowFn)(void* user_data,
    const TF_TString* def_name, const TF_TString* variables_json,
    TF_Worker_CompletionFn completion, void* cb_user_data);
typedef void (*TP_Orchestrator_PauseFn)(void* user_data,
    const TF_TString* exec_id, TF_Worker_CompletionFn completion,
    void* cb_user_data);
typedef void (*TP_Orchestrator_ResumeFn)(void* user_data,
    const TF_TString* exec_id, TF_Worker_CompletionFn completion,
    void* cb_user_data);
typedef void (*TP_Orchestrator_TerminateFn)(void* user_data,
    const TF_TString* exec_id, TF_Worker_CompletionFn completion,
    void* cb_user_data);
typedef void (*TP_Orchestrator_CompleteTaskFn)(void* user_data,
    const TF_TString* exec_id, const TF_TString* node_ref, TF_Bool success,
    const TF_TString* output_json, TF_Worker_CompletionFn completion,
    void* cb_user_data);
...
#define TP_ORCHESTRATOR_STRUCT_SIZE \
  TF_OFFSET_OF_END(TP_Orchestrator, complete_task_cb)
} TP_Orchestrator;
...
static inline void TP_Orchestrator_SetBackendName(TP_Orchestrator* builder, TF_TString backend_name) {
  builder->backend_name = backend_name;
}

static inline void TP_Orchestrator_SetRequiredCallback(TP_Orchestrator* builder, TP_Orchestrator_RequiredFn required_cb) {
  builder->required_cb = required_cb;
}

static inline void TP_Orchestrator_SetStartWorkflowCallback(TP_Orchestrator* builder, TP_Orchestrator_StartWorkflowFn start_workflow_cb) {
  builder->start_workflow_cb = start_workflow_cb;
}

static inline void TP_Orchestrator_SetPauseCallback(TP_Orchestrator* builder, TP_Orchestrator_PauseFn pause_cb) {
  builder->pause_cb = pause_cb;
}

static inline void TP_Orchestrator_SetResumeCallback(TP_Orchestrator* builder, TP_Orchestrator_ResumeFn resume_cb) {
  builder->resume_cb = resume_cb;
}

static inline void TP_Orchestrator_SetTerminateCallback(TP_Orchestrator* builder, TP_Orchestrator_TerminateFn terminate_cb) {
  builder->terminate_cb = terminate_cb;
}

static inline void TP_Orchestrator_SetCompleteTaskCallback(TP_Orchestrator* builder, TP_Orchestrator_CompleteTaskFn complete_task_cb) {
...
void TF_InitOrchestrator(TF_OrchestratorRegistrationParams* params,
                          TF_Status* status);
