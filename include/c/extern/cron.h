...
extern "C"
{
    ...
    typedef TF_Bool (*TP_Cron_NextAfterFn)(
        void* user_data, const TF_TString* expression, int64_t base_time_ms, int64_t* out_time_ms
    );
    typedef void (*TP_Cron_UpsertJobFn)(
        void* user_data, const TF_TString* name, const TF_TString* expression
    );
...
#define CR_PATCH 1
    ...
    static inline void
    TP_Cron_SetNextAfterCallback(TP_Cron* builder, TP_Cron_NextAfterFn next_after_cb)
    {
        builder->next_after_cb = next_after_cb;
    }

    static inline void
    TP_Cron_SetUpsertJobCallback(TP_Cron* builder, TP_Cron_UpsertJobFn upsert_job_cb)
    {
        builder->upsert_job_cb = upsert_job_cb;
    }

    static inline void
    TP_Cron_SetRemoveJobCallback(TP_Cron* builder, TP_Cron_RemoveJobFn remove_job_cb)
    {
