...
typedef const void* (*TP_ReadOnlyMemoryRegion_Data)(
    const TF_ReadOnlyMemoryRegion* region);

/// Returns the length of the memory region in bytes.
typedef uint64_t (*TP_ReadOnlyMemoryRegion_Length)(
    const TF_ReadOnlyMemoryRegion* region);
...
static inline TF_ReadOnlyMemoryRegion* TF_ReadOnlyMemoryRegionNew(void) {
  TF_ReadOnlyMemoryRegion* ptr = (TF_ReadOnlyMemoryRegion*)malloc(sizeof(TF_ReadOnlyMemoryRegion));
...
static inline void TF_ReadOnlyMemoryRegion_SetPluginMemoryRegion(TF_ReadOnlyMemoryRegion* builder, void* plugin_memory_region) {
...
static inline TF_ReadOnlyMemoryRegionOps* TF_ReadOnlyMemoryRegionOpsNew(void) {
  TF_ReadOnlyMemoryRegionOps* ptr = (TF_ReadOnlyMemoryRegionOps*)malloc(sizeof(TF_ReadOnlyMemoryRegionOps));
  if (!ptr) return nullptr;
  ptr->cleanup = nullptr;
  ptr->data = nullptr;
  ptr->length = nullptr;
...
static inline void TF_ReadOnlyMemoryRegionOps_SetCleanup(TF_ReadOnlyMemoryRegionOps* builder, TP_ReadOnlyMemoryRegion_Cleanup cleanup) {
  builder->cleanup = cleanup;
}

static inline void TF_ReadOnlyMemoryRegionOps_SetData(TF_ReadOnlyMemoryRegionOps* builder, TP_ReadOnlyMemoryRegion_Data data) {
  builder->data = data;
}

static inline void TF_ReadOnlyMemoryRegionOps_SetLength(TF_ReadOnlyMemoryRegionOps* builder, TP_ReadOnlyMemoryRegion_Length length) {
