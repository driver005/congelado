...
extern "C"
{
    ...
    typedef struct TF_WritableFileOps
    {
        ...
        void (*sync)(TF_WritableFile* file, TF_Status* status);
        void (*close)(TF_WritableFile* file, TF_Status* status);
    } TF_WritableFileOps;

#define TF_WRITABLE_FILE_STRUCT_SIZE TF_OFFSET_OF_END(TF_WritableFileOps, close)

    TF_CAPI_EXPORT void
    create_writable_file(TF_WritableFileOps** ops, void** plugin_context, TF_Status* status);
