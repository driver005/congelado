...
extern "C"
{
    ...
    typedef void (*TP_Filesystem_NewRandomAccessFile)(
        const TF_Filesystem* filesystem,
        const char* path,
        TF_RandomAccessFile* file,
        TF_Status* status
    );
    typedef void (*TP_Filesystem_NewWritableFile)(
        const TF_Filesystem* filesystem, const char* path, TF_WritableFile* file, TF_Status* status
    );
    typedef void (*TP_Filesystem_NewAppendableFile)(
        const TF_Filesystem* filesystem, const char* path, TF_WritableFile* file, TF_Status* status
    );
    ...
    typedef void (*TP_Filesystem_CreateDir)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef void (*TP_Filesystem_RecursivelyCreateDir)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef void (*TP_Filesystem_DeleteFile)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef void (*TP_Filesystem_DeleteDir)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef void (*TP_Filesystem_DeleteRecursively)(
        const TF_Filesystem* filesystem,
        const char* path,
    ...
    typedef void (*TP_Filesystem_RenameFile)(
        const TF_Filesystem* filesystem, const char* src, const char* dst, TF_Status* status
    );
    typedef void (*TP_Filesystem_CopyFile)(
        const TF_Filesystem* filesystem, const char* src, const char* dst, TF_Status* status
    );
    typedef void (*TP_Filesystem_PathExists)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef bool (*TP_Filesystem_PathsExist)(
        const TF_Filesystem* filesystem, char** paths, int num_files, TF_Status** statuses
    );
    typedef void (*TP_Filesystem_Stat)(
        const TF_Filesystem* filesystem,
        const char* path,
        TF_FileStatistics* stats,
        TF_Status* status
    );
    typedef bool (*TP_Filesystem_IsDirectory)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef int64_t (*TP_Filesystem_GetFileSize)(
        const TF_Filesystem* filesystem, const char* path, TF_Status* status
    );
    typedef char* (*TP_Filesystem_TranslateName)(const TF_Filesystem* filesystem, const char* uri);
    typedef int (*TP_Filesystem_GetChildren)(
        const TF_Filesystem* filesystem, const char* path, char*** entries, TF_Status* status
    );
    typedef int (*TP_Filesystem_GetMatchingPaths)(
        const TF_Filesystem* filesystem, const char* glob, char*** entries, TF_Status* status
    );
    typedef void (*TP_Filesystem_FlushCaches)(const TF_Filesystem* filesystem);
    typedef void (*TP_Filesystem_GetConfiguration)(
        const TF_Filesystem* filesystem,
        TF_Filesystem_Option** options,
        int* num_options,
        TF_Status* status
    );
    typedef void (*TP_Filesystem_SetConfiguration)(
        const TF_Filesystem* filesystem,
    ...
    typedef void (*TP_Filesystem_SetConfigurationOption)(
        const TF_Filesystem* filesystem, const TF_Filesystem_Option* option, TF_Status* status
    );
    typedef void (*TP_Filesystem_GetConfigurationKeys)(
        const TF_Filesystem* filesystem, char** keys, int* num_keys, TF_Status* status
    );
    ...
    static inline void
    TF_Filesystem_SetPluginFilesystem(TF_Filesystem* builder, void* plugin_filesystem)
    {
    ...
    static inline void
    TF_FilesystemOps_SetCleanup(TF_FilesystemOps* builder, TP_Filesystem_Cleanup cleanup)
    {
        builder->cleanup = cleanup;
    }

    static inline void TF_FilesystemOps_SetNewRandomAccessFile(
        TF_FilesystemOps* builder, TP_Filesystem_NewRandomAccessFile new_random_access_file
    )
    {
        builder->new_random_access_file = new_random_access_file;
    }

    static inline void TF_FilesystemOps_SetNewWritableFile(
        TF_FilesystemOps* builder, TP_Filesystem_NewWritableFile new_writable_file
    )
    {
        builder->new_writable_file = new_writable_file;
    }

    static inline void TF_FilesystemOps_SetNewAppendableFile(
        TF_FilesystemOps* builder, TP_Filesystem_NewAppendableFile new_appendable_file
    )
    {
        builder->new_appendable_file = new_appendable_file;
    }

    static inline void TF_FilesystemOps_SetNewReadOnlyMemoryRegionFromFile(
        TF_FilesystemOps* builder,
        TP_Filesystem_NewReadOnlyMemoryRegionFromFile new_read_only_memory_region_from_file
    )
    {
    ...
    static inline void
    TF_FilesystemOps_SetCreateDir(TF_FilesystemOps* builder, TP_Filesystem_CreateDir create_dir)
    {
        builder->create_dir = create_dir;
    }

    static inline void TF_FilesystemOps_SetRecursivelyCreateDir(
        TF_FilesystemOps* builder, TP_Filesystem_RecursivelyCreateDir recursively_create_dir
    )
    {
    ...
    static inline void
    TF_FilesystemOps_SetDeleteFile(TF_FilesystemOps* builder, TP_Filesystem_DeleteFile delete_file)
    {
        builder->delete_file = delete_file;
    }

    static inline void
    TF_FilesystemOps_SetDeleteDir(TF_FilesystemOps* builder, TP_Filesystem_DeleteDir delete_dir)
    {
        builder->delete_dir = delete_dir;
    }

    static inline void TF_FilesystemOps_SetDeleteRecursively(
        TF_FilesystemOps* builder, TP_Filesystem_DeleteRecursively delete_recursively
    )
    {
    ...
    static inline void
    TF_FilesystemOps_SetRenameFile(TF_FilesystemOps* builder, TP_Filesystem_RenameFile rename_file)
    {
        builder->rename_file = rename_file;
    }

    static inline void
    TF_FilesystemOps_SetCopyFile(TF_FilesystemOps* builder, TP_Filesystem_CopyFile copy_file)
    {
        builder->copy_file = copy_file;
    }

    static inline void
    TF_FilesystemOps_SetPathExists(TF_FilesystemOps* builder, TP_Filesystem_PathExists path_exists)
    {
        builder->path_exists = path_exists;
    }

    static inline void
    TF_FilesystemOps_SetPathsExist(TF_FilesystemOps* builder, TP_Filesystem_PathsExist paths_exist)
    {
    ...
    static inline void TF_FilesystemOps_SetIsDirectory(
        TF_FilesystemOps* builder, TP_Filesystem_IsDirectory is_directory
    )
    {
        builder->is_directory = is_directory;
    }

    static inline void TF_FilesystemOps_SetGetFileSize(
        TF_FilesystemOps* builder, TP_Filesystem_GetFileSize get_file_size
    )
    {
        builder->get_file_size = get_file_size;
    }

    static inline void TF_FilesystemOps_SetTranslateName(
        TF_FilesystemOps* builder, TP_Filesystem_TranslateName translate_name
    )
    {
        builder->translate_name = translate_name;
    }

    static inline void TF_FilesystemOps_SetGetChildren(
        TF_FilesystemOps* builder, TP_Filesystem_GetChildren get_children
    )
    {
        builder->get_children = get_children;
    }

    static inline void TF_FilesystemOps_SetGetMatchingPaths(
        TF_FilesystemOps* builder, TP_Filesystem_GetMatchingPaths get_matching_paths
    )
    {
        builder->get_matching_paths = get_matching_paths;
    }

    static inline void TF_FilesystemOps_SetFlushCaches(
        TF_FilesystemOps* builder, TP_Filesystem_FlushCaches flush_caches
    )
    {
        builder->flush_caches = flush_caches;
    }

    static inline void TF_FilesystemOps_SetGetFilesystemConfiguration(
        TF_FilesystemOps* builder, TP_Filesystem_GetConfiguration get_filesystem_configuration
    )
    {
        builder->get_filesystem_configuration = get_filesystem_configuration;
    }

    static inline void TF_FilesystemOps_SetSetFilesystemConfiguration(
        TF_FilesystemOps* builder, TP_Filesystem_SetConfiguration set_filesystem_configuration
    )
    {
        builder->set_filesystem_configuration = set_filesystem_configuration;
    }

    static inline void TF_FilesystemOps_SetGetFilesystemConfigurationOption(
        TF_FilesystemOps* builder,
        TP_Filesystem_GetConfigurationOption get_filesystem_configuration_option
    )
    {
    ...
    static inline void TF_FilesystemOps_SetSetFilesystemConfigurationOption(
        TF_FilesystemOps* builder,
        TP_Filesystem_SetConfigurationOption set_filesystem_configuration_option
    )
    {
    ...
    static inline void TF_FilesystemOps_SetGetFilesystemConfigurationKeys(
        TF_FilesystemOps* builder,
        TP_Filesystem_GetConfigurationKeys get_filesystem_configuration_keys
    )
    {
