...
extern "C"
{
    ...
    // --------------------------------------------------------------------------
    // TF_PubSub — generic message bus, replacing the separate events and
    // logger domain vtables. Both are "publish a named message, optionally
    // with subscribers" — logger's severity levels are just well-known
    // channel names ("log.debug", "log.info", "log.important", "log.warning",
    // "log.error", "log.fatal"), a documented convention, not a new enum.
    //
    // Brought to the real depth a production message bus needs (Kafka
    // consumer groups, MQTT retained/QoS, RabbitMQ dead-lettering), which
    // neither source domain individually exposed.
    typedef struct TF_PubSub_Subscription TF_PubSub_Subscription;

    // subscription is handed back so a handler can ack/nack the delivery it
    ...
    typedef struct TF_PubSub
    {
        ...
        // retain != 0: the backend keeps this as the channel's last-known
        ...
        // support is backend-dependent, a plugin that can't match patterns
        ...
        // across every subscriber sharing the same group_id rather than
        // fanning out to all.
