...
typedef void (*TP_WorkerManager_SpawnFn)(void* user_data, const TF_TString* spec_json,
                                         TF_Status* status);
...
#define TP_WORKER_MANAGER_STRUCT_SIZE \
  TF_OFFSET_OF_END(TP_WorkerManager, list_cb)
} TP_WorkerManager;
...
void TF_InitWorkerManager(TF_WorkerManagerRegistrationParams* params,
                           TF_Status* status);
...
static inline void TP_WorkerManager_SetBackendName(TP_WorkerManager* builder, TF_TString backend_name) {
  builder->backend_name = backend_name;
}

static inline void TP_WorkerManager_SetRequiredCallback(TP_WorkerManager* builder, TP_WorkerManager_RequiredFn required_cb) {
  builder->required_cb = required_cb;
}

static inline void TP_WorkerManager_SetAddWorkerCallback(TP_WorkerManager* builder, TP_WorkerManager_AddWorkerFn add_worker_cb) {
  builder->add_worker_cb = add_worker_cb;
}

static inline void TP_WorkerManager_SetSpawnCallback(TP_WorkerManager* builder, TP_WorkerManager_SpawnFn spawn_cb) {
  builder->spawn_cb = spawn_cb;
}

static inline void TP_WorkerManager_SetStartCallback(TP_WorkerManager* builder, TP_WorkerManager_StartFn start_cb) {
  builder->start_cb = start_cb;
}

static inline void TP_WorkerManager_SetStopCallback(TP_WorkerManager* builder, TP_WorkerManager_StopFn stop_cb) {
  builder->stop_cb = stop_cb;
}

static inline void TP_WorkerManager_SetListCallback(TP_WorkerManager* builder, TP_WorkerManager_ListFn list_cb) {
