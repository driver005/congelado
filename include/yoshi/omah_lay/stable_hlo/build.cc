// Build-time code generator: emits ONLY table.cppm — the one piece of yoshi_omah_lay_stable_hlo
// that actually varies with generator/ops.json's content (the compiled-in
// stable_hlo_op_schema_table() function, one Operation entry per manifest op, each built via the
// same append_parameter/append_attr/append_result/finalize path a normal caller uses — see
// op/op.cppm).
...
// Invoked at build time by the stable_hlo_generated genrule in
// include/yoshi/omah_lay/stable_hlo/BUILD.
...
// --- JSON manifest structures (must match include/yoshi/omah_lay/stable_hlo/generator/ops.json)
// ---
...
std::string render_op_block(const JsonOpDefinition& op, std::size_t& result_counter)
{
    std::string block = std::format(
        ...
        escape_cpp_string(op.category),
        escape_cpp_string(op.summary)
    );
    ...
    for (std::size_t i = 0; i < op.inputs.size(); ++i) {
        ...
        block += std::format(
            ...
            escape_cpp_string(input.name),
            render_placeholder_shape(input.variadic),
            i
        );
    }
    ...
    for (const JsonAttr& attr: op.attrs) {
        ...
        block += std::format(
            ...
            "        }}\n",
            escape_cpp_string(attr.name),
            escape_cpp_string(attr.cpp_type),
            attr.optional ? "true" : "false",
            attr.list ? "true" : "false"
        );
    }
...
// Attribute, Operation) are hand-written (base/parameter.cppm, base/attribute.cppm,
...
std::vector<std::string> render_table_file(const std::vector<JsonOpDefinition>& ops)
{
    ...
    push_block(
        R"GENCODE(// GENERATED FILE — produced at build time by include/yoshi/omah_lay/stable_hlo/build.cc, do not hand-edit.
// One compiled-in table entry per include/yoshi/omah_lay/stable_hlo/generator/ops.json op — no runtime
// JSON parsing anywhere in this module; build.cc read the manifest once, at build time.

module;

export module yoshi_omah_lay_stable_hlo:table;
...
int main(int argument_count, char** arguments)
{
    ...
    if (!parsed) {
        log_line(
            stderr,
            std::format("stable_hlo_build: failed to parse manifest: {}", parsed.error().what())
        );
        return 1;
    }
    ...
    log_line(
        stdout,
        std::format("stable_hlo_build: wrote {} op schema entries into table.cppm", ops.size())
    );
    return 0;
}
