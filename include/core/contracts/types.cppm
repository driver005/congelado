...
constexpr ContractState operator|(ContractState first_state, ContractState second_state) {
    return static_cast<ContractState>(std::to_underlying(first_state) | std::to_underlying(second_state));
}
constexpr ContractState operator&(ContractState first_state, ContractState second_state) {
    return static_cast<ContractState>(std::to_underlying(first_state) & std::to_underlying(second_state));
}
...
class Worker {
  public:
    ...
    bool schedule() noexcept {
        ...
        return std::to_underlying(prev & (ContractState::SCHEDULED | ContractState::EXECUTING)) == 0;
    }
