...
public:
    ...
    /// @brief Attaches (or replaces) the loaded plugin this runtime drives lifecycle calls
    /// against.
                    ...
                    [&] {
                        ...
                        for (auto& bridge: bridges) {
                            auto fn_context = std::make_unique<FnContext>(
                                std::any{invoke_function}, registered_key
                            );
     ...
     * @return the plugin's own return code, or -1 if no plugin is attached, the
     * `congelado_init` symbol wasn't resolved, or the call threw (caught here and swallowed
     * into -1).
     */
    int init(const CongeladoHostCallbacks* host_cb, const CongeladoConfigView* view) noexcept
    {
        ...
        try {
            ...
            if (auto it = m_plugin->m_data.find(symbol_name); it != m_plugin->m_data.end()) {
                ...
                ); // FIXME(clang-tidy): reinterpret_cast usage — cross-ABI cast of a dlsym'd
                   // void* back to its known function pointer type
    ...
    /**
     * @brief Calls the attached plugin's optional `congelado_on_ready` lifecycle hook, if
     * present.
    ...
    void on_ready() noexcept
    {
        ...
        } catch (...) { // NOLINT(bugprone-empty-catch) — deliberate: never let a plugin-side
                        // throw escape across the ABI boundary
            core::events::publish("ffi.plugin.on_ready_failed");
        }
    }

    /**
     * @brief Calls the attached plugin's optional `congelado_on_unload` lifecycle hook, if
     * present.
    ...
    ) noexcept
    {
        try {
            ...
            auto dispatch_args = std::views::iota(0, num_args) |
                                 std::views::transform([&](int arg_index) {
            ...
            *result = AnyConverter::to_any(return_value);
            ...
            core::events::publish(
                "ffi.call.failed", {{"key", std::string{key, key_len}}, {"error", m_last_error}}
            );
            return -1;
        }
    }
...
suite<"FfiRuntime"> ffi_runtime_suite = [] {
    ...
    "call()'s Str result must be read immediately — v_cstr borrows a Value local to call() that dies the instant it returns"_test =
        [] {
            ...
            // Pins the fragile contract at call()'s `*result =
            // AnyConverter::to_any(return_value)`
            // (~line 415-441): to_any() sets result.v_cstr to `return_value.m_value.c_str()`, a
            // pointer into a Value that is LOCAL to call() and destroyed the instant call()
            // returns. Nothing extends that std::string's lifetime — result.v_cstr is only
            // valid transiently. Any statement between call() returning and this read (another
            // call(), an allocation, anything reusing that freed memory) is free to invalidate
            // it. Read it FIRST, read it NOW — this test's ordering IS the assertion, not just
            // its content.
            expect(rc == 0);
            expect(result.type_index == CG_STR);
