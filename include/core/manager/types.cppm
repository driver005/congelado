...
/**
 * @brief Gets the shared connector::Connector* registered with the host's ContractGroup before
 * build() runs — the engine plugin reads this during its on_load so it uses the SDK-owned
 * connector instead of creating its own unregistered one.
 * @tparam T the concrete connector type to cast to (almost always `connector::Connector`).
 * @param host the host callback table handed to on_load.
 * @return the shared Connector pointer, or nullptr if the host did not set it up.
 */
template <typename T>
[[nodiscard]] T *connector_ctx(const CongeladoHostCallbacks &host) noexcept {
    return static_cast<T *>(host.connector_ctx);
}
