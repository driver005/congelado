...
public:
     ...
     */
    PluginError(Kind kind, std::string message) :
        m_kind{kind},
...
public:
    ...
    [[nodiscard]] std::string_view get_module_name() const noexcept
    {
...
{
public:
    /// @brief Constructs a LuaConfig defaulting the table name to `"congelado"` and safe mode
    /// on.
    LuaConfig() :
        m_table_name{"congelado"} {};
    ...
    [[nodiscard]] bool get_safe_mode() const noexcept
    {
        return m_safe_mode;
    }
...
public:
    ...
    void add_runtime(std::string runtime_name)
    {
        m_wanted_runtimes.insert(std::move(runtime_name));
    }

    /// @brief Sets the Python-specific config (module name, etc).
    /// @param config the new PythonConfig.
    void set_python_config(PythonConfig config)
    {
        m_python = std::move(config);
    }

    /// @brief Sets the Lua-specific config (table name, safe mode, etc).
    /// @param config the new LuaConfig.
    void set_lua_config(LuaConfig config)
    {
        m_lua = std::move(config);
    }
    ...
    [[nodiscard]] const PythonConfig& get_python_config() const noexcept
    {
        return m_python;
    }
    ...
    [[nodiscard]] const LuaConfig& get_lua_config() const noexcept
    {
        return m_lua;
    }

    /// @brief Gets the free-form extra key/value config.
    /// @return the extra config map, forwarded to the plugin's `congelado_init` as key/value
    /// pairs.
 ...
 * `Contract<>` it creates (via `controller_ctx`'s `ContractGroup`) into this so it gets
 * released automatically at host shutdown instead of the plugin managing its own release.
 ...
 * @tparam T the concrete interface type to cast to (almost always
...
/**
 * @brief Gets the resolved cron-capable plugin's interfaces::ICron*, if one was found before
 * this host callback table got built — same "resolve before build()" reasoning (and same shape)
 * as search_ctx()/cache_ctx() above, for the engine plugin's on_load to install its fire
 * callback and seed existing schedules into the backend.
 * @tparam T the concrete interface type to cast to (almost always `interfaces::ICron`).
 * @param host the host callback table handed to `on_load`.
 * @return the resolved pointer, or `nullptr` if no cron-capable plugin was found (schedules
 * then never fire).
 */
template<typename T>
...
/**
 * @brief Gets the resolved worker-manager-capable plugin's interfaces::IWorkerManager*, if one
 * was found before this host callback table got built — same "resolve before build()" reasoning
 * (and same shape) as cron_ctx() above, for the host to drive worker lifecycle after plugins go
 * ready.
 * @tparam T the concrete interface type to cast to (almost always
 * `interfaces::IWorkerManager`).
 * @param host the host callback table handed to `on_load`.
...
config_get(const CongeladoConfigView& cfg, const std::string& key) noexcept
{
...
{ // NOLINT(cppcoreguidelines-missing-std-forward) — deliberately not forwarded, see below
...
suite<"ConfigViewBuilder"> config_view_builder_suite = [] {
    "add() accumulates pairs, view() exposes them as parallel C arrays"_test = [] {
        ...
        expect(view.count == 2);
        expect(
            std::string_view{view.keys[0]} == "threads"
        ); // FIXME(clang-tidy): non-constant array index
        expect(
            std::string_view{view.values[0]} == "4"
        ); // FIXME(clang-tidy): non-constant array index
        expect(
            std::string_view{view.keys[1]} == "migrations_dir"
        ); // FIXME(clang-tidy): non-constant array index
        expect(
            std::string_view{view.values[1]} == "db/migrations"
        ); // FIXME(clang-tidy): non-constant array index
    };
};
