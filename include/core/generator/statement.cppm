...
class Param {
  public:
     ...
     */
    Param(std::string type, std::string name) : m_type(std::move(type)), m_name(std::move(name)) {}
...
class Stmt {
  public:
     ...
     */
    [[nodiscard]] static Stmt for_stmt(std::string init, std::string condition, std::string step,
  ...
  private:
    ...
    enum class Kind : std::uint8_t { RAW, EXPR, VAR_DECL, RETURN, IF, FOR };
