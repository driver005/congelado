...
/**
 * @brief Async request executor: the http2 dispatch enqueues a request here and returns,
 * freeing the receiver thread; this handler — registered as a `core::contract` worker like
 * `connector::Connector` — drains the queue on a worker thread, running `match()` (route +
 * handler
...
public:
     ...
     * @param req the request; a reference into the live stream, valid until its match
     * completes.
     ...
     * `send`), then self-reschedule if more remain. Empty queue → quietly idle until
     * `enqueue()` wakes it again. Mirrors `connector::Connector::on_execute`.
...
private:
    ...
    // True while on_execute() is running a popped request, so a reentrant enqueue() doesn't
    // wake.
