...
public:
    ...
    /**
     * @brief Appends a route to the flat route array and bumps the base-router child count if
     * it hangs straight off the root.
     ...
     * walk and any per-node middleware. `build()` carries these into the produced
     * `RouteHandler`.
     ...
     * @throws std::out_of_range if `index` is past the currently used range — no silent
     * clamping, you get bounced hard.
     */
    constexpr Route<MaxHandlerSize, MaxMiddlewareSize>& operator[](std::size_t index)
    {
        ...
        return m_routes[index]; // FIXME(clang-tidy): unchecked operator[], consider .at();
     ...
     * @warning Consumes `m_routes` via `insert_route()`'s `is_build()` guard — call this twice
     * and the second pass just skips every already-built route, silently returning a handler
     * missing whatever you expected to be freshly rebuilt. No cap, that's an easy L if you
     * don't know it's one-shot.
    ...
    RouteHandler<> build()
    {
        ...
        while (group_start != active_routes.end()) {
            ...
            auto group_end = std::ranges::find_if(
                group_start, active_routes.end(), [&](const auto& current_route) {
        ...
        while (idx < m_router_size) {
            insert_route(m_routes[idx], table); // FIXME(clang-tidy): unchecked operator[],
        ...
        for (const auto& row: table) {
            ...
            for (const auto& [index, node]: row | std::views::enumerate) {
                ...
                handler.add_route(
                    node.m_kind, node.m_literal, offset, node.m_handlers, node.m_middlewares,
                    node.m_children
                );
            }
        }
     ...
     * different numbers.
     * @note Drawn from a `static inline` counter shared across every `RouterContext` instance
     * in the process (for this template instantiation), not a per-instance one —
     * `utils::openapi:: Registry` is itself a process-wide singleton that more than one
     * `RouterContext` can feed (e.g. engine's live router plus a throwaway one built purely to
     * register another module's routes for documentation, see `engine.cc`'s `on_load`), and
     * Generator reconstructs full paths by walking `router_number`/`base_router` across that
     * ENTIRE Registry, not scoped to whichever `RouterContext` produced a given entry. A
     * per-instance counter that always restarts at 1 collides across instances — two unrelated
     * nodes end up sharing the same `router_number`, and the path-reconstruction walk picks up
     * the wrong parent chain for whichever entry it finds first. A shared counter makes every
     * number unique process-wide, matching what Registry's own flat, cross-instance walk
     * actually needs.
     * @return the router number to claim for this call.
     */
    std::size_t get_highest_router_number() noexcept
    {
        return s_global_router_number.fetch_add(1, std::memory_order_relaxed);
    }
     ...
     */
    [[nodiscard]] constexpr std::size_t get_router_size() const noexcept
    {
        return m_router_size;
    }
...
private:
    ...
    /**
     * @brief Classifies a raw path segment into its edge kind and, for literal paths, the
     * literal text to match on.
     * @param path the router-level path to classify — `"*"` for wildcard, a leading `:` for a
     * param, anything else is a literal path segment.
     * @return a pair of the detected `EdgeKind` and the literal view (empty for
     * `Wild`/`Param`).
     ...
     */
    static std::size_t resolve_target_row(
        const std::vector<std::vector<RouterBuildingHelper>>& table,
    ...
    )
    {
        ...
        if (row_iterator == table.end()) {
            throw std::runtime_error(
                std::format(
                    "Parent router with number {} not found for route {}", route.get_base_router(),
                    route.get_path()
                )
            );
        }
     ...
     * node under its parent's slot or as a leaf route hanging off whichever router it belongs
     * to.
     ...
     * leaf.
     * @warning Marks `route.is_build()` as a side channel — `build()` calls this once per
     * stored route, and a route already flagged built is silently skipped, no error. Skip
     * re-running `build()` on the same context or this quietly drops routes.
     * @throws std::runtime_error if the router path is empty, parameterized, or wildcard
     * (routers can't be any of those — only leaf routes can), or if the parent router
     * referenced by `route.get_base_router()` isn't found in `table` yet.
     */
    constexpr void insert_route(
        const Route<MaxHandlerSize, MaxMiddlewareSize>& route,
        std::vector<std::vector<RouterBuildingHelper>>& table
    )
    { // FIXME(clang-tidy): readability-function-cognitive-complexity — needs decomposition
        ...
        table[NEW_IDX].push_back(
            RouterBuildingHelper{
        ...
        (void)
            route.is_build(); // FIXME(clang-tidy): clang-diagnostic-unused-result — likely meant to
                              // call the mutating set_build(), but route is a const reference here,
                              // so fixing it properly needs a signature change; not guessing
    }
...
public:
     ...
     * @note Named `delt` and not `delete` for the obvious reason — `delete` is a keyword, so
     * this is the workaround. No cap, kind of a weird one to type at first.
     * @return `*this`, moved out.
     */
     ...
     * real footgun: a dupe route that would blow up at runtime can slide right through a
     * constexpr build. Straight up cooked if you're relying on compile-time checks to catch
     * this.
     ...
     */
    constexpr Route<>
    add_handler(interfaces::io::types::Method method, interfaces::HandlerFn handler)
    {
        ...
        if consteval {
            // static_assert(false, "Duplicate handler found for method please check your
            // routes");
            return std::move(*this);
        } else {
            throw std::runtime_error(
                std::format(
                    "Handler for method {} already exists on path {}", std::to_underlying(method),
                    m_path
                )
            );
        }
     ...
     */
    [[nodiscard]] constexpr std::string_view get_path() const noexcept
    {
        return m_path;
    }
     ...
     */
    [[nodiscard]] constexpr bool is_build() const noexcept
    {
        return m_is_build;
    }
     ...
     */
    [[nodiscard]] constexpr std::size_t get_child_routes() const noexcept
    {
        return m_child_routes;
    }
     ...
     */
    [[nodiscard]] constexpr std::size_t get_base_router() const noexcept
    {
     ...
     * anything above 255 and it truncates on assignment, silently, no warning, no throw.
     * Straight L waiting to happen on a big fan-out router.
     */
     ...
     * skip it.
     */
    constexpr void set_build() noexcept
    {
        m_is_build = true;
    }
     ...
     */
    constexpr void increment_child_routes() noexcept
    {
        ++m_child_routes;
    }
...
public:
    ...
    constexpr Router(RouterContext<>& ctx, std::string_view path, Args&&... middlewares) :
        m_ctx{ctx},
        m_router_number{ctx.get_highest_router_number()},
        m_router_index{ctx.add_route(
            Route<>{path, std::forward<Args>(middlewares)...}.set_router_number(m_router_number)
        )}
    {
    }
    ...
    {
        m_ctx.get()[m_router_index].add_middleware(
            middleware
        ); // FIXME(clang-tidy): unchecked operator[], consider .at()
        return std::move(*this);
    }
    ...
    {
        m_ctx.get()[m_router_index].add_handler_in_place(
            interfaces::io::types::Method::GET, // FIXME(clang-tidy): unchecked operator[],
                                                // consider .at()
            std::move(handler)
        );
        return std::move(*this);
    ...
    {
        m_ctx.get()[m_router_index].add_handler_in_place(
            interfaces::io::types::Method::POST, // FIXME(clang-tidy): unchecked operator[],
                                                 // consider .at()
            std::move(handler)
        );
        return std::move(*this);
    ...
    {
        m_ctx.get()[m_router_index].add_handler_in_place(
            interfaces::io::types::Method::PUT, // FIXME(clang-tidy): unchecked operator[],
                                                // consider .at()
            std::move(handler)
        );
        return std::move(*this);
    ...
    {
        m_ctx.get()[m_router_index].add_handler_in_place(
            interfaces::io::types::Method::DELETE, // FIXME(clang-tidy): unchecked operator[],
                                                   // consider .at()
            std::move(handler)
        );
        return std::move(*this);
    ...
    {
        m_ctx.get()[m_router_index].add_handler_in_place(
            interfaces::io::types::Method::PATCH, // FIXME(clang-tidy): unchecked operator[],
                                                  // consider .at()
            std::move(handler)
        );
        return std::move(*this);
    ...
    constexpr Router<> add_router(Router<> sub) &&
    {
        ...
        m_ctx.get()[sub.get_router_index()].update_base_router(
            m_router_number
        ); // FIXME(clang-tidy): unchecked operator[], consider .at()
        ...
        m_ctx.get()[m_router_index].increment_child_routes(); // FIXME(clang-tidy): unchecked
     ...
     */
    [[nodiscard]] constexpr std::size_t get_router_index() const noexcept
    {
        return m_router_index;
    }
...
public:
     ...
     * @warning `m_port` is a `std::uint8_t`, but a real port number needs up to 16 bits
     * (0-65535). The cast here is explicit (`static_cast<std::uint8_t>(port)`) so the compiler
     * won't even warn you — `port(8080)` silently becomes `144`. That's not lowkey, that's a
     * straight-up L for anyone passing a normal HTTP port. Document now, don't fix per
     * instructions, but heads up: this one bites hard.
     * @note Rvalue-qualified (`&&`).
     * @return `*this`, moved out.
     */
    ...
    // TODO: figue out a way to make this function consteval
    /**
     * @brief Finalizes the build: drops the root and fallback routes into slots 0 and 1 of
     * `ctx`, syncs the root's child count from the context's tracked base-router children, and
     * delegates to `RouterContext::build()`.
     * @param ctx the router context to build from, taken by value (this builder owns its own
     * copy to finalize).
...
suite<"Router"> router_suite = [] {
    ...
    "use()/get() build against the router's own route"_test = [] {
        ...
        expect(ctx[router.get_router_index()].get_middlewares().get_size() == 1);
        expect(
            ctx[router.get_router_index()].get_handlers().find(
                interfaces::io::types::Method::GET
            ) != nullptr
        );
    };
