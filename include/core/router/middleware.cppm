...
public:
     ...
     * error, has nothing to do with `MiddlewareSize`. Message is straight up wrong, no cap,
     * don't trust it for debugging, trust the actual bound (`MiddlewareSize`).
    ...
    constexpr void add_middleware(interfaces::MiddlewareFn middleware)
    {
        ...
        if (m_middleware_index >= MiddlewareSize) {
            throw std::runtime_error(
                "MiddlewareSize cannot be greater than MaxHandlerSize due to "
                "offerflow limitations, please check "
                "your routes"
            );
        }

        // tack it onto the end, in registration order — bet, that order is execution order
        // later
        m_middleware[m_middleware_index++] = middleware; // FIXME(clang-tidy): unchecked operator[],
     ...
     * via continuation-passing — the real entry point into the middleware pipeline for a
     * matched route.
     ...
     * @note Execution order is strictly ascending by index within the slice — each middleware
     * only advances by calling its `next` callback, so ordering here is 100% caller-controlled
     * by how `offset`/`length` were computed upstream. Get that math wrong and you run the
     * wrong slice, no cap.
     */
    void execute(
...
private:
     ...
     * @param border the exclusive upper bound of the slice — recursion stops once `offset`
     * reaches this.
     ...
     * fallthrough to the handler. That's the classic "forgot to call next()" L, and it's on
     * every middleware author to get right since nothing here enforces it. Also captures `this`
     * by reference in the `next` lambda, so the `Middleware` object needs to stay alive for the
    ...
    ) const noexcept
    {
        ...
        if (offset < border) {
            const auto CURRENT_MW =
                m_middleware[offset]; // FIXME(clang-tidy): unchecked operator[], consider
