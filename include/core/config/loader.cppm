...
public:
     ...
     */
    static void
    parse_field(PluginConfig& plugin_config, std::string_view key, simdjson::dom::element value)
    {
...
static std::expected<Config, std::string> parse_toml(const std::filesystem::path& path)
{
    ...
    try {
        ...
        core::events::publish(
            "config.load.parse_failed", {{"path", path.string()}, {"error", e.what()}}
        );
        return std::unexpected(
            std::format("TOML parse error in '{}': {}", path.string(), e.what())
        );
    }

    // Top-level `threads` scalar — the process-wide worker-thread count. Absent falls back to
    // Config's own default (1). Drives the app context's contract thread pool and is injected
    // as the per-plugin `threads` default downstream (see App::load_plugins).
    if (auto threads = toml_table["threads"].value<std::int64_t>();
        threads && *threads > 0) { // FIXME(clang-tidy): unchecked operator[], consider .at()
    ...
    if (auto migrations_dir =
            toml_table["migrations_dir"].value<std::string>()) { // FIXME(clang-tidy): unchecked
    ...
    if (auto* plugins =
            toml_table["plugins"]
                .as_table()) { // FIXME(clang-tidy): unchecked operator[], consider .at()
...
static std::expected<Config, std::string> parse_json(const std::filesystem::path& path)
{
    ...
    if (auto ec = parser.load(path.string()).get(doc); ec) {
        return std::unexpected(
            std::format("JSON parse error in '{}': {}", path.string(), simdjson::error_message(ec))
        );
    }
    ...
    if (doc["threads"].get_int64().get(threads_value) == 0U &&
        threads_value > 0) { // FIXME(clang-tidy): unchecked operator[], consider .at()
    ...
    if (doc["migrations_dir"].get_string().get(migrations_dir_value) ==
        0U) { // FIXME(clang-tidy): unchecked operator[], consider .at()
    ...
    if (doc["plugins"].get(plugins_element) ==
        0U) { // FIXME(clang-tidy): unchecked operator[], consider .at()
        ...
        if (plugins_element.get(plugins_obj) == 0U) {
            for (auto [pname, psection]: plugins_obj) {
                ...
                for (auto [section_key, section_value]: section_obj) {
                    JsonPluginFieldParser::parse_field(
                        plugin_config, std::string_view{section_key}, section_value
                    );
                }
    ...
    if (doc["providers"].get(providers_element) ==
        0U) { // FIXME(clang-tidy): unchecked operator[], consider .at()
...
suite<"load"> load_suite = [] {
    ...
    "valid toml parses threads, plugins, and providers"_test = [] {
        ...
        expect(
            result->get_plugins().at("file_logger").get_fields().at("path") == "/var/log/app.log"
        );
