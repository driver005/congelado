...
public:
     ...
     */
    [[nodiscard]] const std::unordered_map<std::string, std::string>& get_fields() const noexcept
    {
        return m_fields;
    }
     ...
     * @return all key/value fields for this plugin section.
     */
    std::unordered_map<std::string, std::string>& get_fields() noexcept
    {
        return m_fields;
    }
...
public:
     ...
     */
    void add_plugin(PluginConfig plugin)
    {
        m_plugins[plugin.get_name()] = std::move(plugin);
    }
     ...
     * @return all plugin configs, name → PluginConfig.
     */
    std::unordered_map<std::string, PluginConfig>& get_plugins() noexcept
    {
        return m_plugins;
    }
     ...
     * shape is uniform, the "how many actually get used" policy lives with each capability's
     * own resolution code, not here.
     ...
     */
    [[nodiscard]] const std::optional<std::size_t>& get_threads() const noexcept
    {
        return m_threads;
    }
     ...
     * was never set.
     */
    [[nodiscard]] const std::string& get_migrations_dir() const noexcept
    {
        return m_migrations_dir;
    }
