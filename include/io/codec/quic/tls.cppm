...
public:
    /**
     * @brief Stands up a server-side `SSL_CTX` wired for QUIC: loads the cert/key pair, pins
     * ALPN to "h3" only, disables session tickets, skips client-cert verification, and locks
     * the proto range to TLS 1.3. This is the whole security posture for every connection born
     * from the resulting context, so get the inputs right.
     * @warning Throws (not an error code) on any setup failure — cert file missing, key
     * mismatch, whatever. No partial/degraded context ever gets handed back; it's
     * all-or-nothing. Also: this hard-requires TLS 1.3 with `SSL_VERIFY_NONE` for client certs
     * — that's a deliberate choice for server-only auth, not a placeholder to "fix later."
     * Don't loosen it without knowing why it's set that way.
    ...
    /**
     * @brief Builds an empty, invalid context — no `SSL_CTX` yet. Use from_files() to get a
     * real one.
     */
    TlsContext() = default;

    /**
     * @brief Frees the underlying `SSL_CTX` if this context ever got a real one from
     * from_files().
     */
    ~TlsContext()
    {
    ...
    /**
     * @brief Deleted — `SSL_CTX*` ownership can't be shared, copying would double-free on
     * teardown.
     */
...
public:
     ...
     * @note Uses a local RAII guard around the freshly-allocated SSL* so a throw
     * mid-construction (e.g. `BIO_new_dgram` failing) still frees it instead of leaking — that
     * guard gets disarmed right before the constructor body finishes, once ownership's safely
     * landed in `m_ssl`.
     * @warning `udp_fd` isn't owned by this session — `BIO_NOCLOSE` means the BIO won't close
     * it, so lifetime management of the fd itself is entirely on the caller. Free the SSL
     * first, close the fd whenever you're actually done with it.
     * @param ctx the TLS context to pull the underlying `SSL_CTX*` from; must be valid() or
     * this throws.
    ...
    {
        // Allocate the SSL* off the shared context first — nothing downstream matters without
        // it.
        ...
        {
        public:
            explicit Guard(SSL* ssl) noexcept :
                m_ssl(ssl)
            {
            }
     ...
     * @warning The return value conflates two different states — true means either "still
     * going, come back later" (WANT_READ/WANT_WRITE) or "fully done." It does NOT distinguish
     * between them. Don't treat a `true` as "handshake complete" without checking
     * connection/session state separately, or you'll think you're ready to send app data when
     * you're actually still mid-flight.
     * @return true if the handshake is still in progress or just completed; false only on a
     ...
     */
    [[nodiscard]] SSL* create_stream(std::uint64_t flags = 0) noexcept
    {
        return ::SSL_new_stream(m_ssl, flags);
    }
