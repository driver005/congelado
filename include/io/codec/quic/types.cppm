...
export constexpr std::size_t varint_len(VarInt value) noexcept
{
    ...
    // onto these four buckets (1/2/4/8 bytes), so whichever range `value` lands in decides the
    // width.
    if (value < (1ULL << 6)) {
        return 1;
    }
...
export constexpr std::pair<VarInt, std::size_t>
varint_decode(const std::byte* buf, std::size_t len) noexcept
{
    ...
    // Low 6 bits of the first byte seed the value, then every following byte folds in
    // MSB-first.
...
{
    std::uint8_t
        data[CID_MAX_LEN]{}; // NOLINT(readability-identifier-naming) — renaming to m_data would
                             // break the public-field access in crypto.cppm (cid.data)
    std::uint8_t len{0};     // NOLINT(readability-identifier-naming) — renaming to m_len would
                             // break the public-field access in crypto.cppm (cid.len)
    ...
    [[nodiscard]] bool operator==(const ConnectionId& other) const noexcept
    {
        return len == other.len &&
               std::memcmp(data, other.data, len) == 0; // FIXME(clang-tidy): array-to-pointer decay
    }

    /** @brief Gets a read-only view over the active `len` bytes. @return span over the CID
     * data. */
     ...
     * @warning `out` needs at least `len * 2` bytes of room — this doesn't check, it just
     * writes. Buffer's on you.
    ...
    void hex_into(char* out) const noexcept
    {
        ...
        for (std::uint8_t i = 0; i < len; ++i) {
            ...
            out[OFFSET + 1] =
                HEX_DIGITS[data[i] & 0xF]; // FIXME(clang-tidy): non-constant array index
        }
    }
...
// NOLINTNEXTLINE(readability-identifier-naming) — renaming these enumerators to UPPER_CASE
// would break every ConnState::Handshaking/Connected/Closing/Closed call site in
// connection.cppm
export enum class ConnState : std::uint8_t
{
    Handshaking,
    Connected,
    Closing,
...
export constexpr bool stream_is_uni(std::uint64_t streamId) noexcept
{
    return (streamId & 0x2) != 0;
}

export constexpr bool stream_is_bidi(std::uint64_t streamId) noexcept
{
    return (streamId & 0x2) == 0;
}
...
public:
    /**
     * @brief Wraps a read-only cursor around `buf`, starting at offset 0 — no copy, just a
     * view.
    ...
    /**
     * @brief Decodes a QUIC varint starting at the cursor and advances past however many bytes
     * it took.
     * @param out written with the decoded value on success, left untouched on failure.
     * @return true if a full varint was available to decode, false if the buffer cut off
     * mid-value.
     */
     ...
     * @return a span over the sliced bytes, or an empty span if fewer than `count` remain —
     * check before trusting the result, an empty span isn't always "I asked for zero".
     */
    ...
    {
        // Guard first — no partial skips, either the whole `count` bytes are there or nothing
        // moves.
        if (remaining() < count) {
            return false;
        }
...
public:
    ...
    bool write_u8(std::uint8_t value) noexcept
    {
        ...
        m_buf[m_pos++] = static_cast<std::byte>(
            value
        ); // FIXME(clang-tidy): unchecked operator[], consider .at()
        return true;
    }
    ...
    /**
     * @brief Carves out `count` bytes of writable space and advances the cursor past them,
     * letting the caller fill it in directly instead of staging a copy first.
    ...
    {
        // Same shape as read_bytes() but on the write side — carve out the span, move the
        // cursor past it, and let the caller fill it in directly.
        if (remaining() < count) {
            return {};
        }
        auto sliced = m_buf.subspan(m_pos, count);
     ...
     */
    [[nodiscard]] std::span<std::byte> written_span() noexcept
    {
...
struct std::hash<quic::ConnectionId>
{
    ...
    std::size_t operator()(const quic::ConnectionId& cid) const noexcept
    {
        ...
        std::size_t digest = 14'695'981'039'346'656'037ULL;
        for (std::uint8_t i = 0; i < cid.len; ++i) {
            digest = (digest ^ cid.data[i]) *
                     1'099'511'628'211ULL; // FIXME(clang-tidy): non-constant array index
        }
        return digest;
    }
};
...
suite<"varint_len/varint_encode/varint_decode"> varint_suite = [] {
    "round-trips every length-class boundary"_test = [] {
        for (VarInt value:
             {VarInt{0}, VarInt{63}, VarInt{64}, VarInt{16'383}, VarInt{16'384},
              VarInt{1'073'741'823}, VarInt{1'073'741'824}, VARINT_MAX}) {
    ...
    "varint_len picks the smallest width for each RFC 9000 bucket"_test = [] {
        ...
        expect(varint_len(16'383) == 2);
        expect(varint_len(16'384) == 4);
        expect(varint_len(1'073'741'823) == 4);
        expect(varint_len(1'073'741'824) == 8);
    };
    ...
    "varint_decode on a truncated buffer returns the {0,0} invalid sentinel"_test = [] {
        std::array<std::byte, 8> buf{};
        varint_encode(buf.data(), VarInt{16'384}); // needs 4 bytes
...
suite<"ConnectionId"> connection_id_suite = [] {
    ...
    // under NDEBUG/release, leaving an unchecked std::memcpy into the fixed 20-byte `data`
    // array. A span exactly CID_MAX_LEN long is the largest input that's safe to exercise here;
    // anything past it would either trip the assert (aborting the whole shared test binary) or,
    // with asserts disabled, perform a real OOB memcpy, so that case is deliberately not
    // exercised.
    "constructing from exactly CID_MAX_LEN bytes succeeds at the boundary"_test = [] {
        std::array<std::uint8_t, CID_MAX_LEN> raw{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
