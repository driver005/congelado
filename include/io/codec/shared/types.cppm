...
enum class IndexCalculation : std::uint8_t
{
    Q_PACK,
    H_PACK
};
...
public:
     ...
     * @param is_static true if the entry lives in the static table, false (default) for
     * dynamic.
     ...
     */
    static constexpr SearchResult none() noexcept
    {
        return {};
    }
     ...
     * @return true if this is a found result representing a full name+value match, as opposed
     * to name-only.
     */
...
public:
     ...
     */
    [[nodiscard]] constexpr bool is_never_indexed() const noexcept
    {
        return (m_prefix_bits & 0x02) != 0;
    }
...
template<typename T>
concept CastableToUint8 =
    std::convertible_to<T, std::uint8_t> ||
...
constexpr std::uint8_t operator|(PrefixHelper lhs, std::uint8_t rhs)
{
    return static_cast<std::uint8_t>(lhs) | rhs;
}

constexpr std::uint8_t operator|(std::uint8_t lhs, PrefixHelper rhs)
{
...
suite<"detect_representation_qpack_stream"> detect_representation_qpack_stream_suite = [] {
    "classifies each unambiguous priority-ordered pattern"_test = [] {
        ...
        expect(
            detect_representation_qpack_stream(0x01) == PrefixHelper::QPACK_POST_BASE_INDEXED_FIELD
        );
    };
...
suite<"detect_representation_qpack_encoder"> detect_representation_qpack_encoder_suite = [] {
    "classifies each unambiguous priority-ordered pattern"_test = [] {
        ...
        expect(
            detect_representation_qpack_encoder(0x20) ==
