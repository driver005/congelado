...
public:
     ...
     */
    explicit QPackTable(std::size_t max_capacity = 0) :
        m_dyn{max_capacity}
    {
    }
     ...
     * @tparam IsIndexPostBase when true, `idx` is a post-base index; when false, a relative
     * one.
     ...
     */
    template<bool IsIndexPostBase = false, bool IsStatic>
    [[nodiscard]] std::shared_ptr<interfaces::io::HeaderField<true>>
    at(std::size_t idx, std::size_t base = 0) const
    {
     ...
     */
    void set_max_size(std::size_t cap)
    {
        m_dyn.set_max_size(cap);
    }
     ...
     */
    [[nodiscard]] std::size_t insert_count() const noexcept
    {
        return m_dyn.get_insert_count();
    }
...
private:
    ...
    post_base_to_absolut_index(std::size_t base, std::size_t post_base) noexcept
    {
        return base + 1 + post_base;
    }

    // [[nodiscard]] static std::size_t abs_to_post_base(std::size_t base, std::size_t abs)
    // noexcept
    // {
    //     if (abs <= base)
    //         return shared_codec::SIZE_MAX;
