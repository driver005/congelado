...
public:
     ...
     */
    Flow(leverage::Leverager<leverage::Context>& lev, Controller ctrl) :
        m_leverager(lev),
        m_controller(ctrl)
    {
    }
    ...
    /**
     * @brief Builder-chain add — queues up a new `T` recipe (constructed lazily at build()
     * time) and returns a fresh `Flow` whose type pack now includes it. Bet: this doesn't
     * mutate
     * `*this`, it hands back a brand-new `Flow<Controller, Ts..., decltype(recipe)>` instead,
     * so each `.add()` call in a chain changes the static type.
...
private:
    ...
    leverage::Leverager<leverage::Context>&
        m_leverager; // FIXME(clang-tidy): cppcoreguidelines-avoid-const-or-ref-data-members —
                     // switching to a pointer would change nullability/rebinding semantics
                     // across the builder chain (add()/build()); not a mechanical fix
...
public:
    ...
    Scheduled
    create(std::string_view, shared::WorkerFunction, shared::ReleaseFunction, shared::ErrorHandler)
    {
        return {};
    }
};

// Satisfies shared::FlowBase<MockFlowComponent, MockFlowController,
// leverage::Leverager<leverage::Context>> — constructible from (ReadCallback&&,
// Leverager<Context>&, Controller), has on_send(int) -> SendCallback. This is exactly the shape
// `Flow::add<T>()` requires of T.
...
// `leverage::Leverager<leverage::Context>&` — constructing one spins up a live io_uring ring,
// the same "not unit-testable in isolation" situation `io_base_leverage`'s own test block
// documents for Leverager<Context> itself. There's no injection point here (unlike
// Receiver/Sender, which take a mockable Worker type param), so runtime
// construction/add()/build() can't be exercised without live io_uring — only the compile-time
// concept gating add<T>() relies on is covered above.
