...
public:
    /**
     * @brief Builds a Sender with no error callback yet — nothing's armed until build() sets
     * one and attaches.
    ...
    /**
     * @brief Move ctor deleted — this handler gets `create()`'d against a controller and
     * captures `this` in its lambdas, moving it would dangle those captures.
     */
    ...
    {
        core::logger::debug(
            "io/send", "fd {} enqueue {} bytes", m_worker.get().get_fd(), slot.get_written()
        );
     ...
     * up — resume()s and either reschedules or releases based on the outcome.
    ...
    shared::ErrorHandler on_error() override
    {
        return [this](const std::exception_ptr& eptr) {
            ...
            try {
                ...
                core::logger::warning(
                    "io/send", "fd {} sys error: {} ({})", m_worker.get().get_fd(), e.what(),
                    e.code().value()
                );
                core::events::publish(
                    "io.send.sys_error", {{"fd", std::to_string(m_worker.get().get_fd())},
                                          {"error", e.what()},
                ...
                core::logger::warning(
                    "io/send", "fd {} exception: {}", m_worker.get().get_fd(), e.what()
                );
                core::events::publish(
                    "io.send.exception",
                ...
                core::logger::warning(
                    "io/send", "fd {} unknown exception", m_worker.get().get_fd()
                );
                core::events::publish(
                    "io.send.unknown_exception", {{"fd", std::to_string(m_worker.get().get_fd())}}
                );
                m_on_error(m_worker.get().get_fd(), -1);
            }
        };
    ...
    {
        return [this](utils::buffering::BufferNode&& node) {
            this->send(std::move(node));
        };
    }

    /**
     * @brief Arms another async write attempt if this sender isn't already dead.
     * @note Same one-way-door deal as the async Receiver's `m_fatal` — once it flips true
     * there's no resetting it on this object.
     ...
     */
    [[nodiscard]] bool has_on_error() const noexcept
    {
        return !m_on_error;
    }
...
private:
     ...
     * write's still in flight is a straight-up dangling-pointer UB situation. Owning code has
     * to guarantee this outlives every in-flight write.
     */
    void arm_write()
    {
        ...
        m_worker.get().async_send(data, static_cast<unsigned>(size), [this](int result) mutable {
     ...
     * flips fatal and reports on any other negative result, otherwise consumes the written
     * bytes off the front of the queue.
...
static_assert(
    interfaces::io::AsyncSendable<socket::Socket<socket::Protocol::TCP>, socket::SocketStatus>
);
...
//   3. Sender overrides shared::HandlerBase's pure virtual
//   on_execute()/on_released()/on_error().
...
//      returns.
//
//   A get_fd() returning `int` fails the class's own `requires
//   interfaces::io::IoAsyncSend<...>` clause outright (the type can't be named at all). A
//   get_fd() returning `void` satisfies that clause but then fails to compile inside
//   on_execute()/on_error()/arm_write(). No Worker can satisfy both at once. Confirms this
//   isn't a mocking gap: grepping the codebase, `interfaces::io::IoAsyncSend` is referenced
//   exactly once anywhere — this class's own requires-clause — and every static_assert in this
//   file (see above) and its Receiver/sync- Sender siblings checks the plain
//   `AsyncSendable`/`AsyncReceivable`/`SyncSendable` concept instead, never a composite that
//   drags in `AsyncGetter`/`SyncGetter`. So what follows only exercises the compile-time
//   concept machinery the class's requires-clause depends on.
...
public:
    ...
    void async_send(
        const std::byte*, std::size_t, interfaces::io::IoCallback<socket::SocketStatus>
    ) noexcept
    {
    }
};
...
suite<"Sender (async) — IoAsyncSend concept gating (see block comment above)">
    sender_async_concept_suite = [] {
...
// UAF-design-gap test (same structural pattern as Receiver's async.cppm / Connector's) NOT
// added here: as the block comment at the top of this suite proves, no `Sender<Worker, Status,
// Args...>` can be instantiated at all for ANY Worker — the class's own `requires
...
// this test would target is unreachable without a live `Sender` instance to destroy, and none
// can exist, this finding is documented-and-skipped rather than faked with a type that wouldn't
