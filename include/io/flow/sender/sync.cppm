...
public:
     ...
     * @warning Unlike the async Sender's build(), this doesn't attach or arm anything — purely
     * a presence check on the callback.
    ...
    /**
     * @brief Builds the exception handler for this sender — same unpack-and-route pattern as
     * the async version.
    ...
    bool resume()
    {
        // A closed sender keeps flushing until its pool is empty (arm_write() now writes even
        // when closed — see there), then releases. This lets a final GOAWAY / response tail go
        // out over the still-open socket; a dead socket drops its backlog in arm_write(), so
        // this converges.
...
private:
     ...
     * through `m_on_error`.
     * @note Every branch resets `m_stalled` back to false, bet, letting the next resume()
    ...
    void arm_write()
    {
        ...
        switch (status.get_status()) {
            ...
            case socket::VALUES::TIMED_OUT:
                core::logger::warning(
                    "io/send", "fd {} send error: {} ({})", DESCRIPTOR, status.get_error_code(),
                ...
                core::events::publish(
                    ...
                    {{"fd", std::to_string(DESCRIPTOR)},
                     ...
                     {"error",
                      std::string{utils::ErrnoTranslator::describe_errno(status.get_error_code())}}}
                );
                m_closed = true;
                // Socket's dead — drop whatever's still queued so a closed sender's resume()
                // sees an empty pool and releases instead of looping on an unsendable backlog.
...
// so this is a const member function with mutable bookkeeping, same as the real
// Socket::sync_send().
...
suite<"Sender (sync) on_error"> sender_sync_error_suite = [] {
    ...
    "real error code — see NOTE above"_test = [] {
        MockSyncSendWorker worker;
        int error_code = 0;
        TestSender sender{worker, [&](int, int code) {
                              error_code = code;
                          }};
