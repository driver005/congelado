...
class Receiver : public shared::HandlerBase {
  ...
  private:
    ...
    void arm_read() {
        ...
        // RAII guard for the buffer slot — every exit path must drop the reference acquire()
        // handed out, otherwise the BufferNode leaks. Disarm the guard on the success path
        // because notify_read() itself drops exactly this reference.
        struct SlotGuard {
            utils::buffering::BufferWriter &pool;
            utils::buffering::NodeReader *slot;
            ~SlotGuard() {
                if (slot != nullptr) {
                    pool.release(slot);
                }
            }
            void disarm() noexcept { slot = nullptr; }
        };

        // Grab a buffer slot and do the actual blocking-ish sync receive call.
        auto *slot = m_pool.acquire();
        SlotGuard guard{m_pool, slot};
        ...
        case socket::VALUES::VALID: {
            ...
            // notify_read() drops the slot reference; disarm the guard so it doesn't double-release.
            guard.disarm();
        ...
        case socket::VALUES::NON_BLOCKING_WOULD_HAVE_BLOCKED:
            ...
            // Guard releases the slot on the way out — no bytes landed.
            m_stalled = false;
            return;
