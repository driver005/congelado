...
public:
     ...
     */
    void add_on_accept(OnAcceptCallback on_accept) &
    {
    ...
    /**
     * @brief Blocks on a sync accept and, if a connection actually landed, flips it
     * non-blocking and forwards it to `m_on_accept`.
    ...
    void accept()
    {
        ...
        if (accepted_socket) {
            ...
            core::logger::debug(
                "io/socket", "accepted {} from {}", m_socket.get_endpoint().to_string(),
    ...
    bool resume()
    {
        ...
        if (m_closed) {
            core::logger::warning(
                "io/socket", "endpoint {} closed, cannot resume",
                m_socket.get_endpoint().to_string()
            );
            core::events::publish(
                "io.socket.closed_cannot_resume",
     ...
     * up — resume()s and either reschedules or releases based on whether it's still open.
     ...
     */
    [[nodiscard]] const socket::SOCKET& get_fd() const noexcept
    {
        return m_socket.get().get_fd();
    }
     ...
     */
    [[nodiscard]] socket::Endpoint get_endpoint() const noexcept
    {
        return m_socket.get_endpoint();
    }
     ...
     * @return the bound endpoint.
     */
    socket::Endpoint get_endpoint() noexcept
    {
        return m_socket.get_endpoint();
    }
     ...
     */
    socket::Endpoint get_recived_endpoint() noexcept
    {
        return m_socket.get_recived_endpoint();
    }
...
public:
    ...
    void start()
    {
        ...
        if (connect_status.get_status() != socket::VALUES::VALID) {
            core::logger::error(
                "io/flow/client", "connect to {} failed", m_socket.get_endpoint().to_string()
            );
            core::events::publish(
                "io.flow.client.connect_failed", {{"endpoint", m_socket.get_endpoint().to_string()}}
            );
            return;
        }
...
public:
     ...
     * socket off to `m_on_success` and moves it out of `m_socket`; on hard failure, just
     * reports it; otherwise signals the caller to keep pumping.
    ...
    ConnectResult handshake()
    {
        ...
        if (status.is_errored()) {
            ...
            core::events::publish(
                "io.connector.handshake_failed", {{"fd", std::to_string(get_fd())}}
            );
    ...
    /**
     * @brief Builds the work callback the controller runs each time this handler's turn comes
     * up — keeps pumping handshake() while IN_PROGRESS; on both SUCCESS and ERROR it releases
     * the handler instead of tearing down directly here. release() only flags/reschedules the
     ...
     * container owns it (e.g. `ServerFlowSocket::m_pending_connectors`), destroying `this` in
     * the process. No explicit socket close here: `~ConnectorSocket()`/`~Socket()` already
     * close the underlying fd/SSL state automatically as part of that destruction, same as any
     * other RAII teardown — see `Socket::~Socket()`. On SUCCESS, `m_socket` was already moved
     * out to `m_on_success` in handshake() (left in its moved-from, `INVALID_SOCKET` state), so
     * that destructor-time close is a documented no-op there too.
     ...
     * `ServerFlowSocket::m_pending_connectors.erase(fd)`), which destroys `this` as a side
     * effect and closes the socket via `~ConnectorSocket()`/`~Socket()`.
...
public:
     ...
     */
    WorkerSocket(
        socket::Socket<Protocol> socket,
        shared::ErrorCallback on_send_error,
        shared::ErrorCallback on_receive_error
    ) :
        m_socket{std::move(socket)},
        m_sender{m_socket, std::move(on_send_error)},
    ...
    {
        // Every callback's already wired via the init list — just validate/finalize both
        // halves.
        build();
    }
     ...
     * (erased after drain_senders() waited for its contracts to go idle) both halves are
     * already closed and their contracts released, so this just closes the fd for real.
     */
    ~WorkerSocket()
    {
        m_socket.sync_close();
    ...
    {
        m_receiver.add_on_read(std::move(on_read));
    }
     ...
     * @throws std::runtime_error if either the sender or receiver is missing a required
     * callback — see `Sender::build()`/`Receiver::build()`.
     */
    void build() &
    {
     ...
     * closed passes have run (armed slot released). drain_senders() polls this before
     * destroying the worker (which closes the socket).
     ...
     */
    Sender<socket::Socket<Protocol>, socket::SocketStatus>& get_sender()
    {
        return m_sender;
    }
...
// Wrapper that connects the base cocket to the worker and manages the types for the thread
// model.
...
public:
    ...
    ServerFlowSocket(socket::Endpoint end, Leverager& leverager, Controller& controller) :
        m_base_socket{std::move(end), leverager},
    ...
    ~ServerFlowSocket()
    {
        ...
        core::logger::debug(
            "io/flow", "closing base socket {}", m_base_socket.get_endpoint().to_string()
        );
    ...
    /**
     * @brief Copy ctor deleted — this thing owns live sockets and a worker map, no copying
     * that.
     */
    ...
    void build() &
    {
        ...
        if (!m_on_established) {
            throw std::runtime_error(
                "ConnectionEstablished callback must be set before building the "
                "ServerFlowSocket"
            );
        }
    ...
    {
        core::logger::debug(
            "io/flow", "base socket {} start", m_base_socket.get_endpoint().to_string()
        );
        m_base_socket_contract.emplace(m_base_socket.template create<Controller>(m_controller));
    }

    /**
     * @brief Stops accepting new connections — closes the listening socket, releases its
     * contract, and clears any connectors still mid-handshake.
     */
    void stop_accepting()
    {
        m_base_socket.set_closed();
     ...
     * @note No timeout, and MUST run while the contract pool is still scheduling, so those
     * final passes actually execute. Sockets are closed later in ~WorkerSocket(). Call after
     * GOAWAY and session drain.
     */
    void close() noexcept
    {
...
private:
     ...
     * `WorkerSocket`, invokes `m_on_established` to get the application read callback,
     * registers the worker against the controller, and stashes it in `m_workers`.
     * @warning The nested lambdas here capture `this`, `worker` (a `shared_ptr`), and reach
     * back into `m_workers`/`m_pending_connectors` from deep inside handshake/read/error
     * callbacks. If this `ServerFlowSocket` gets destroyed while a handshake or worker is still
     * mid-flight, those captured `this` pointers dangle — same class of footgun as the
     * raw-`this` captures in the async Sender/Receiver `arm_write()`/`arm_read()`.
     * `m_pending_connectors` (which owns every in-flight `ConnectorSocket` outright, see its
     * own doc) keeps the connector itself alive across the handshake, but doesn't protect this
     * `ServerFlowSocket` from an early death.
     */
    void helper()
    {
        ...
        m_base_socket.add_on_accept([this](socket::Socket<Protocol> accepted_socket) mutable {
            core::logger::debug(
                "io/flow", "accepted {} from {}", m_base_socket.get_endpoint().to_string(),
                accepted_socket.get_endpoint().to_string()
            );
                ...
                [this](socket::Socket<Protocol> encrypted_socket) mutable {
                    ...
                    auto worker = std::make_shared<WorkerSocket<Protocol>>(
                        ...
                        [this](socket::SOCKET socket_fd, int err) {
                            ...
                            core::logger::error(
                                "io/flow/server", "send error on socket {}: {} ({})", socket_fd,
                        ...
                        [this](socket::SOCKET socket_fd, int err) {
                            m_workers.erase(socket_fd);
                            core::logger::error(
                                "io/flow/server", "receive error on socket {}: {} ({})", socket_fd,
                                err, utils::ErrnoTranslator::describe_errno(err)
                            );
                            core::events::publish(
                    ...
                    core::logger::debug(
                        "io/flow", "fd {} connected under {}", worker->get_fd(),
            ...
            connector->set_on_released([this, ACCEPTED_FD]() {
                m_pending_connectors.erase(ACCEPTED_FD);
            });
     ...
     * automatically via `~ConnectorSocket()`/`~Socket()`, no explicit close call needed.
     * `clear()` runs those destructors through its own internal sweep (not our own iteration),
     * so there's no risk of the on-released erase-hook (only fired from the controller-driven
     * path, never from plain destruction) invalidating anything mid-loop. Without this, a
     * connection still mid-TLS-handshake when shutdown hits never gets a chance to run its
     * normal on_released() path either (nothing's left to schedule it there), leaking its
     * socket/SSL state — confirmed live under LeakSanitizer.
     */
    void close_pending_connectors()
    {
        m_pending_connectors.clear();
    }
...
public:
    ...
    /**
     * @brief Builds a ClientFlowSocket targeting `end` — doesn't actually connect yet, that's
     * on build()/helper().
     * @param end the endpoint to eventually connect to.
     ...
     */
    ClientFlowSocket(
        socket::Endpoint end, Leverager& leverager, Controller& controller, bool verify_peer = true
    ) :
        m_endpoint{std::move(end)},
        m_leverager{leverager},
        m_controller{controller},
        m_on_established{nullptr},
    ...
    /**
     * @brief Move ctor — steals the endpoint, leverager/controller refs, worker, and
     * established callback off `other`.
     * @param other the flow socket to move from.
     */
    ...
    /**
     * @brief Hands back a callback that sends through the live worker's sender, if there is
     * one.
     * @return a send callback bound to the worker's sender, or `nullptr` if no worker exists
     * yet.
     */
...
private:
     ...
     * `WorkerSocket`, invokes `m_on_established` for the application read callback, registers
     * the worker against the controller, and stores it in `m_worker`.
            ...
            std::move(socket), [this](socket::Socket<Protocol> encrypted_socket) mutable {
                ...
                auto worker = std::make_shared<WorkerSocket<Protocol>>(
                    ...
                    [this](socket::SOCKET socket_fd, int err) {
                        m_worker.reset();
                        core::logger::error(
                            "io/flow/server", "send error on socket {}: {} ({})", socket_fd, err,
                    ...
                    [this](socket::SOCKET socket_fd, int err) {
                        m_worker.reset();
                        core::logger::error(
                            "io/flow/server", "receive error on socket {}: {} ({})", socket_fd, err,
                            utils::ErrnoTranslator::describe_errno(err)
                        );
                        core::events::publish(
                            "io.flow.server.receive_error",
                ...
                core::logger::debug(
                    "io/flow", "fd {} connected under {}", worker->get_fd(), m_endpoint.to_string()
                );
...
// (not template-parameterized on a mockable leverager, unlike Receiver/Sender's Worker param)
// to take a real `Leverager&` (= `leverage::Leverager<leverage::Context>&`) in their
// constructors, and every one of them immediately opens a real socket / does real address
// resolution in that constructor (e.g. ServerBaseSocket's ctor: `socket::Socket<Protocol,
// true>{std::move(end), std::ref(leverager)}`). Constructing a `Leverager<Context>` requires a
// live io_uring ring — the exact same "not unit-testable in isolation" situation
// `io_base_leverage`'s own test block documents for `Leverager<Context>` itself, and the same
// reason `Flow` (io/flow/flow.cppm) skips runtime construction. There is no injection point to
// route around this (no mock leverager type accepted anywhere), so none of these four classes'
// constructors — and by extension none of their methods — can be exercised here. This was
// genuinely attempted: every constructor and method of all four was traced above before
// reaching this conclusion; it's not a blanket skip.
//
...
// accept→handshake→worker-construction `this`-captures (see helper()'s own @warning above):
// both are private, only reachable through a real, un-constructible `ServerFlowSocket`/
...
suite<"WorkerSocket (sync) construction/build"> worker_socket_ctor_suite = [] {
    ...
    "the two-error-callback ctor still throws: the receiver still needs on_read"_test = [] {
        WorkerSocket<socket::Protocol::TCP> worker{
            socket::Socket<socket::Protocol::TCP>{}, [](int, int) {}, [](int, int) {}
        };
...
suite<"WorkerSocket (sync) accessors/lifecycle"> worker_socket_lifecycle_suite = [] {
    ...
    // `m_sender_contract.emplace(m_sender.template create<Controller>(controller))` — the
    // target is the concrete `core::contract::Contract<>` type, not whatever the Controller's
    // create() happens to return, so a lightweight mock controller (the kind used in
    // shared/handler.cppm's own tests) doesn't type-check here: only a controller wired to a
    // real contract pool produces a `Contract<>`. That's a real scheduling subsystem, not
    // something this file's mocking can safely stand in for, so start() is left unexercised.
};
