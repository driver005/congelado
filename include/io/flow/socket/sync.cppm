...
class WorkerSocket {
  public:
    ...
    void close() {
        ...
        // Release the sender/receiver contracts so the controller stops scheduling them.
        if (m_sender_contract.has_value()) {
            m_sender_contract->release();
            m_sender_contract.reset();
        }
        if (m_receiver_contract.has_value()) {
            m_receiver_contract->release();
            m_receiver_contract.reset();
        }
...
class ServerFlowSocket {
  public:
    using ConnectionEstablishedCallback = std::move_only_function<shared::ReadCallback(
        shared::SendCallback, shared::CloseCallback)>;
    ...
    /**
     * @brief Waits until every live worker's sender queue is empty, then closes each worker.
     * @note No timeout — this blocks until the connections naturally go idle. Call after
     * the application layer has stopped producing new sends (e.g. after GOAWAY and session drain).
     */
