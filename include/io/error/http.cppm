...
public:
    /**
     * @brief Builds an Http2Exception tagging the RFC 9113 error code onto a plain
     ...
     */
    Http2Exception(Http2ErrorCode code, const std::string& msg) :
     ...
     */
    [[nodiscard]] Http2ErrorCode get_code() const
    {
        return m_error_code;
    }
...
public:
    /**
     * @brief Builds a StreamError — a per-stream HTTP/2 failure (RFC 9113 §5.4.2), scoped to
     * one stream instead of tanking the whole connection.
     ...
     */
    [[nodiscard]] std::uint32_t get_stream_id() const
    {
        return m_stream_id;
    }
...
public:
    ...
    ConnectionError(
        ...
        std::uint32_t last_stream_id = MAX_CONNECTED_STREAMS
    ) :
        Http2Exception{code, msg},
     ...
     */
    [[nodiscard]] std::uint32_t get_last_stream_id() const
    {
...
public:
    /**
     * @brief Builds an InvalidIndexError for an HPACK index that's either 0 or past the
     * combined static+dynamic table size — lowkey just straight out of bounds, no valid header
     * lives there.
     * @param index the offending index.
     */
    explicit InvalidIndexError(UInt index) :
        DecodeError{std::format("hpack: invalid index {}", index)},
        m_index{index}
    {
    }
...
public:
    /**
     * @brief Builds an EmptyNameError — fires when a literal header field representation shows
     * up with a zero-length name, which HPACK doesn't allow.
     */
    EmptyNameError() :
        DecodeError{"hpack: literal header field has empty name"}
    {
    }
};
...
public:
    /**
     * @brief Builds a TruncatedDataError — the header block ran out of bytes mid-decode, cut
     * off before a complete field could be read.
     */
    TruncatedDataError() :
        DecodeError{"hpack: unexpected end of header block"}
    {
    }
};
