...
class Atom {
  ...
  public:
    ...
    static void write_big_endian(Out &out, UInt val) {
        constexpr std::size_t BYTES = sizeof(UInt);
        // Delegate the actual byte-packing to the range overload, aimed at a counted view of `out`.
    ...
    /**
     * @brief Iterator-based varint write — same bucket-picking as the range overload, `out` gets
     * advanced past however many bytes the encoding actually took.
     * @tparam Out a writable byte iterator satisfying `shared::ByteIteratorWriter`.
    ...
    static std::pair<UInt, std::uint8_t> read_varint(R &&data) {
        ...
        // View data as plain uint8_t so the bit ops below don't need std::to_integer everywhere.
        auto view = std::forward<R>(data) |
                    std::views::transform([](std::byte byte) { return std::to_integer<std::uint8_t>(byte); });

        // First byte's top 2 bits pick the length bucket; the low 6 bits are value's first chunk.
  ...
  private:
    ...
    /**
     * @brief Packs `val` into `range` given an already-picked `prefix`/`length` bucket — this is
     * where the actual byte-shuffling happens for write_varint(), no cap.
     * @tparam R a writable byte range satisfying `shared::ByteRangeWriter`.
    ...
    static void write_varint_helper(R &&range, UInt val, std::uint8_t prefix, std::size_t length) {
        ...
        *std::ranges::begin(range) = static_cast<std::byte>((val >> (8 * (length - 1))) | (prefix << 6));
        ...
        for (auto [i, b] : std::forward<R>(range) | std::views::drop(1) | std::views::take(length - 1) |
                               std::views::enumerate) {
