...
class ServerFlow {
  public:
     ...
     * the transport wired up at accept time (which is what actually closes the underlying
     * socket).
     ...
     * still-incomplete preface: it bails quietly and waits for more bytes next call, no
        ...
        return [this](utils::buffering::BufferReader &view) {
            ...
            if (!m_handshake_completed) {
                ...
                if (result == HandshakeState::COMPLETED) {
                    ...
                    // Bad preface — kill the connection and don't touch whatever's left in view.
