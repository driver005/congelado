...
public:
    ...
    ServerFlow(
        ...
        m_session{
            std::move(send), std::move(close), extension_registry, Role::SERVER, std::move(dispatch)
        },
    ...
    /**
     * @brief Tears this connection's session down — sends GOAWAY and, unless `graceful`,
     * invokes the close callback the transport wired up at accept time (which is what actually
     * closes the underlying socket). Graceful leaves the socket open so the sender can flush
     * first.
     ...
     */
    void close(
        error::http::Http2ErrorCode code = error::http::Http2ErrorCode::NO_ERROR_CODE,
        bool graceful = false
    )
    {
        if (m_closed) {
        ...
        // Ask the frame executor to drain any final frames, then release its own contract once
        // the session's fully idle — instead of yanking it out immediately. is_idle() waits on
        // it.
        ...
        return [this](utils::buffering::BufferReader& view) {
            ...
            if (!m_handshake_completed) {
                ...
                if (result == HandshakeState::COMPLETED) {
                    ...
                    // Bad preface — L, kill the connection and don't touch whatever's left in
                    // view.
                    ...
                    // Still incomplete, just need more bytes — bail quietly and wait for next
                    // call.
                    return;
                }
            }
            ...
            if (!view.empty()) {
                ...
                // WIREDUMP (temporary): dump inbound bytes exactly as they arrived off the
                // wire.
                {
                    std::string hex;
                    std::size_t n = 0;
                    ...
                    core::logger::warning(
                        "WIREDUMP", "engine-inbound size={} bytes[{}]: {}", view.size(), n, hex
                    );
                }
...
public:
    ...
    ClientFlow(
        ...
        m_session{
            std::move(on_send), std::move(close), extension_registry, Role::CLIENT,
            std::move(dispatch)
        },
        m_executor{m_session},
        m_handshake{m_session.get_local_settings(), [this](utils::buffering::BufferNode&& node) {
    ...
    {
        // Same self-rescheduling frame executor as ServerFlow — the socket read callback (built
        // in on_connect()) hands response bytes to it instead of parsing inline.
        m_executor_contract.emplace(
            m_executor.create(group, core::contract::ContractState::SCHEDULED)
        );
    }

    /// @brief Releases the frame executor's contract so it stops rescheduling before this flow
    /// (and the executor it owns) is torn down.
    ~ClientFlow()
    {
    ...
    OnConnectCallback on_connect()
    {
        return [this]() {
            ...
            // Client sends the preface, no waiting on the peer to confirm — synchronous and
            // done.
...
// NOTE on scope: none of these tests ever call
// `core::contract::ContractGroup::process_next_contract()` — the frame executor's SCHEDULED
// contract created in each flow's ctor is therefore observed as "not drained" (`is_idle()` false)
// throughout. Actually running the contract to drain it would exercise
// `ContractGroup::schedule()`'s "already scheduled" guard, which `core::logger::fatal()`s straight
// into `std::abort()` on a mismatch — not exception-safe, so a wrong assumption there would take
// down the whole test binary instead of just failing one assertion. `ContractGroup`'s own module
// already covers that scheduling machinery in isolation; these tests stick to what
...
suite<"ServerFlow"> server_flow_suite = [] {
    "ctor wires up the session/handshake/executor; the fresh executor contract keeps is_idle() false"_test =
        [] {
            ...
            // The executor contract is created SCHEDULED and never driven here, so it never
            // reaches released/idle — is_idle() requires both the session AND the executor
            // contract to be done, so this is false even though the session itself has zero
            // active streams.
            expect(not flow.is_idle());
            expect(close_calls == 0);
        };

    "close() (default, hard) sends GOAWAY(stream=0) and invokes the close callback exactly "
    "once, "
    "and is idempotent"_test = [] {
        ...
        auto header = captured | ReadFrameHeaderAdaptor{16'384U};
...
suite<"ClientFlow"> client_flow_suite = [] {
    ...
    "dtor releases the executor contract without touching the (still-scheduled, never-driven) tree"_test =
        [] {
            ...
            // Flow destructed above; reaching this line at all is the assertion — release() on
            // an already-scheduled contract only flips a flag, it never re-touches the signal
            // tree (see ContractGroup::release()'s `if (!is_scheduled())` guard), so this is
            // safe without a scheduler running.
            expect(true);
        };
    ...
    "on_connect() completes the client handshake (SETTINGS+preface sent) and returns a working "
    "read "
    "callback"_test = [] {
        core::contract::ContractGroup<> group;
        HttpExtensionRegistry registry;
        ...
        // The steady-state read callback hands everything off to the executor and consumes the
        // view.
        expect(reader.empty());
    };
    ...
    "sender() hands the request to the session and returns the assigned (odd) client stream id"_test =
        [] {
            ...
            // documented pre-existing caveat) — guarded with nothrow rather than exact wire
            // bytes.
            std::uint32_t sid = 0;
            expect(nothrow([&] {
                sid = flow.sender(req);
