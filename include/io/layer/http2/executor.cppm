 ...
 *
 * The socket receiver hands newly-read bytes off via `feed()` and returns immediately (same
 * spirit as the http2 dispatch handing requests to `core::router::RouterExecutor`). This
 * handler — registered as its own `core::contract` worker and **infinitely self-rescheduling**
 * like the socket sender/receiver contracts — parses **one** frame off `m_handoff` per turn via
 ...
 * single write left the trailing frame stranded: `Session::receive()` parses one frame per
 * call, and the plain receiver only calls it when new bytes arrive — so a complete-but-unparsed
 * frame would sit forever with no further read to trigger it. This contract reschedules
 * regardless of reads, so the leftover is parsed on the next turn — no dependency on a future
 * read.
 *
 * ## Why parsing is serial per connection
 * Frame demux is inherently sequential: you must read a frame's 9-byte header to learn its
 * length before the next frame boundary is known — no reading ahead. HPACK's dynamic table and
 * the connection-level flow-control window are ordered, shared state too. So one executor per
 * connection does all framing. Parallelism comes from (a) many connections, each with its own
 * executor, and (b) request handlers, which `core::router::RouterExecutor` runs on the worker
 * pool once a request has been demuxed and handed off.
 *
 * ## Buffer & guard
...
public:
     ...
     * @param session the owning flow's `Session`; must outlive this executor (it's the flow's
     * own member, sitting right next to this one).
     */
    ...
    /**
     * @brief The per-tick work: resume() and either reschedule or release depending on the
     * outcome — same release-on-done pattern as the sync Sender/Receiver.
     ...
     * `m_resume` (same re-entrancy pattern as the sync Sender/Receiver's resume()). Once a
     * close has been requested and everything's drained — buffer empty and the session has no
     * active streams — returns false so the caller releases this contract; otherwise returns
     * true to keep going.
     * @note One `receive()` per turn is enough: the contract reschedules unconditionally (see
     * on_execute()), so buffered-but-not-yet-parsed frames get picked up on the next turn
     * without depending on a socket read to trigger them.
     * @return false when closed and fully drained (release), true otherwise (keep
     * rescheduling).
     */
    bool resume()
    {
        ...
        if (!get_resume()) {
            ...
            if (!reader.empty()) {
                // WIREDUMP (temporary): dump what the parser is about to read off m_handoff.
                {
                    ...
                    core::logger::warning(
                        "WIREDUMP", "engine-preparse size={} bytes[{}]: {}", reader.size(), n, hex
                    );
                }
     ...
     * flight) and only releases its own contract once everything's drained and the session has
     * no active streams left — so a final response/GOAWAY still gets parsed before this handler
     * goes idle. The owning flow polls the contract (see `is_idle()` there) to know when that
     * happened.
     */
...
private:
    /// @brief Reads the guard (acquire ordering). Pairs with set_resume()'s release store —
    /// same approach as the sync Receiver/Sender's get_stalled()/set_stalled().
...
suite<"SessionExecutor"> session_executor_suite = [] {
    ...
    "feed() hands bytes to the executor, and resume() parses a complete frame off them"_test = [] {
        ...
        // A non-ACK PING at the connection level — Session::receive() should reply with a
        // PING ACK carrying the same 8-byte payload, driven entirely through
        // feed()+resume().
    ...
    "mark_close() releases (resume() returns false) once drained and the session is idle"_test =
        [] {
            ...
            executor.mark_close();
            // Nothing was ever fed and the session never opened a stream — closing + empty +
            // idle.
    ...
    "mark_close() with bytes still queued keeps draining (returns true) until they're parsed"_test =
        [] {
            ...
            expect(executor.resume());
            // Drained now, closing, and the session never opened a stream — next resume()
            // releases.
            expect(not executor.resume());
        };
};
