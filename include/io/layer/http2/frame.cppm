...
class FrameHeader {
  public:
    ...
    void validate_payload_size(std::size_t actual_size) const {
        if (actual_size != m_length) {
            throw error::http::ConnectionError(
                ...
                std::format("Payload size mismatch for frame type {}: expected {}, got {}", std::to_underlying(m_type),
                            m_length, actual_size),
                get_stream_id());
        }
    }
    ...
    [[nodiscard]] bool is_end_stream() const noexcept { return (m_flags & shared_layer::Flags::END_STREAM) != 0; }
    [[nodiscard]] bool is_padded() const noexcept { return (m_flags & shared_layer::Flags::PADDED) != 0; }
  ...
  private:
    ...
    void validate_push_promise() const {
        if constexpr (Role == shared_layer::FrameRole::RECEIVER) {
            ...
            if ((m_flags & ~(shared_layer::Flags::END_HEADERS | shared_layer::Flags::PADDED)) != 0) {
                throw error::http::ConnectionError(error::http::Http2ErrorCode::PROTOCOL_ERROR,
...
struct ReadWindowIncrementAdaptor : std::ranges::range_adaptor_closure<ReadWindowIncrementAdaptor> {
    ...
    std::uint32_t operator()(R &&range) const {
        std::uint32_t inc =
            (std::forward<R>(range) | std::views::take(4) | utils::codec::ReadBigEndianAdaptor{}) & 0x7FFFFFFF;
