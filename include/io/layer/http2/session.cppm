...
class Session {
  public:
     ...
     * then invokes the close callback.
  ...
  private:
    ...
    void send_response(std::uint32_t stream_id) {
        ...
        // The response is fully framed and enqueued; this stream is done from our side.
        // Null its entry so `is_idle()` can tell when every connection has finished.
