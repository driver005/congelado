...
inline int closesocket(SOCKET socket_descriptor)
{
...
inline void set_non_blocking_impl(SOCKET socket, bool non_blocking)
{
    ...
    if (fcntl(socket, F_SETFL, flags) < 0) { // NOLINT(cppcoreguidelines-pro-type-vararg) — fcntl is
                                             // a POSIX vararg API, no safe C++ alternative
