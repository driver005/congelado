...
using LPFN_CONNECTEX = BOOL(PASCAL FAR*)(
    SOCKET s,
...
public:
    ...
    explicit Context(int entries) :
        ...
        m_pending_ops{0},
        m_connect_ex_ptr{nullptr},
        m_accept_ex_ptr{nullptr}
    {
        init(entries);
    };
    ...
    void init(int entries)
    {
        ...
        m_iocp_handle =
            CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, 0, static_cast<DWORD>(entries));
        if (!m_iocp_handle) {
    ...
    /** @brief Closes `m_iocp_handle` if it's live and nulls it out — safe to call more than
     * once. */
    ...
    /**
     * @brief Stashes `op`'s address in its own `OVERLAPPED.hEvent` (so the completion handler
     * can find its way back to the `pending_op`) and hands ownership off to the OS.
     * @warning `op.release()` deliberately leaks the smart pointer — the raw `pending_op *`
     * only gets reclaimed with `delete` inside process_completions() once the matching
     * completion packet shows up. If the op never completes, that allocation leaks. Same
     * footgun shape as the posix backend's `submit_async`.
    ...
    /**
     * @brief Pulls completion packets off the IOCP queue in a loop, reclaiming each
     * `pending_op` and firing its callback with the result.
    ...
    /**
     * @brief Grabs the raw IOCP handle for
     * `CreateIoCompletionPort`/`PostQueuedCompletionStatus` calls elsewhere.
     * @return the completion port handle.
     */
    [[nodiscard]] HANDLE get_iocp_handle() const noexcept
    {
     ...
     */
    [[nodiscard]] LPFN_CONNECTEX get_connect_ex() const noexcept
    {
        return m_connect_ex_ptr;
    }
     ...
     */
    [[nodiscard]] LPFN_ACCEPTEX get_accept_ex() const noexcept
    {
        return m_accept_ex_ptr;
    }

    /**
     * @brief Reinterprets a posix-style int fd as a Windows `HANDLE` — pure bit-cast, no real
     * conversion happening (this codebase stores real `HANDLE`s in an `int`-sized slot
     * upstream).
    ...
    {
        return reinterpret_cast<HANDLE>(static_cast<std::uintptr_t>(fd));
    };
...
private:
    ...
    /**
     * @brief Lazily runs `WSAStartup` exactly once process-wide, guarded by
     * `m_wsa_initialized`.
     ...
     * leaves `m_connect_ex_ptr`/`m_accept_ex_ptr` null if either `WSAIoctl` call fails — no
     * error surfaced here, so a broken lookup only shows up later when
     * async_connect()/async_accept() dereference a null function pointer. Sus failure mode to
     * debug blind.
     */
 ...
 */
template<>
void Leverager<Context>::async_read(
    int fd,
    void* buf,
    unsigned nbytes,
    off_t offset,
 ...
 */
template<>
void Leverager<Context>::async_write(
    int fd,
    const void* buf,
    unsigned nbytes,
    off_t offset,
 ...
 */
template<>
void Leverager<Context>::async_readv(
    int fd,
    const iovec* iovecs,
    unsigned nr_vecs,
    off_t offset,
 ...
 */
template<>
void Leverager<Context>::async_writev(
    int fd,
    const iovec* iovecs,
    unsigned nr_vecs,
    off_t offset,
 ...
 */
template<>
void Leverager<Context>::async_readv2(
    int fd,
    const iovec* iovecs,
    unsigned nr_vecs,
    off_t offset,
 ...
 */
template<>
void Leverager<Context>::async_writev2(
    int fd,
    const iovec* iovecs,
    unsigned nr_vecs,
    off_t offset,
 ...
 */
template<>
void Leverager<Context>::async_read_fixed(
    int fd,
    void* buf,
    unsigned nbytes,
    off_t offset,
    [[maybe_unused]] int buf_index,
    completion_callback cb,
    std::uint8_t iflags
)
{
 ...
 */
template<>
void Leverager<Context>::async_write_fixed(
    int fd,
    const void* buf,
    unsigned nbytes,
    off_t offset,
 ...
 * and fires `cb` inline before returning; nothing actually goes through the completion port
 * here.
 ...
 */
template<>
void Leverager<Context>::async_fsync(
    int fd,
    [[maybe_unused]] unsigned fsync_flags,
    completion_callback cb,
    [[maybe_unused]] std::uint8_t iflags
)
{
...
/**
 * @brief Partial-range sync shim — Windows has no partial flush primitive, so this just
 * forwards to a full async_fsync(), dropping the range/flag params entirely.
 * @param fd descriptor to flush.
 ...
 */
template<>
void Leverager<Context>::async_sync_file_range(
    int fd,
    [[maybe_unused]] off64_t offset,
    [[maybe_unused]] off64_t nbytes,
    [[maybe_unused]] unsigned sync_range_flags,
    completion_callback cb,
    std::uint8_t iflags
)
{
 ...
 * @param cb completion callback, invoked with the syscall result (or negative
 * `WSAGetLastError`).
 * @param iflags unused on this backend.
 */
template<>
void Leverager<Context>::async_recv(
    int sockfd,
    void* buf,
    unsigned nbytes,
    std::uint32_t flags,
...
)
{
    ...
    int result =
        WSARecv(op->socket, &op->wsabuf, 1, &bytes_received, &wsa_flags, &op->overlapped, nullptr);
 ...
 */
template<>
void Leverager<Context>::async_send(
    int sockfd,
    const void* buf,
    unsigned nbytes,
    std::uint32_t flags,
 ...
 */
template<>
void Leverager<Context>::async_poll(
    [[maybe_unused]] int fd,
    [[maybe_unused]] short poll_mask,
    completion_callback cb,
    [[maybe_unused]] std::uint8_t iflags
)
{
 ...
 * @warning Actual bug, not just a footgun: `op->callback = std::move(cb)` moves `cb` out on
 * line one, but the `PostQueuedCompletionStatus` failure branch then calls the now-moved-from
 * `cb` directly instead of `op->callback`. Invoking a moved-from `std::move_only_function`
 * throws `std::bad_function_call` instead of running the intended error callback — that's an L
 * on the failure path specifically, the success path (going through `op.release()` +
...
{
    // no cap, this is just a round-trip: stash the callback, then post a zero-byte packet
    // through IOCP
    auto op = std::make_unique<pending_op>();
    op->callback = std::move(cb);
 ...
 */
template<>
void Leverager<Context>::async_accept(
    int fd,
    [[maybe_unused]] sockaddr* addr,
...
)
{
    ...
    // lowkey annoying but AcceptEx needs a pre-created socket to accept into, unlike posix
    // accept()
...
/**
 * @brief Overlapped `ConnectEx` shim — `ConnectEx` requires the socket be locally bound first,
 * so this does an explicit zero-port `bind()` before kicking off the connect.
 ...
 */
template<>
void Leverager<Context>::async_connect(
    int fd,
    sockaddr* addr,
    socklen_t addrlen,
    completion_callback cb,
...
)
{
    ...
    DWORD bytes_sent;
    BOOL result =
        m_context.get_connect_ex()(sock, addr, addrlen, nullptr, 0, &bytes_sent, &op->overlapped);
...
)
{
    ...
    due_time.QuadPart = -(ts->tv_sec * 10'000'000LL + ts->tv_nsec / 100);
 ...
 * @note `dfd` (the `AT_FDCWD`-relative directory fd) is unused — `path` is always resolved
 * as-is, no `*at`-style relative-to-directory-handle behavior on this backend.
 ...
 */
template<>
void Leverager<Context>::async_openat(
    [[maybe_unused]] int dfd,
    const char* path,
    int flags,
 ...
 * `statx` struct.
 * @warning Only fills `stx_mask`, `stx_size`, and `stx_mode` (with a hardcoded
 * 0755-dir/0644-file permission guess) — every other `statx` field (inode, timestamps, link
 * count, etc.) stays zeroed regardless of `mask`. Don't read anything but those three fields
 * out of `statxbuf` on this backend, the rest is silently unpopulated, not an error.
 ...
 */
template<>
void Leverager<Context>::async_statx(
    [[maybe_unused]] int dfd,
    const char* path,
    [[maybe_unused]] int flags,
    unsigned mask,
...
)
{
    ...
    if (statxbuf) {
        ...
        statxbuf->stx_size =
            (static_cast<std::uint64_t>(attr_data.nFileSizeHigh) << 32) | attr_data.nFileSizeLow;
        statxbuf->stx_mode =
            (attr_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? 040755 : 0100644;
    }
 ...
 * fails every call with `cb(-ERROR_NOT_SUPPORTED)`. All params are dropped
 * (`[[maybe_unused]]`).
 ...
 */
template<>
void Leverager<Context>::async_splice(
    [[maybe_unused]] int fd_in,
    [[maybe_unused]] loff_t off_in,
    [[maybe_unused]] int fd_out,
    [[maybe_unused]] loff_t off_out,
 ...
 */
template<>
void Leverager<Context>::async_tee(
    [[maybe_unused]] int fd_in,
    [[maybe_unused]] int fd_out,
    [[maybe_unused]] size_t nbytes,
    [[maybe_unused]] unsigned flags,
 ...
 * @warning Odd flag mapping: this passes `MOVEFILE_REPLACE_EXISTING` when `RENAME_NOREPLACE`
 * *is* set — that reads backwards from what the flag name implies (posix `RENAME_NOREPLACE`
 * means "fail if the destination exists," which is the opposite of "replace existing"). Worth
 ...
 */
template<>
void Leverager<Context>::async_renameat(
    [[maybe_unused]] int olddfd,
    const char* oldpath,
    [[maybe_unused]] int newdfd,
    const char* newpath,
    unsigned flags,
    completion_callback cb,
    [[maybe_unused]] std::uint8_t iflags
...
{
    BOOL result =
        MoveFileExA(oldpath, newpath, (flags & RENAME_NOREPLACE) ? MOVEFILE_REPLACE_EXISTING : 0);
    cb(result ? 0 : -static_cast<int>(GetLastError()));
}
 ...
 */
template<>
void Leverager<Context>::async_mkdirat(
    [[maybe_unused]] int dirfd,
    const char* pathname,
    [[maybe_unused]] mode_t mode,
    completion_callback cb,
    [[maybe_unused]] std::uint8_t iflags
...
/**
 * @brief Symlink shim — synchronous `CreateSymbolicLinkA` targeting a file (flag `0`, i.e. not
 * a directory symlink).
 * @warning Always creates a *file*-type symlink (flags `0`) — pointing this at a directory
 * target will make a broken/wrong-type link on Windows, which distinguishes file vs. directory
 * symlinks unlike posix. No detection/handling of that case here.
 ...
 */
template<>
void Leverager<Context>::async_symlinkat(
    const char* target,
    [[maybe_unused]] int newdirfd,
    const char* linkpath,
    completion_callback cb,
    [[maybe_unused]] std::uint8_t iflags
)
 ...
 */
template<>
void Leverager<Context>::async_linkat(
    [[maybe_unused]] int olddirfd,
    const char* oldpath,
    [[maybe_unused]] int newdirfd,
    const char* newpath,
    [[maybe_unused]] int flags,
 ...
 */
template<>
void Leverager<Context>::async_unlinkat(
    [[maybe_unused]] int dfd,
    const char* path,
    unsigned flags,
    completion_callback cb,
 ...
 */
template<>
void Leverager<Context>::async_msg_ring(
    [[maybe_unused]] int fd,
    [[maybe_unused]] unsigned len,
    [[maybe_unused]] std::uint64_t data,
    [[maybe_unused]] unsigned flags,
    completion_callback cb,
    [[maybe_unused]] std::uint8_t iflags
)
 ...
 * accept doesn't land within a single run_once() pass, plus async_accept()'s use-after-move
 * `cb` bug on its own failure paths.
 * @param fd listening socket descriptor.
