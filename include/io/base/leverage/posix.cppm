...
public:
     ...
     * the ctor — you shouldn't need to call this yourself unless you're doing something
     * unusual.
     * @param entries submission queue depth to reserve.
    ...
    liburing::io_uring_sqe* get_sqe_safe()
    {
        ...
        // second attempt — if it's still null the ring is genuinely out of memory, straight
        // cooked
     ...
     * @param count the new count to set — mostly here for tests/edge cases, normal flow just
     * lets process_completions() manage this itself.
     */
 ...
 * since `Context::~Context()` is an empty no-op — see its own note). Don't copy this pattern
 * onto a `Context` with a non-trivial dtor without re-checking that equivalence.
 */
template<>
...
/**
 * @brief io_uring specialization of read() — preps via `io_uring_prep_read`. Full param
 * contract lives on the primary declaration in types.cppm.
 */
template<>
...
void Leverager<Context>::fsync(
    int file_descriptor, unsigned fsync_flags, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
/**
 * @brief io_uring specialization of sync_file_range() — preps via the generic
 * `io_uring_prep_rw` entry point using `liburing::OP_SYNC_FILE_RANGE`, then patches
 * `sync_range_flags` onto the SQE directly since there's no dedicated
 * `io_uring_prep_sync_file_range` helper here. Full param contract lives on the primary
 * declaration in types.cppm.
 */
template<>
...
)
{
    ...
    liburing::io_uring_prep_rw(
        liburing::OP_SYNC_FILE_RANGE, sqe, file_descriptor, nullptr, nbytes, offset
    );
...
void Leverager<Context>::recvmsg(
    int sockfd, msghdr* msg, std::uint32_t flags, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
/**
 * @brief io_uring specialization of recv() — preps via `io_uring_prep_recv`. Full param
 * contract lives on the primary declaration in types.cppm.
 */
template<>
...
/**
 * @brief io_uring specialization of send() — preps via `io_uring_prep_send`. Full param
 * contract lives on the primary declaration in types.cppm.
 */
template<>
...
/**
 * @brief io_uring specialization of poll() — preps via `io_uring_prep_poll_add`, a real
 * readiness watch (unlike the win32 backend's stub). Full param contract lives on the primary
 * declaration in types.cppm.
 */
template<>
void Leverager<Context>::poll(
    int file_descriptor, short poll_mask, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
/**
 * @brief io_uring specialization of yield() — preps via `io_uring_prep_nop`. Full param
 * contract lives on the primary declaration in types.cppm.
 */
template<>
...
void Leverager<Context>::timeout(
    __kernel_timespec* timeout_spec, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
void Leverager<Context>::close(
    int file_descriptor, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
void Leverager<Context>::shutdown(
    int file_descriptor, int how, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
void Leverager<Context>::mkdirat(
    int dirfd, const char* pathname, mode_t mode, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
void Leverager<Context>::unlinkat(
    int dfd, const char* path, unsigned flags, completion_callback callback, std::uint8_t iflags
)
{
    auto* sqe = m_context.get_sqe_safe();
...
void Leverager<Context>::register_files(std::span<const int> fds)
{
    int ret = liburing::io_uring_register_files(
        m_context.get_ring(), fds.data(), static_cast<unsigned>(fds.size())
    );
...
void Leverager<Context>::register_files_update(unsigned off, std::span<int> files)
{
    int ret = liburing::io_uring_register_files_update(
        m_context.get_ring(), off, files.data(), static_cast<unsigned>(files.size())
    );
...
void Leverager<Context>::register_buffers(std::span<const iovec> iovecs)
{
    int ret = liburing::io_uring_register_buffers(
        m_context.get_ring(), iovecs.data(), static_cast<unsigned>(iovecs.size())
    );
...
/**
 * @brief io_uring specialization of unregister_file() — forwards to register_files_update()
 * with a single `-1` sentinel to clear the slot.
 */
...
// meaningful — not unit-testable in isolation, skipped here. PendingOp is a plain value type
// with no syscalls involved, so that's covered for real. Context::submit_async() is the one
// exception: it's `static` and only ever touches the `io_uring_sqe*` argument it's handed
// (stamps flags, calls `io_uring_sqe_set_data()`) — both real liburing calls, but pure
// struct-field writes with no ring/kernel interaction (verified against
// /usr/include/liburing.h: `io_uring_sqe_set_data()` is just `sqe->user_data = (unsigned
// long)data`). A stack-allocated `io_uring_sqe{}` is enough to exercise it for real, no live
// ring required.
...
suite<"PendingOp"> pending_op_suite = [] {
    ...
    "stashed callback fires with the value it's invoked with"_test = [] {
        PendingOp op;
        int seen = 0;
        op.set_callback([&seen](int result) {
            seen = result;
        });
...
suite<"Context::submit_async"> submit_async_suite = [] {
    "stamps the requested SQE flags onto the entry"_test = [] {
        ...
        delete reinterpret_cast<PendingOp*>(
            static_cast<std::uintptr_t>(sqe.user_data)
        ); // NOLINT(cppcoreguidelines-owning-memory) — reclaiming what submit_async()
           // deliberately leaked into user_data, see below
    };

    "hangs a heap PendingOp carrying the callback off user_data, unconditionally"_test = [] {
        liburing::io_uring_sqe sqe{};
        int seen = -1;
        Context::submit_async(
            &sqe,
            [&seen](int result) {
                seen = result;
            },
        ...
        // user_data now holds the raw pointer submit_async() released ownership of — exactly
        // the handoff process_completions() expects to reclaim later via
    ...
    // Regression/design-gap marker, NOT a fix (matches submit_async()'s own @warning): this
    // test proves the missing "reject submissions once the ring is tearing down" guard
    // structurally, through the function's own signature, rather than by actually tearing down
    // a live ring mid-flight (unsafe — real ops could still be in flight against kernel
    // memory). submit_async() is `static` and takes no `Context&`/`this` at all — its only
    // inputs are the caller-prepped `io_uring_sqe*`, the callback, and the iflags byte. A
    // function that cannot observe `this` cannot consult any instance state (a "ring is
    // shutting down" flag, an op counter, anything) to decide whether to refuse the submission
    // — there is structurally nowhere for such a guard to live short of adding a new parameter,
    // which is exactly what's missing. Every call unconditionally heap-allocates and hands off
    // ownership, confirmed by the two tests above.
