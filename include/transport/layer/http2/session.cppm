export module layer_http2:session;
...
class Session {
  public:
    explicit Session(tls::basic::Connection &&conn)
  ...
  private:
    ...
    bool receive_exact(std::span<std::byte> target) {
        ...
        while (offset < target.size()) {
            ...
            std::size_t received = m_conn.recv(target.subspan(offset));
