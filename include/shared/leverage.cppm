...
concept AsyncExecutor =
    ...
    requires(E &executor, int descriptor, const iovec *iovecs, unsigned count, off_t offset, CompletionCallback callback, IFlags iflags) {
        { executor.readv(descriptor, iovecs, count, offset, std::move(callback), iflags) } -> std::same_as<void>;
        { executor.writev(descriptor, iovecs, count, offset, std::move(callback), iflags) } -> std::same_as<void>;
    } &&
    requires(E &executor, int descriptor, void *buf, const void *cbuf, unsigned nbytes, off_t offset, CompletionCallback callback,
             IFlags iflags) {
        { executor.read(descriptor, buf, nbytes, offset, std::move(callback), iflags) } -> std::same_as<void>;
        { executor.write(descriptor, cbuf, nbytes, offset, std::move(callback), iflags) } -> std::same_as<void>;
    } &&
    requires(E &executor, int descriptor, void *buf, const void *cbuf, unsigned nbytes, off_t offset, int buf_index,
        ...
        { executor.read_fixed(descriptor, buf, nbytes, offset, buf_index, std::move(callback), iflags) } -> std::same_as<void>;
        { executor.write_fixed(descriptor, cbuf, nbytes, offset, buf_index, std::move(callback), iflags) } -> std::same_as<void>;
    } &&
    requires(E &executor, int descriptor, sockaddr *addr, socklen_t *addrlen, int flags, CompletionCallback callback, IFlags iflags) {
        { executor.accept(descriptor, addr, addrlen, flags, std::move(callback), iflags) } -> std::same_as<void>;
        { executor.connect(descriptor, addr, *addrlen, std::move(callback), iflags) } -> std::same_as<void>;
    } &&
    requires(E &executor, int descriptor, void *buf, const void *cbuf, unsigned nbytes, int flags, CompletionCallback callback,
             IFlags iflags) {
        { executor.recv(descriptor, buf, nbytes, flags, std::move(callback), iflags) } -> std::same_as<void>;
        { executor.send(descriptor, cbuf, nbytes, flags, std::move(callback), iflags) } -> std::same_as<void>;
    } &&
    requires(E &executor, int dfd, const char *path, int flags, mode_t mode, CompletionCallback callback, IFlags iflags) {
        { executor.openat(dfd, path, flags, mode, std::move(callback), iflags) } -> std::same_as<void>;
