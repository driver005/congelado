...
struct Entry
{
    ...
    // NOTE: used by compiler-generated get<I> for structured bindings, not intended for direct
    // use.
    /**
 ...
 * overload largely redundant — it's not actually the one doing the work for `auto [k, v] =
 * entry`. Leaving the structure alone, this pass is comment-only.
 * @tparam I the tuple-like index (0 for key, 1 for value).
...
public:
    ...
    class IteratorBase
    {
        ...
        /**
         * @brief Skips forward over empty/deleted/sentinel control bytes until it lands on a
         * live slot (or runs off the end of the table).
         */
        void advance()
        {
    ...
    public:
        ...
        /**
         * @brief Builds an iterator at slot `idx` in `map`, then immediately advance()s past
         * any dead slots sitting right there.
     ...
     * slots with the same key, which is a straight L for anything that assumes uniqueness
     * (find() will just return whichever one probing happens to hit first).
     ...
     * `KeyEqual{}(slot(s).key(), args...)` — whatever you pass here has to produce the exact
     * same hash as whatever `K` value was actually inserted, or you're silently missing entries
     * that are sitting right there in the table. No cap, heterogeneous lookup is powerful but
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-missing-std-forward) — args is read multiple times (once
    // per probed slot); forwarding would use-after-move on the second probe
    [[nodiscard]] std::optional<V> find(Args&&... args) const
    {
        ...
        for (std::size_t probe = 0; probe < (m_capacity / GROUP_WIDTH); ++probe) {
            ...
            // Hit an empty slot in this group — open addressing says the key can't be any
            // further along the probe sequence, so bail early instead of scanning the whole
            // table, no cap.
    ...
    /**
     * @brief Removes the entry matching `args`, if any — runs the entry's destructor in place
     * and tombstones the slot with `DELETED` (open-addressing tables can't just zero the
     * control byte, that'd break probing for every other key that hashed into the same group).
     * @tparam Args the heterogeneous lookup key argument pack.
    ...
    void erase(Args&&... args)
    {
        ...
        // the slot instead of zeroing it, so probing for every OTHER key sharing this group
        // still works right.
        for (std::size_t probe = 0; probe < (m_capacity / GROUP_WIDTH); ++probe) {
            std::size_t cur = (grp + probe) % (m_capacity / GROUP_WIDTH);
            std::uint32_t match = match_byte(cur, fp);
    ...
    void clear()
    {
        ...
        if (m_capacity > 0) {
            m_control.assign(
                m_capacity + GROUP_WIDTH, static_cast<std::uint8_t>(ControlByte::EMPTY)
            );
        }
        m_size = 0;
    }
...
private:
     ...
     * sentinel and you get a reference to an Entry that was never constructed there.
     * `std::launder` makes the reinterpret legal from the object-model's perspective, it does
     * NOT mean there's actually a live object underneath. Caller's on the hook for only calling
     * this on slots the control bytes say are live.
     ...
     * @note `std::vector<RawSlot>`'s own destructor only frees raw bytes — it has no idea
     * `Entry` objects are living inside them, so skipping this step would leak every live
     * `K`/`V` pair (file handles, heap allocations, whatever they own) without ever running
     * their dtors.
     */
    void destroy_all()
    {
     ...
     * `m_control`'s actual size — this only stays safe because `m_control` is always allocated
     * with `m_capacity + GROUP_WIDTH` bytes (see rehash()/clear()), giving every group room to
     * overread into that padding without going past the vector's real allocation. Mess with
     * that
     ...
     * @warning Same overread footgun as match_byte() — depends on the `m_capacity +
     * GROUP_WIDTH` padding invariant holding.
     * @param group_idx which group of 16 control bytes to scan.
    ...
    [[nodiscard]] std::uint32_t match_empty_or_deleted(std::size_t group_idx) const
    {
        ...
        __m128i empty_mask = _mm_cmpeq_epi8(
            grp, _mm_set1_epi8(static_cast<char>(std::to_underlying(ControlByte::EMPTY)))
        );
        __m128i del_mask = _mm_cmpeq_epi8(
            grp, _mm_set1_epi8(static_cast<char>(std::to_underlying(ControlByte::DELETED)))
        );
     ...
     * into the fresh table — this also has the side effect of clearing out all tombstones,
     * since the new control array starts all-`EMPTY`.
     * @warning Every `iterator`/`const_iterator` and any raw slot index taken before calling
     * this is dead the instant it returns — the whole backing storage gets replaced. Hold onto
     * an iterator across an insert() that triggers a rehash and that's a straight
     * use-after-free waiting to happen, classic iterator-invalidation footgun, don't get cooked
     * by it.
     */
    void rehash()
    {
        ...
        // Walk the old table and move every live entry into the new one, destroying the old
        // copy in place once it's moved — vector<RawSlot>'s own dtor only frees raw bytes, it
        // has no idea an Entry was living there.
     ...
     * without finding room. Callers (insert()/upsert()) are the ones responsible for keeping
     * the load factor sane before reaching here.
     * @tparam KeyArg the forwarding-reference key argument type.
     * @tparam ValueArg the forwarding-reference value argument type.
     * @param key the key to construct in place.
     * @param value the value to construct in place.
     * @throws std::runtime_error if every slot in the table is occupied and probing wraps
     * without finding an empty or tombstoned slot — a hard L that should be unreachable given
     * insert()/ upsert() always rehash() before this gets called.
     */
    template<typename KeyArg, typename ValueArg>
    void insert_impl(KeyArg&& key, ValueArg&& value)
    {
        ...
        // Should be unreachable — insert()/upsert() always rehash() before this is called, so
        // the table should never actually be full here.
...
{ // NOLINT(bugprone-std-namespace-modification) — required tuple_element specialization for
  // structured bindings, permitted under [namespace.std]
    // Simple swith at compile time via std::conditional_t to return the correct type based on
    // the index I
