 ...
 * at all — so this registry only supports registering and iterating/checking, not a keyed
 * find().
 */
...
public:
    ...
    /// @brief Gets every registered generator. @return all registered generators, in
    /// registration order.
     ...
     * the process's one `OpenApiGeneratorRegistry`, mirroring
     * `SerdeFormatRegistry::set_active()`.
