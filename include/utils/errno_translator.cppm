...
public:
    ...
    [[nodiscard]] static std::string_view describe_errno(int code) noexcept
    {
        switch (code) {
            case 0:
                return "No errno set — for a plain syscall failure this would mean success, "
                       "but for "
                       "SSL_ERROR_SYSCALL specifically it means the peer closed the TCP "
                       "connection "
                       "without a clean TLS shutdown (no close_notify), so libssl has nothing "
                       "more "
                       "specific to report";
            case EPIPE:
                return "Broken pipe — the peer already closed its end of the connection, so "
                       "this "
                       "write landed on a dead socket";
            case ECONNRESET:
                return "Connection reset by peer — the remote side tore the connection down "
                       "abruptly "
                       "(TCP RST), typically after a crash, timeout, or forceful close on "
                       "their end";
            case ECONNREFUSED:
                return "Connection refused — nothing was listening on the target address/port, "
                       "or a "
                       "firewall actively rejected the connection attempt";
            case ETIMEDOUT:
                return "Connection timed out — the peer never responded within the allotted "
                       "time";
            case ENOTCONN:
            ...
            case EWOULDBLOCK:
#endif
                return "Resource temporarily unavailable — the operation would have blocked on "
                       "a "
                       "non-blocking socket; not a real failure, just come back later";
            case EBADF:
                return "Bad file descriptor — the socket handle is invalid, already closed, or "
                       "not a "
                       "socket at all";
            case EINTR:
                return "Interrupted system call — a signal arrived and interrupted the "
                       "operation "
                       "before it could complete";
            case EINVAL:
                return "Invalid argument — the socket or one of the call's parameters is in a "
                       "state "
                       "that doesn't support this operation";
            case ENOMEM:
                return "Out of memory — the kernel couldn't allocate the resources this "
                       "operation "
                       "needed";
            case EACCES:
            ...
            case EMFILE:
                return "Too many open files — this process has hit its own file descriptor "
                       "limit";
            case ENFILE:
                return "Too many open files in system — the system-wide file descriptor limit "
                       "has "
                       "been reached";
            case EADDRINUSE:
                return "Address already in use — another socket is already bound to this "
                       "address "
                       "and port";
            case EADDRNOTAVAIL:
                return "Address not available — the requested local address isn't assigned to "
                       "any "
            ...
            case ENOBUFS:
                return "No buffer space available — the kernel ran out of network buffer "
                       "memory for "
                       "this operation";
            case EPROTOTYPE:
            ...
            case EOPNOTSUPP:
                return "Operation not supported — this operation isn't supported on this "
                       "socket type";
            case ENOSPC:
...
suite<"ErrnoTranslator"> errno_translator_suite = [] {
    ...
    "unmapped codes fall back to Unknown error"_test = [] {
        expect(ErrnoTranslator::describe_errno(-999'999) == "Unknown error");
    };
};
