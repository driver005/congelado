...
public:
     ...
     */
    explicit NodeView(
        BufferNode* node, std::size_t start, std::size_t length, NodeView* next = nullptr
    ) :
        m_node{node},
        m_start{start},
        m_length{length},
    ...
    /**
     * @brief Indexes into the view's slice, offset from `m_start` — index 0 here means the
     * first byte of the *view*, not the underlying node.
    ...
    /**
     * @brief Links this view to the next one in the chain. Plain pointer store, no atomics —
     * this one's not built for concurrent linking like its `BufferReader` cousin.
     * @param next the next view to link to.
     */
    void set_next(NodeView* next) noexcept
    {
        m_next = next;
    }
     ...
     */
    [[nodiscard]] NodeView* get_next() const noexcept
    {
        return m_next;
    }
     ...
     */
    [[nodiscard]] const std::size_t& get_length() const noexcept
    {
        return m_length;
    }
     ...
     */
    std::size_t& get_length() noexcept
    {
        return m_length;
    }
     ...
     */
    [[nodiscard]] const std::size_t& get_start() const noexcept
    {
        return m_start;
    }
...
public:
    ...
    public:
         ...
         * @warning Unlike `BufferReader::Iterator`'s equivalent ctor, this one does NOT acquire
         * a reference on construction — only the copy ctor/assignment do that. Kinda
         * inconsistent with its `BufferReader` sibling, so don't assume symmetric refcount
         * behavior between the two, that's an easy mixup.
         * @param node the view to start iterating from.
         * @param offset the starting byte offset within `node`'s slice.
         */
        Iterator(NodeView* node, std::size_t offset) :
            m_node{node},
            m_offset{offset}
        {
            if (m_node != nullptr) {
        ...
        /**
         * @brief Default dtor — no reference to release, construction didn't acquire one (see
         * the ctor's @warning).
         */
        ~Iterator()
        {
        ...
        /**
         * @brief Copy assignment — acquires the new node before releasing the old one, avoiding
         * a premature drop on self-assignment-adjacent edge cases.
        ...
        Iterator& operator=(const Iterator& other) noexcept
        {
            if (this != &other) {
                ...
                // ordering trick as BufferReader::Iterator, dodges dropping the last reference
                // on a self-referential edge case.
        ...
        Iterator& operator++() noexcept
        {
            ...
            // acquiring the new one before releasing the old to keep a live reference
            // throughout.
     ...
     * reference and folding its length into the running size. Straightforward tail-link, no
     * cap.
     ...
     * own its `NodeReader`s the same way. Don't get the two release() semantics mixed up,
     * that's an easy L to take if you're jumping between the two types.
     */
    void release() noexcept
    {
     ...
     */
    [[nodiscard]] NodeView* get_head() const noexcept
    {
        return m_head;
    }
     ...
     */
    [[nodiscard]] NodeView* get_tail() const noexcept
    {
        return m_tail;
    }
