...
class BufferNode {
  public:
     ...
     * this node never frees `data`, that's on whoever handed it over. Also marks the whole thing
     * as already written (`m_written == size`), since it's assumed to already hold real data.
    ...
    /**
     * @brief Appends an entire input range one element at a time via push_back(). Straightforward
     * loop, no batching trick.
     ...
     * @warning No bounds check against `m_limit` — `fetch_add` just keeps incrementing the index
     * no matter what. Call this more times than the node has capacity for and you're writing past
     * the allocation, straight cooked. Safe for concurrent pushers to land on distinct indices
     * (that's the whole point of `fetch_add`), but only if the total stays under `m_limit` —
     ...
     * zero, `this` is a dangling pointer, full stop. Don't touch the node after calling release()
