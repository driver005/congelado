# Manager ↔ Plugin SDK Bridge Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Connect `core/manager` to plugins via the SDK's `Plugin` virtual class, moving `FfiBridge` out of `core/ffi` and into the manager as an internal detail, with `mode.h` for compile-time guest/host selection.

**Architecture:** `FfiBridge` logic moves to `include/core/manager/loader.cppm` as an internal partition (`core_plugin:loader`). A new `BridgeImpl` class (simplified FfiBridge, implements `ILogger`) is wrapped by `ManagedPlugin : congelado::Plugin`. `PluginManager` is the only public-facing type — it owns all `ManagedPlugin` instances, runs the 4-phase load (probe → filter → Kahn sort → activate), and registers logger plugins with `LoggerRegistry`. `App::load_plugins()` delegates entirely to `PluginManager`.

**Tech Stack:** C++26 modules, libffi, dlopen (POSIX), `congelado_plugin` SDK module, xmake build system.

## Global Constraints

- All class members: `m_` prefix, no abbreviations, ≥3-char names
- Getters: `get` prefix; Setters: `set` prefix; Adders: `add` prefix
- No `struct` for domain types — always `class` with explicit `public:`/`private:`
- Members always in `private:` section at bottom of class body
- No mutexes/strands — ASIO handlers already serialize
- Returned `std::string_view` from Plugin virtuals must point to stable string literals or `m_`-prefixed stable members
- `congelado_plugin.cppm` SDK module: imported via `import congelado_plugin;`
- `plugin.h` SDK header: included in global module fragment `#include <congelado/plugin.h>`
- Module partition `core_plugin:loader` is `export`ed but `BridgeImpl`/`ManagedPlugin` live in `namespace core::plugin::detail` (not in `export namespace`) — invisible to importers
...
|---|---|---|
| `sdk/plugin/include/congelado/mode.h` | Create | `CONGELADO_GUEST`/`CONGELADO_HOST` compile-time check |
| `sdk/plugin/include/congelado/plugin.h` | Modify | Add `mode.h` include; guard `CONGELADO_PLUGIN(T)` in `#ifdef CONGELADO_GUEST` |
| `include/core/manager/loader.cppm` | Create | `BridgeImpl`, `ManagedPlugin`, `Cap`, `PluginManager` (partition `core_plugin:loader`) |
| `include/core/manager/plugin.cppm` | Modify | Add `export import :loader;` |
| `include/core/ffi/bridge.cppm` | Delete | Was `core_ffi:bridge` — content moved to `loader.cppm` |
| `include/core/ffi/ffi.cppm` | Modify | Remove `export import :bridge;` (now empty module) |
| `include/congelado.cppm` | Modify | Remove `export import core_ffi;` |
| `include/core/heart/app.cppm` | Modify | Rewrite `load_plugins()` using `PluginManager`; remove `PluginHandle` |
| `defaults/plugins/http2/http2.cc` | Modify | Add `#define CONGELADO_GUEST` before SDK include |
| `defaults/plugins/file_logger/file_logger.cc` | Modify | Add `#define CONGELADO_GUEST` before SDK include |
| `defaults/plugins/postgres/postgres_plugin.cc` | Modify | Add `#define CONGELADO_GUEST` before SDK include |
| `defaults/plugins/engine/engine.cc` | Modify | Add `#define CONGELADO_GUEST` before SDK include |

---

### Task 1: Create `mode.h`

**Files:**
- Create: `sdk/plugin/include/congelado/mode.h`

**Interfaces:**
- Produces: compile-time enforcement of `CONGELADO_GUEST` xor `CONGELADO_HOST`; included by `plugin.h`

- [ ] **Step 1: Create the file**

```c
// sdk/plugin/include/congelado/mode.h
#pragma once
...
```bash
git add sdk/plugin/include/congelado/mode.h
git commit -m "feat(sdk): add mode.h for compile-time guest/host selection"
```

---

### Task 2: Guard `CONGELADO_PLUGIN(T)` in `plugin.h`

**Files:**
- Modify: `sdk/plugin/include/congelado/plugin.h`

**Interfaces:**
- Consumes: `mode.h` (just created)
- Produces: `CONGELADO_PLUGIN(T)` only available when `CONGELADO_GUEST` is defined; `CongeladoHostCallbacks`, `CongeladoConfigView`, `CONGELADO_CAP_*` remain unguarded (needed by both sides)

- [ ] **Step 1: Add `mode.h` include after `#pragma once` and wrap the macro**
...
After `#pragma once`, add the `mode.h` include. Then find the `#define CONGELADO_PLUGIN(T)` block (lines 51–132) and wrap it:
...
And around the `CONGELADO_PLUGIN(T)` macro (between `// ── CONGELADO_PLUGIN(T) ───` comment and `// NOLINTEND`):

```c
// ── CONGELADO_PLUGIN(T) ───────────────────────────────────────────────────────
// Only available in GUEST builds (.so plugin compilation).
// Generates all C dlsym symbols from a congelado::Plugin subclass.
#ifdef CONGELADO_GUEST
#define CONGELADO_PLUGIN(T) /* ... existing macro body unchanged ... */
#endif // CONGELADO_GUEST
// NOLINTEND
```

The macro body (lines 51–132 of `plugin.h`, the multi-line `\`-continued block starting with `static T *s_plugin = nullptr;`) is not modified — only the `#ifdef CONGELADO_GUEST` / `#endif` wrapper is added around it. Open the file, find the `// ── CONGELADO_PLUGIN(T) ───` comment, insert `#ifdef CONGELADO_GUEST` on the line before `#define CONGELADO_PLUGIN(T)`, and insert `#endif // CONGELADO_GUEST` on the line before `// NOLINTEND`.

- [ ] **Step 2: Verify C ABI structs and `CONGELADO_CAP_*` are NOT guarded**

Confirm lines defining `CongeladoHostCallbacks`, `CongeladoConfigView`, `CONGELADO_CAP_LOGGER`, `CONGELADO_CAP_PROTOCOL`, `CONGELADO_CAP_STORAGE`, `CONGELADO_CAP_CUSTOM` are outside any `#ifdef`.

- [ ] **Step 3: Commit**

```bash
git add sdk/plugin/include/congelado/plugin.h
git commit -m "feat(sdk): include mode.h and guard CONGELADO_PLUGIN in CONGELADO_GUEST"
```

---

### Task 3: Create `include/core/manager/loader.cppm`

**Files:**
- Create: `include/core/manager/loader.cppm`

**Interfaces:**
- Consumes: `std`, `interfaces` (for `ILogger`, `LogLevel`), `congelado_plugin` (for `Plugin`, `HostCallbacks`, `ConfigView`), `core_logger` (for `LoggerRegistry`), `core_config` (for `PluginConfig`)
- Consumes (global module fragment): `<ffi.h>`, `<dlfcn.h>` (POSIX) or `<windows.h>` (Win32), `<congelado/plugin.h>`
- Produces (exported): `core::plugin::Cap` enum, `core::plugin::PluginManager` class
- Produces (module-private, in `core::plugin::detail`): `Closure`, `PluginSymbols`, `BridgeImpl`, `ManagedPlugin`
...
```bash
git add include/core/manager/loader.cppm
git commit -m "feat(manager): add loader partition with BridgeImpl, ManagedPlugin, PluginManager"
```

---

### Task 4: Update `include/core/manager/plugin.cppm`

**Files:**
- Modify: `include/core/manager/plugin.cppm`

**Interfaces:**
- Consumes: `:loader` partition (just created)
- Produces: re-exports `core::plugin::Cap` and `core::plugin::PluginManager` to all importers of `core_plugin`

- [ ] **Step 1: Add `export import :loader;`**
...
```bash
git add include/core/manager/plugin.cppm
git commit -m "feat(manager): re-export loader partition from core_plugin"
```

---

### Task 5: Remove `core_ffi` module

**Files:**
- Delete: `include/core/ffi/bridge.cppm`
- Modify: `include/core/ffi/ffi.cppm`
- Modify: `include/congelado.cppm`

**Interfaces:**
- Consumes: nothing (deletion task)
- Produces: `core_ffi` module becomes empty; `congelado` umbrella module no longer re-exports it

- [ ] **Step 1: Empty out `include/core/ffi/bridge.cppm`**

Rather than deleting (which may need build system changes), replace content with empty module:

```cpp
export module core_ffi:bridge;
// Moved to core_plugin:loader
```

- [ ] **Step 2: Remove the re-export from `include/core/ffi/ffi.cppm`**
...
- [ ] **Step 3: Remove `core_ffi` from `include/congelado.cppm`**
...
```bash
git add include/core/ffi/bridge.cppm include/core/ffi/ffi.cppm include/congelado.cppm
git commit -m "refactor(ffi): empty core_ffi module — FfiBridge moved to core_plugin:loader"
```

---

### Task 6: Rebuild `App::load_plugins()`

**Files:**
- Modify: `include/core/heart/app.cppm`

**Interfaces:**
- Consumes: `core::plugin::PluginManager`, `core::plugin::Cap` (from `core_plugin` via `export import :loader`)
- Consumes: `core::config::Config::get_plugins()` → iterable of `[name, PluginConfig]` pairs
- Consumes: `AppContext::get_router()` → `RouterContext<Protocol>*`, `get_contract_group()` → `ContractGroup<>&`, `get_leverager()` → `Leverager<Context>&`
- Produces: `App::run()` uses `PluginManager`; no more `PluginHandle`

- [ ] **Step 1: Rewrite `app.cppm`**
  ...
  private:
    ...
    bool load_plugins(const core::config::Config &cfg, AppContext &ctx) {
        for (auto &[name, plugin_cfg] : cfg.get_plugins()) {
            ...
            m_manager.addPlugin(so, &plugin_cfg);
        }

        m_manager.activate(ctx.get_router(), &ctx.get_contract_group(), &ctx.get_leverager());

        std::println("[heart] loaded {} plugins", m_manager.getAll().size());
...
git commit -m "feat(heart): rewrite load_plugins() using PluginManager"
```

---

### Task 7: Add `CONGELADO_GUEST` to default plugins

**Files:**
- Modify: `defaults/plugins/http2/http2.cc`
- Modify: `defaults/plugins/file_logger/file_logger.cc`
- Modify: `defaults/plugins/postgres/postgres_plugin.cc`
- Modify: `defaults/plugins/engine/engine.cc`

**Interfaces:**
- Consumes: `mode.h` guard (enforced via `plugin.h` include)
- Produces: `CONGELADO_PLUGIN(T)` macro available in each plugin file

- [ ] **Step 1: Add `#define CONGELADO_GUEST` to `http2.cc`**
...
- [ ] **Step 2: Add `#define CONGELADO_GUEST` to `file_logger.cc`**
...
- [ ] **Step 3: Add `#define CONGELADO_GUEST` to `postgres_plugin.cc` and `engine.cc`**

Read each file and add `#define CONGELADO_GUEST` before the first `#include <congelado/plugin.h>` occurrence.

- [ ] **Step 4: Commit**

```bash
git add defaults/plugins/http2/http2.cc \
        defaults/plugins/file_logger/file_logger.cc \
        defaults/plugins/postgres/postgres_plugin.cc \
        defaults/plugins/engine/engine.cc
git commit -m "feat(plugins): define CONGELADO_GUEST in all default plugin files"
```
