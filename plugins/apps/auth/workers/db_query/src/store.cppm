...
/// `serde::Ser::from_map` — see the `Serializable<DbQueryInput>` specialization below.
/// `username` defaults to "default_user" via its in-class member initializer.
class DbQueryInput
{
public:
    ...
    void setPasswordHash(std::string value)
    {
        m_password_hash = std::move(value);
    }
...
/// @brief The connector-touching half of the `db_query` worker, kept in a module TU so the
/// plugin's own `.cc` never imports `connector` directly (that import crashes clang's modules
/// in a plugin entry TU — the engine dodges it the same way). Casts the opaque `connector_ctx`
/// the host injected and runs a real upsert + read-back through it.
class UserStore
{
public:
    /// @brief Outcome callback for `store_and_read_async` — the read-back (username,
    /// password_hash) pair, or std::nullopt on any failure.
    using Completion =
        std::move_only_function<void(std::optional<std::pair<std::string, std::string>>)>;

    /**
     * @brief Upserts an AuthUser then reads it back through the injected connector — proving a
     * "db call is a worker" against whatever backend the worker host resolved (in-memory
     * LocalStore when no storage plugin is loaded, a real DB otherwise). Chains straight
     * through the connector's own callbacks — no thread ever blocks.
