...
class DtoWriter {
  public:
     ...
     * property, then bolts on a serde::Serializable specialization per class. This is the W that
     * makes the generated DTOs actually (de)serialize.
    ...
    write(const std::unordered_map<std::string, SchemaType> &namedSchemas,
        ...
        for (const auto &name : order) {
            ...
            // Only object schemas can become classes — anything else means the OpenAPI doc named
            // a primitive/array at the top level, which isn't a supported shape here.
            ...
            for (const auto &[prop_name, prop_type] : schema.get_properties()) {
                ...
                cls.addMethod("void", std::format("set{}", member))
                    ...
                    .add_statement(core::generator::Stmt::expr(
            ...
            for (const auto &[prop_name, prop_type] : schema.get_properties()) {
                ...
                cls.addMethod(std::format("const {} &", type), std::format("get{}", member))
                    .add_statement(core::generator::Stmt::return_stmt(std::format("m_{}", prop_name)))
                    .set_const()
                    .set_noexcept()
                    .set_nodiscard()
                    .set_inline();
        ...
        for (const auto &name : order) {
            const auto &schema = namedSchemas.at(name);
            std::string block = std::format("template <>\nstruct serde::Serializable<{}::{}> {{\n",
                                            moduleName, name);
  ...
  private:
    /**
     * @brief Turns a snake_case property name into PascalCase so it can be glued onto "get"/"set"
     * for the generated accessor names — no cap, straight naming-convention motion.
     ...
     * `std::vector<...>`, nullable wraps the whole thing in `std::optional<...>`. Straightforward
     * motion, no funny business.
     ...
     * the ref sits right on the node or one level deep inside an Array's items — no missed deps,
     ...
     */
    static void topo_sort(const std::string &name,
