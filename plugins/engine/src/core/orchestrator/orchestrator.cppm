export module engine:orchestrator;
...
// one row and never advancing again. Bound to the shared EngineContext the same way TaskHandler/
...
class Orchestrator {
  public:
    explicit Orchestrator(EngineContext &ctx) noexcept : m_ctx{ctx} {}
    ...
    void start(std::string def_name, std::unordered_map<std::string, std::string> variables,
        ...
        m_ctx.get().get_connector().find<model::WorkflowDef>(
            def_name, [this, variables = std::move(variables), parent_exec_id,
                            ...
                            m_ctx.get().get_connector().insert<model::WorkflowExecution>(
                                ...
                                [callback = std::move(callback),
                                 to_return = std::move(to_return)](bool oke) mutable {
                                    callback(oke ? std::optional<model::WorkflowExecution>{
                                                        std::move(to_return)}
                                                 : std::nullopt);
                                });
                        });
            ...
            [this, exec_id = std::format("{}", child.get_exec_id()),
             status = child.get_status()](std::optional<model::WorkflowDef> def) {
                if (def && def->get_workflow_status_listener_enabled()) {
                    publish_event("conductor:workflow_status",
     ...
     * Intended to be called periodically by the engine plugin's background sweep thread.
     */
    void sweep_advance() {
        ...
        m_ctx.get().get_connector().find_all<model::WorkflowSchedule>(
            [this, now](std::vector<model::WorkflowSchedule> schedules) {
                for (auto &schedule : schedules) {
                    ...
                    auto base = schedule.get_last_fired_at().value_or(now - std::chrono::minutes{1});
     ...
     * def forbids it.
     */
    void rerun(std::string exec_id, std::string node_ref,
              std::unordered_map<std::string, std::string> input,
              std::move_only_function<void(bool)> callback) {
        m_ctx.get().get_connector().find<model::WorkflowExecution>(
            exec_id, [this, node_ref = std::move(node_ref), input = std::move(input),
                     callback = std::move(callback)](
                         std::optional<model::WorkflowExecution> exec) mutable {
                if (!exec) {
                    callback(false);
    ...
    void signal(std::string exec_id, std::string node_ref, std::optional<std::string> payload,
               std::move_only_function<void(bool)> callback) {
        m_ctx.get().get_connector().find<model::WorkflowExecution>(
            exec_id, [this, node_ref = std::move(node_ref), payload = std::move(payload),
                     callback = std::move(callback)](
                         std::optional<model::WorkflowExecution> exec) mutable {
                if (!exec) {
                    callback(false);
  ...
  private:
    ...
    bool edge_condition_ok(const model::WorkflowExecution &exec, const model::TaskEdge &edge,
        ...
        return LuaEval{m_ctx.get().get_lua_bridge()}.eval_condition(*edge.get_condition(), bindings);
    }
    ...
    void publish_event(std::string event_name, std::unordered_map<std::string, std::string> payload) {
        ...
        m_ctx.get().get_connector().find_all<model::EventHandler>(
            [this, event_name = std::move(event_name), payload = std::move(payload)](
                std::vector<model::EventHandler> handlers) {
    ...
    void execute_event_action(const model::EventAction &action) {
        ...
        case model::EventActionType::TERMINATE_WORKFLOW: {
            ...
            m_ctx.get().get_connector().find<model::WorkflowExecution>(
                exec_it->second, [this, status](std::optional<model::WorkflowExecution> exec) mutable {
                    if (!exec || model::is_terminal(exec->get_status())) {
                        return;
                    }
    ...
    void complete_instance_by_ref(std::string exec_id, std::string node_ref, model::TaskStatus status,
            ...
            if (!node.get_join_on().empty()) {
                bool ready =
                    node.get_join_type() == model::JoinType::ALL
                        ...
                        : std::ranges::any_of(
                              node.get_join_on(), [&](std::string const &ref) {
                                  auto const *instance =
                                      find_instance(exec.get_task_instances(), ref);
                                  return instance != nullptr &&
    ...
    void spawn_with_def(model::WorkflowExecution exec, model::WorkflowDef def,
        ...
        if (task_def && task_def->get_type() == model::TaskType::HUMAN) {
            ...
            exec.add_task_instance(instance);
            m_ctx.get().get_connector().insert<model::TaskInstance>(
                instance, log_on_failure(std::format("spawn HUMAN instance for node '{}'", ref_name)));
            spawn_next(std::move(exec), std::move(def), std::move(eligible_refs), index + 1,
                      std::move(done));
            return;
        }
        ...
        if (task_def && task_def->get_type() == model::TaskType::WAIT) {
            ...
            if (task_def->get_wait_duration_ms()) {
                instance.set_deadline_at(now + std::chrono::milliseconds{*task_def->get_wait_duration_ms()});
            }
            exec.add_task_instance(instance);
            m_ctx.get().get_connector().insert<model::TaskInstance>(
        ...
        if (task_def && task_def->get_type() == model::TaskType::START_WORKFLOW) {
            ...
            start(workflow_name_it->second, std::move(variables), std::nullopt,
                 [this, exec = std::move(exec), def = std::move(def),
                  ...
                  resolved_def_name, done = std::move(done)](
                     std::optional<model::WorkflowExecution> child) mutable {
                     auto now = std::chrono::system_clock::now();
                     model::TaskInstance instance;
                     instance.set_task_id(model::generate_id());
                     instance.set_workflow_exec_id(exec.get_exec_id());
                     ...
                     instance.set_status(child ? model::TaskStatus::COMPLETED
                                               : model::TaskStatus::FAILED);
                     if (child) {
                         instance.add_output_data("exec_id", std::format("{}", child->get_exec_id()));
                     }
                     model::ExecutionTimings timings;
                     timings.set_scheduled_at(now);
                     timings.set_started_at(now);
                     ...
                     m_ctx.get().get_connector().insert<model::TaskInstance>(
                         instance,
                         log_on_failure(std::format("spawn START_WORKFLOW instance for node '{}'",
                                                    ref_name)));
                     advance(std::move(exec), std::move(def), std::move(done));
                 });
            return;
            ...
            start(workflow_name_it->second, std::move(variables), exec.get_exec_id(),
                 [this, exec = std::move(exec), def = std::move(def),
                  ...
                  resolved_def_name, done = std::move(done)](
                     ...
                     m_ctx.get().get_connector().insert<model::TaskInstance>(
                         instance,
                         log_on_failure(std::format("spawn SUB_WORKFLOW instance for node '{}'",
                                                    ref_name)));
        ...
        if (task_def && task_def->get_type() == model::TaskType::EVENT) {
            ...
            exec.add_task_instance(instance);
            m_ctx.get().get_connector().insert<model::TaskInstance>(
                instance, log_on_failure(std::format("spawn EVENT instance for node '{}'", ref_name)));
            advance(std::move(exec), std::move(def), std::move(done));
            return;
        }
        ...
        if (task_def && task_def->get_enforce_schema() && task_def->get_input_schema()) {
            if (auto check = SchemaValidator::validate(*task_def->get_input_schema(), input); !check) {
                ...
                instance.set_status(model::TaskStatus::FAILED);
                instance.set_input_data(input);
                instance.add_output_data("error", std::format("input schema validation failed: {}",
                                                              check.error()));
                model::ExecutionTimings timings;
                timings.set_scheduled_at(now);
                timings.set_started_at(now);
                ...
                m_ctx.get().get_connector().insert<model::TaskInstance>(
                    instance,
                    log_on_failure(std::format("spawn schema-rejected instance for node '{}'", ref_name)));
                handle_failure(std::move(instance), std::move(exec), std::move(def),
                              task_def->get_retry());
                return;
            }
        ...
        bool within_limit = !has_limit || in_flight < task_def->get_rate_limit()->get_max_concurrent();
        ...
        if (within_limit) {
            ...
            core::logger::info("engine",
                              ...
                              "sweep_advance()",
                              ref_name);
            core::events::publish("engine.task.rate_limited",
                                 {{"node_ref", ref_name},
    ...
    void maybe_complete(model::WorkflowExecution &exec, const model::WorkflowDef &def) {
        ...
        bool all_done = std::ranges::all_of(
            exec.get_task_instances(), [](model::TaskInstance const &instance) {
    ...
    void handle_failure(model::TaskInstance instance, model::WorkflowExecution exec,
        ...
        if (retries_left) {
            ...
            core::logger::info("engine", "task '{}' retry {}/{} armed",
                               instance.get_def_name(), instance.get_retry_count(),
        ...
        m_ctx.get().get_connector().find<model::TaskDef>(
            ...
            [this, instance = std::move(instance)](std::optional<model::TaskDef> task_def) mutable {
                ...
                if (action == model::TimeoutAction::ALERT_ONLY) {
                    core::logger::warning("engine", "task '{}' timed out (alert only, still running)",
                                          instance.get_def_name());
                    return;
                }
