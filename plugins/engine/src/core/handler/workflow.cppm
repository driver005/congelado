  ...
  public:
    ...
    void create_definition(interfaces::io::IRequest &req, interfaces::io::IResponse &res) {
        ...
        m_ctx.get().get_connector().insert<model::WorkflowDef>(*parsed, [&](bool oke) {
            ...
            core::events::publish("engine.workflow_def.created", {{"name", parsed->get_name()}});
    ...
    void start_execution(interfaces::io::IRequest &req, interfaces::io::IResponse &res) {
        ...
        Orchestrator{m_ctx.get()}.start(
            ...
            [&res, accept, def_name](std::optional<model::WorkflowExecution> exec) mutable {
                if (!exec) {
    ...
    void get_execution(interfaces::io::IRequest &req, interfaces::io::IResponse &res) noexcept {
        ...
        m_ctx.get().get_connector().find<model::WorkflowExecution>(
            exec_id_str, [this, &res, accept](std::optional<model::WorkflowExecution> result) {
                if (!result) {
                    reply(res, serde::Ser::serialize_error(accept, "not found"),
                          interfaces::io::types::Status::NOT_FOUND);
                    return;
                }
                mask_and_reply(res, accept, std::move(*result));
            });
    }
    ...
    void terminate_execution(interfaces::io::IRequest &req, interfaces::io::IResponse &res) {
        ...
        m_ctx.get().get_connector().update<model::WorkflowExecution>(
            ...
            [&](bool oke) {
                ...
                core::events::publish("engine.workflow.terminated", {{"exec_id", exec_id_str}});
                Orchestrator{m_ctx.get()}.on_execution_terminal(found.value());  // NOLINT(bugprone-unchecked-optional-access) — same deliberate .value() check
    ...
    void pause_execution(interfaces::io::IRequest &req, interfaces::io::IResponse &res) {
        ...
        Orchestrator{m_ctx.get()}.pause(
            exec_id_from_action_path(req), [&res, accept](bool oke) {
                if (!oke) {
    ...
    void signal_execution(interfaces::io::IRequest &req, interfaces::io::IResponse &res) {
        ...
        Orchestrator{m_ctx.get()}.signal(
            ...
            [&res, accept](bool oke) {
                if (!oke) {
                    reply(res, serde::Ser::serialize_error(accept, "no matching in-progress instance"),
                          interfaces::io::types::Status::CONFLICT);
                    return;
                }
                res.set_status(interfaces::io::types::Status::OK);
    ...
    void bulk_pause(interfaces::io::IRequest &req, interfaces::io::IResponse &res) {
        bulk_dispatch(req, res, [](Orchestrator orchestrator, std::string exec_id,
  ...
  private:
    ...
    void mask_and_reply(interfaces::io::IResponse &res, std::string_view accept,
        ...
        m_ctx.get().get_connector().find_all<model::TaskDef>(
            [&res, accept, exec = std::move(exec)](std::vector<model::TaskDef> defs) mutable {
    ...
    void bulk_dispatch(
        ...
        bulk_apply(parsed->get_exec_ids(), 0, {}, std::move(op), res, accept);
    }
    ...
    void bulk_apply(
        ...
        op(Orchestrator{m_ctx.get()}, exec_id,
           [this, exec_ids = std::move(exec_ids), index, results = std::move(results),
            op = std::move(op), &res, accept](bool oke) mutable {
