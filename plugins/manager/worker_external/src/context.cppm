...
/// @brief Completion for `WorkerContext::run_task_async` — an `unexpected` `WorkerError` means
/// the task type is unregistered or `execute_async` threw (FAILURE); a present `WorkerOutput`
/// means the worker ran to completion (SUCCESS), matching `run_task`'s `WorkerResult` return
/// contract.
...
/// connection via `set_runtime()`) — non-copyable/non-movable by design, so that Register
/// member stays put at one stable address for the process's whole lifetime, same as the object
/// owning it. Workers themselves are stateless — all persistence lives engine-side.
class WorkerContext
{
public:
    ...
    void set_worker_id(std::string_view worker_id)
    {
        m_worker_id = std::string{worker_id};
    }
    ...
    /// stored callback — forwards to this context's own Register. What the transport layer
    /// calls the second a response shows up.
    ...
    /// @brief Looks up the registered worker for a task type. @param task_type the type to look
    /// up.
    ...
    /// @brief Runs a registered worker by type against a dynamic input value — the
    /// failure-catching wrapper around `IWorker::execute`, firing on_error/on_released hooks.
    /// @param task_type the task type to run. @param input the task's input value.
    /// @return the worker's output on success, or a WorkerError if unregistered or execute
    /// threw.
    [[nodiscard]] interfaces::WorkerResult
    run_task(std::string_view task_type, const serde::Value& input)
    {
        auto* worker = get_task_worker(task_type);
        if (worker == nullptr) {
            return std::unexpected{interfaces::WorkerError{"unregistered worker type"}};
        }
        interfaces::WorkerResult output = std::unexpected{interfaces::WorkerError{"unknown error"}};
        try {
    ...
    /// @brief Runs a registered worker by type asynchronously — the `execute_async` counterpart
    /// to `run_task`. `completion` fires exactly once with the task's WorkerResult.
    /// @param task_type the task type to run. @param input the task's input value.
    ...
    /// @param method the HTTP method. @param path the request path. @param body optional JSON
    /// body.
    /// @return the engine's status + body.
    /// @warning Never call from the transport's own dispatch thread — instant deadlock, no
    /// timeout.
    /// @warning Aborts the process if set_runtime() was never called — same fail-fast deal as
    /// every other core_client entry point.
    [[nodiscard]] EngineResponse
    call_engine(std::string_view method, std::string_view path, std::string_view body = "")
    {
        if (!m_register.has_runtime()) {
            std::abort();
        }
        auto request = core::client::Client::custom(method, path).build(m_register.runtime());
        // The engine's content-negotiation middleware picks the response serde format off
        // Accept — without it every reply with a body is a 406. Engine traffic is JSON both
        // ways.
...
{
public:
    explicit FakeWorker(std::string task_type) :
        m_task_type{std::move(task_type)}
    {
    }

    void set_result(interfaces::WorkerResult result)
    {
        m_result = std::move(result);
    }

    void set_throw_std_exception(bool value) noexcept
    {
        m_throw_std_exception = value;
    }

    void set_throw_unknown(bool value) noexcept
    {
        m_throw_unknown = value;
    }
    ...
    [[nodiscard]] interfaces::WorkerResult execute(const interfaces::Value& /*input*/) override
    {
        ...
        if (m_throw_unknown) {
            throw 42; // NOLINT — deliberately not a std::exception, to hit the catch(...)
                      // branch.
        }
        return m_result;
    }
    ...
    void on_released() noexcept override
    {
        ++m_on_released_count;
    }

    [[nodiscard]] int get_execute_count() const noexcept
    {
        return m_execute_count;
    }

    [[nodiscard]] int get_on_error_count() const noexcept
    {
        return m_on_error_count;
    }

    [[nodiscard]] int get_on_released_count() const noexcept
    {
        return m_on_released_count;
    }
...
/// against — set_header/set_body are no-ops (store nothing meaningful), find_header hands back
/// a stub value. Mirrors core_client:registry's own RegisterFakeRequest test double.
...
suite<"WorkerContext::run_task_async"> worker_context_run_task_async_suite = [] {
    ...
    // IWorker::execute_async() is documented to "always park the caller" — it queues the job
    // onto the worker's own TaskQueue rather than running it inline, and nothing here drains
    // that queue (that only happens once bound to a real contract via set_contract_group(),
    // which needs a live ContractGroup this unit test never stands up). So for a registered
