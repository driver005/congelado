...
// wraps every dispatched handler in a SERVER span on its own, same as
...
has_route_with_summary(std::size_t since, std::uint8_t method, std::string_view summary)
{
    ...
    for (std::size_t index = since; index < routes.size(); ++index) {
        ...
        // Registry::get_operations() walker (openapi_generator's
        // doc_generator.cppm::generate()) already uses, and it never crashes. Router nodes
        // (e.g. "/api", "/v1") register with zero operations, so this loop is simply a no-op
        // for them.
