...
class AdminConfig {
  public:
    AdminConfig() = default;
    AdminConfig(bool db_configured, bool lua_bridge_configured,
...
class AdminHandler {
  public:
     ...
     */
    void config(interfaces::io::IRequest &req, interfaces::io::IResponse &res,
                std::function<void()> send) noexcept {
        auto accept = req.find_header("accept");
        AdminConfig cfg{m_ctx.get().get_db() != nullptr, m_ctx.get().get_lua_bridge() != nullptr,
                        5};
...
suite<"AdminHandler"> admin_handler_suite = [] {
    "config reports db_configured/lua_bridge_configured false on an unwired context"_test = [] {
        ...
        handler.config(req, res, [&sent] { sent = true; });
    ...
    "consistency replies 200 when the workflow orchestrator reconciles successfully"_test = [] {
        ...
        handler.consistency(req, res, [&sent] { sent = true; });
