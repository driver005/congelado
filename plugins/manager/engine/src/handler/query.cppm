...
public:
     ...
     * lifetime.
     */
    explicit QueryHandler(EngineContext& ctx) noexcept :
        m_ctx{ctx}
    {
    }
     ...
     * that doesn't start with `SELECT` (case-insensitive, leading whitespace trimmed) with a
     * 400 before it ever reaches the database — a simple keyword gate, not a real SQL parser,
     * so it's a speed bump against accidental DDL/DML from this box, not a hard security
     * boundary.
    ...
    ) noexcept
    {
        ...
        // supports stacked statements, so a body like "SELECT 1; DROP TABLE x;" passes this
        // gate and executes the DROP. This route accepts the raw request body as SQL with no
        // other validation and (per routes.cppm) no authentication — effectively an
        // unauthenticated SQL console with a bypassable read-only fig-leaf. Needs a real
        ...
        // Callback calls serde::Ser::serialize_raw()/serialize_error(), which can throw, so
        // it's wrapped instead of relying on this whole method staying noexcept through the
        // callback.
...
{
public:
    explicit FakeDatabase(std::string result) :
...
/// @brief Copies a plain string into the byte vector run_query() reads as the SQL body —
/// mirrors Ser::to_bytes, just local to this test block.
