...
public:
     ...
     * lifetime.
     */
    explicit MetadataHandler(EngineContext& ctx) noexcept :
        m_ctx{ctx}
    {
    }
    ...
    ) noexcept
    {
        ...
        // cache-first — if a cached "ok" is already sitting there, ship it and skip the db
        // entirely
        ...
        // no cache hit — fall back to pinging the db, warming the cache with the OK payload on
        // the way out
...
{
public:
    explicit CachedValueCache(std::string value) :
...
public:
    ...
    [[nodiscard]] bool was_queried() const noexcept
    {
        return m_query_called;
    }
...
// JsonPlugin) — needed so list_task_definitions()/list_workflow_definitions() below can
// actually go through serde::Ser::serialize() and produce a real, countable JSON array instead
// of the "no format plugin loaded" error payload serialize() falls back to with nothing
// registered.
