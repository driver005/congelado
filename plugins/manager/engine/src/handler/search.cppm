...
/// @brief Search collection names — the wire contract shared with the workflow_orchestrator plugin's
/// SummaryProjector (which writes these collections on every terminal transition). Kept in sync with
/// that plugin's copy; they must match for search to read what the projector wrote.
...
class SearchRequestBody {
  public:
    ...
    [[nodiscard]] interfaces::SearchQuery to_search_query() const {
        return interfaces::SearchQuery{.query = m_query,
...
class SearchHandler {
  ...
  private:
    ...
    void run_search(interfaces::io::IRequest &req, interfaces::io::IResponse &res,
                    std::function<void()> send, std::string_view collection) {
...
class FakeSearchProvider final : public interfaces::ISearchProvider {
  public:
    explicit FakeSearchProvider(std::string result) : m_result{std::move(result)} {}
    ...
    [[nodiscard]] const std::string &last_collection() const noexcept { return m_last_collection; }
...
suite<"SearchHandler"> search_handler_suite = [] {
    "search_workflows replies 200 with an empty array when no search provider is configured"_test =
        [] {
            ...
            handler.search_workflows(req, res, [&sent] { sent = true; });
    ...
    "search_tasks replies 200 with an empty array when no search provider is configured"_test = [] {
        ...
        handler.search_tasks(req, res, [&sent] { sent = true; });
