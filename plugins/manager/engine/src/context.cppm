...
class EngineContext {
  public:
     ...
     */
    void set_connector(connector::Connector *connector) noexcept { m_connector = connector; }
    ...
    /**
     * @brief Wires in the dispatch/orchestration backend the queue endpoints route through — the
     * built-in engine Orchestrator by default, or a resolved worker_orchestrator plugin that
     * overrides it. EnginePlugin::on_load sets this after resolving the capability.
     ...
     * @return the orchestrator pointer, or nullptr if none was set — check before dereferencing.
     */
     ...
     * orchestrator) — a resolved workflow_orchestrator plugin. EnginePlugin::on_load sets it after
     * resolving the capability. NULL means none was resolved.
...
class FakeDatabase final : public interfaces::IDatabase {
  public:
    ...
    void query(std::string_view,
               std::move_only_function<void(std::string_view)> &&result) noexcept override {
        result("");
    }
...
class FakeBridge final : public interfaces::IBridge {
  public:
    ...
    [[nodiscard]] std::string_view runtime_name() const noexcept override { return "fake_lua"; }
    [[nodiscard]] std::string_view script_extension() const noexcept override { return ".fake"; }
...
class FakeSearchProvider final : public interfaces::ISearchProvider {
  public:
    ...
    void index(std::string_view, std::string_view, std::string_view,
               std::move_only_function<void(std::string_view)> &&callback) noexcept override {
        callback("ok");
    }
    void remove(std::string_view, std::string_view,
                std::move_only_function<void(std::string_view)> &&callback) noexcept override {
        callback("ok");
    }
    void search(std::string_view, const interfaces::SearchQuery &,
               std::move_only_function<void(std::string_view)> &&callback) noexcept override {
        callback("[]");
    }
};
...
class FakeCron final : public interfaces::ICron {
  public:
    [[nodiscard]] std::string_view backend_name() const noexcept override { return "fake_cron"; }
...
class FakeWorkerOrchestrator final : public interfaces::IWorkerOrchestrator {
  public:
    ...
    void
    set_health_callback(std::move_only_function<void(const interfaces::OrchestratorInfo &)>) override {
    }
};
...
class FakePayloadStorage final : public interfaces::IExternalPayloadStorage {
  public:
    void write(interfaces::PayloadType, std::string_view,
              std::move_only_function<void(std::string_view)> &&callback) noexcept override {
        callback("ref");
    }
