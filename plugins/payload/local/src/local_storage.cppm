...
/// configured directory, named by a fresh id; the file path itself is the returned reference. Plain
/// local disk I/O, no external service connection — the default payload backend, resolved as the
/// `payload_storage` capability. The directory is set from the plugin's config in on_load (defaults
/// to "payloads"); the storage instance itself exists at plugin construction so the host can resolve
/// its pointer before build() (the resolve-before-build contract every capability follows).
class LocalPayloadStorage : public interfaces::IExternalPayloadStorage {
  public:
    ...
    LocalPayloadStorage() { set_directory("payloads"); }

    /// @brief Out-of-line dtor (a key function) so the vtable is emitted in this module TU rather
    /// than left to vague-linkage dedup — clang's C++20-modules handling is inconsistent about
    /// emitting it across a construction site in the plugin .cc, causing undefined-symbol at dlopen.
    ...
    void set_directory(std::filesystem::path directory) {
        ...
        if (error_code) {
            core::logger::warning("payload.local", "could not create '{}': {}", m_dir.string(),
                                  error_code.message());
            core::events::publish("payload.local.create_directory_failed",
    ...
    // doc, this interface "is NOT yet wired into any actual input/output write path." The moment
    // a `reference` is derived from anything caller/attacker-influenced (a payload key echoed
    // back through an API, a row a caller can edit, etc.) this becomes a straight arbitrary-file
    // read off whatever this process can access. Needs the resolved path canonicalized and
    // checked to still be lexically inside `m_dir` before opening it.
    void read(std::string_view reference, shared::QueryReadFn &&callback) noexcept override {
        ...
        if (!in) {
            ...
            core::events::publish("payload.local.open_read_failed",
...
class TempSandbox {
  public:
    TempSandbox() {
        static std::atomic<std::uint64_t> counter{0};
        auto unique = std::format("congelado_payload_local_test_{:x}_{:x}",
...
suite<"LocalPayloadStorage::write/read"> local_payload_write_read_suite = [] {
    "write() then read() round-trips the exact bytes"_test = [] {
        ...
        storage.write(interfaces::PayloadType::TASK_OUTPUT, "hello payload",
    ...
    "write() with empty data still succeeds and round-trips an empty payload"_test = [] {
        ...
        storage.write(interfaces::PayloadType::WORKFLOW_INPUT, "",
                      [&](std::string_view reference) { written_reference = std::string{reference}; });
    ...
    "two consecutive write() calls land in two distinct files, never colliding"_test = [] {
        ...
        storage.write(interfaces::PayloadType::TASK_INPUT, "first",
                      [&](std::string_view reference) { first_reference = std::string{reference}; });
        storage.write(interfaces::PayloadType::TASK_INPUT, "second",
                      [&](std::string_view reference) { second_reference = std::string{reference}; });
        ...
        storage.read(first_reference, [&](std::string_view data) { first_read = std::string{data}; });
        storage.read(second_reference, [&](std::string_view data) { second_read = std::string{data}; });
    ...
    "read() on a reference that was never written reports failure via an empty callback string"_test =
        [] {
            ...
            std::string read_back = "unset";
            storage.read((sandbox.get_root() / "never-written.payload").string(),
                        [&](std::string_view data) { read_back = std::string{data}; });
    ...
    "write() into a directory that couldn't be created (blocked by a same-named file) fails via an empty callback string"_test =
        [] {
            ...
            std::string written_reference = "unset";
            storage.write(interfaces::PayloadType::TASK_OUTPUT, "data",
                          [&](std::string_view reference) { written_reference = std::string{reference}; });
...
// path like /etc/passwd) but demonstrate the exact same escape a hostile reference would achieve
// against a real deployment's payload directory.
suite<"LocalPayloadStorage::read path confinement (SECURITY)"> local_payload_traversal_suite = [] {
    ...
    "a relative '../' reference resolves against the process's current directory, not the configured storage directory — read() never joins the reference with m_dir at all"_test =
        [] {
            ...
            std::string read_back;
            storage.read("../outside/secret.txt",
                        [&](std::string_view data) { read_back = std::string{data}; });
