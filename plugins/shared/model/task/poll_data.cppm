...
/// @brief One worker type's last-seen poll heartbeat — populated by TaskHandler::poll() on
/// every hit (worker_type is whatever the caller polled for, hit or miss). Backs the `GET
/// /api/v1/tasks/queue/polldata` admin introspection route; actual queue depth is computed on
/// demand by `/queue/sizes` rather than cached here, so this is purely "is anything out there
