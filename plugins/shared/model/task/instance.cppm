...
public:
    ...
    void set_node_ref(std::string node_ref)
    {
        m_node_ref = std::move(node_ref);
    }
    ...
    /// @brief Replaces the whole input blob wholesale — a dynamic JSON-like value
    /// (`serde::Value`), so a task's input can carry typed fields (numbers/bools/nested
    /// objects), not just a flat string map.
    /// @param data the new input value.
    void set_input_data(serde::Value data)
    {
    ...
    void set_timings(ExecutionTimings timing)
    {
        m_timings = timing;
    }
    ...
    void set_retry_count(std::uint32_t count) noexcept
    {
        m_retry_count = count;
    }
    ...
    /// that still has retry attempts left — the background sweep flips it back to SCHEDULED
    /// once this elapses.
    ...
    /// @param exec_id the child execution's id, or std::nullopt for a non-SUB_WORKFLOW
    /// instance.
    ...
    /// @brief Gets the input blob — a dynamic JSON-like value (string/bool/int/double/array/
    /// object), not a flat string map.
    ...
    [[nodiscard]] std::uint32_t get_retry_count() const noexcept
    {
...
suite<"TaskInstance"> task_instance_suite = [] {
    "defaults to SCHEDULED, seq 0, retry_count 0, nil ids"_test = [] {
        ...
        expect(instance.get_seq() == 0);
        expect(instance.get_retry_count() == 0);
        expect(instance.get_task_id().is_nil());
    ...
    "add_output_data accumulates by key, last write wins"_test = [] {
        ...
        instance.add_output_data("status", "delivered");
    ...
    "an invalid nested timings ordering propagates through validate"_test = [] {
        ...
        timings.set_completed_at(now - std::chrono::seconds{1});
