...
public:
    ...
    void add_input_key(std::string key)
    {
        m_input_keys.push_back(std::move(key));
    }

    /// @brief Appends a key to the list of output keys this task produces.
    /// @param key the output key to add.
    void add_output_key(std::string key)
    {
        m_output_keys.push_back(std::move(key));
    }
    ...
    void set_type(TaskType type) noexcept
    {
        m_type = type;
    }
    ...
    void set_worker_type(std::string type)
    {
        m_worker_type = std::move(type);
    }

    /// @brief Replaces the whole input key list wholesale.
    /// @param input the new set of input keys.
    void set_input_keys(std::vector<std::string> input)
    {
        m_input_keys = std::move(input);
    }

    /// @brief Replaces the whole output key list wholesale.
    /// @param output the new set of output keys.
    void set_output_keys(std::vector<std::string> output)
    {
        m_output_keys = std::move(output);
    }

    /// @brief Sets the retry policy for this task.
    /// @param retry the new retry policy.
    void set_retry(RetryPolicy retry) noexcept
    {
        m_retry = retry;
    }
    ...
    void set_timeout(TimeoutPolicy timeout) noexcept
    {
        m_timeout = timeout;
    }
    ...
    void set_input_schema(std::optional<std::string> schema)
    {
        m_input_schema = std::move(schema);
    }
    ...
    void set_enforce_schema(bool enforce) noexcept
    {
        m_enforce_schema = enforce;
    }
    ...
    void set_masked_fields(std::vector<std::string> fields)
    {
        m_masked_fields = std::move(fields);
    }
    ...
    [[nodiscard]] TaskType get_type() const noexcept
    {
        return m_type;
    }
    ...
    [[nodiscard]] const RetryPolicy& get_retry() const noexcept
    {
        return m_retry;
    }
    ...
    [[nodiscard]] const TimeoutPolicy& get_timeout() const noexcept
    {
        return m_timeout;
    }
    ...
    [[nodiscard]] const std::optional<std::string>& get_domain() const noexcept
    {
        return m_domain;
    }
    ...
    [[nodiscard]] bool get_enforce_schema() const noexcept
    {
    ...
    [[nodiscard]] std::expected<void, std::string> validate() const noexcept
    {
        ...
        // Delegate to the nested retry policy's own validate() and bubble its error straight
        // up.
...
suite<"TaskDef"> task_def_suite = [] {
    ...
    "setters round-trip and add_input_key/add_output_key accumulate"_test = [] {
        ...
        def.set_worker_type("email_worker");
        def.add_input_key("to");
        def.add_input_key("subject");
        def.add_output_key("message_id");

        expect(def.get_name() == "send_email");
        expect(def.get_worker_type() == "email_worker");
