...
public:
    ...
    [[nodiscard]] std::expected<void, std::string> validate() const noexcept
    {
        ...
        // Second pair: same ordering check, but completion can't come before the start — no
        // cap.
...
suite<"ExecutionTimings"> execution_timings_suite = [] {
    "defaults to all-unset timestamps and passes validation"_test = [] {
        ...
        expect(not timings.get_started_at().has_value());
    ...
    "setters round-trip through their getters"_test = [] {
        ...
        expect(timings.get_scheduled_at() == now);
    ...
    "rejects started_at before scheduled_at"_test = [] {
        ...
        timings.set_scheduled_at(now);
        timings.set_started_at(now - std::chrono::seconds{1});
