...
public:
    ...
    RetryPolicy(
        ...
        std::uint32_t interval_ms = 1'000
    ) :
    ...
    void set_max_attempts(std::uint32_t max_attempts) noexcept
    {
        m_max_attempts = max_attempts;
    }
    ...
    void set_backoff(RetryBackoff backoff) noexcept
    {
        m_backoff = backoff;
    }
    ...
    [[nodiscard]] std::uint32_t get_max_attempts() const noexcept
    {
    ...
    [[nodiscard]] RetryBackoff get_backoff() const noexcept
    {
        return m_backoff;
    }
    ...
    [[nodiscard]] std::uint32_t get_interval_ms() const noexcept
    {
    ...
    {
        // Zero attempts means the task never gets a first try — check that before anything
        // else.
        if (m_max_attempts == 0) {
            return std::unexpected{"max_attempts must be at least 1"};
        }
        // Same deal for the interval — a zero wait between retries isn't really backoff at all,
        // bet.
        if (m_interval_ms == 0) {
...
public:
    ...
    void set_timeout_ms(std::uint32_t timeout_ms) noexcept
    {
        m_timeout_ms = timeout_ms;
    }
    ...
    void set_action(TimeoutAction action) noexcept
    {
        m_action = action;
    }
    ...
    [[nodiscard]] std::uint32_t get_timeout_ms() const noexcept
    {
        return m_timeout_ms;
    }
    ...
    [[nodiscard]] TimeoutAction get_action() const noexcept
    {
        return m_action;
    }
...
public:
    ...
    void set_rate_limit_per_second(std::uint32_t rps) noexcept
    {
        m_rate_limit_per_second = rps;
    }

    /// @brief Gets the configured max concurrent in-flight executions.
    /// @return the concurrency cap.
    [[nodiscard]] std::uint32_t get_max_concurrent() const noexcept
    {
...
suite<"RetryPolicy"> retry_policy_suite = [] {
    "defaults to 3 attempts, fixed backoff, 1000ms interval"_test = [] {
        ...
        expect(policy.get_backoff() == RetryBackoff::FIXED);
