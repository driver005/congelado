...
public:
    ...
    void set_event_id(EventId event_id)
    {
        m_event_id = event_id;
    }
    ...
    void set_type(WorkflowEventType type) noexcept
    {
        m_type = type;
    }
    ...
    void set_payload(std::optional<std::string> payload)
    {
    ...
    /// @return the event id.
    [[nodiscard]] const EventId& get_event_id() const noexcept
    {
        return m_event_id;
    }
    ...
    [[nodiscard]] WorkflowEventType get_type() const noexcept
    {
        return m_type;
    }
...
suite<"WorkflowEvent"> workflow_event_suite = [] {
    "defaults to a nil exec_id, type PAUSE, no payload, and fails validation"_test = [] {
        ...
        expect(event.get_type() == WorkflowEventType::PAUSE);
    ...
    "setters round-trip and a non-nil exec_id passes validation"_test = [] {
        ...
        event.set_type(WorkflowEventType::SIGNAL);
        event.set_payload("order_shipped");
        event.set_issued_at(now);

        expect(event.get_exec_id() == exec_id);
        expect(event.get_type() == WorkflowEventType::SIGNAL);
        expect(event.get_payload() == "order_shipped");
        expect(event.get_issued_at() == now);
        expect(bool(event.validate()));
    };
};
