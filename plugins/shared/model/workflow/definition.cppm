...
public:
    ...
    void add_input_param(std::string param)
    {
        m_input_params.push_back(std::move(param));
    }
    ...
    void set_nodes(std::vector<TaskNode> nodes)
    {
    ...
    void set_input_params(std::vector<std::string> params)
    {
        m_input_params = std::move(params);
    }
    ...
    void set_timeout(std::optional<TimeoutPolicy> timeout) noexcept
    {
        m_timeout = timeout;
    }

    /// @brief Sets whether executions of this def may be retried/restarted/rerun after reaching
    /// a terminal state — gates the pause/resume/retry/restart/rerun routes.
    /// @param restartable true to allow it (the default), false to forbid it.
    void set_restartable(bool restartable) noexcept
    {
        m_restartable = restartable;
    }

    /// @brief Turns on publishing a "conductor:workflow_status" event (via the Phase 5 event
    /// dispatcher) every time an execution of this def reaches a terminal status — lets an
    /// EventHandler subscribed to that event name react without polling.
    /// @param enabled true to publish on every terminal transition.
    void set_workflow_status_listener_enabled(bool enabled) noexcept
    {
        m_workflow_status_listener_enabled = enabled;
    }
    ...
    [[nodiscard]] const std::vector<TaskNode>& get_nodes() const noexcept
    {
        return m_nodes;
    }
    ...
    /// @brief Gets whether executions of this def may be retried/restarted/rerun.
    /// @return true if allowed (the default), false if forbidden.
    [[nodiscard]] bool get_restartable() const noexcept
    {
        return m_restartable;
    }

    /// @brief Gets whether terminal-transition events are published for this def.
    /// @return true if publishing is enabled.
    [[nodiscard]] bool get_workflow_status_listener_enabled() const noexcept
    {
    ...
    [[nodiscard]] std::expected<void, std::string> validate() const noexcept
    {
        ...
        for (const auto& mapping: m_output_mappings) {
...
private:
    ...
    bool m_workflow_status_listener_enabled{false};
};
    ...
    static constexpr auto fields()
    {
        return std::tuple{
            ...
            serde::FieldDesc<
                ...
                &model::WorkflowDef::get_workflow_status_listener_enabled,
                &model::WorkflowDef::set_workflow_status_listener_enabled>{},
        };
    }
};
...
suite<"WorkflowDef"> workflow_def_suite = [] {
    "defaults to version 1, restartable, no nodes, and fails validation"_test = [] {
        ...
        expect(def.get_restartable());
        expect(not def.get_workflow_status_listener_enabled());
    ...
    "add_node/add_input_param/add_output_mapping accumulate"_test = [] {
        ...
        def.add_node(node);
        def.add_input_param("order_id");
        def.add_output_mapping(OutputMapping{"$.result", "order_status"});

        expect(def.get_nodes().size() == 1);
        expect(def.get_input_params().size() == 1);
