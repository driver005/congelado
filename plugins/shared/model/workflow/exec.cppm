...
class WorkflowExecution {
  public:
    ...
    void set_dynamic_nodes(std::vector<TaskNode> nodes) { m_dynamic_nodes = std::move(nodes); }
    /// @brief Appends a single materialized dynamic node.
    /// @param node the node to add.
    void add_dynamic_node(TaskNode node) { m_dynamic_nodes.push_back(std::move(node)); }
...
struct serde::Serializable<model::WorkflowExecution> {
    ...
    static constexpr auto fields() {
        return std::tuple{
            ...
            serde::FieldDesc<"dynamic_nodes", &model::WorkflowExecution::get_dynamic_nodes,
                       &model::WorkflowExecution::set_dynamic_nodes>{},
            serde::FieldDesc<"parent_exec_id", &model::WorkflowExecution::get_parent_exec_id,
