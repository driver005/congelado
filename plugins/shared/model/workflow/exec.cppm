...
public:
    ...
    void set_timings(ExecutionTimings timings)
    {
        m_timings = timings;
    }

    /// @brief Replaces the whole dynamic-node list wholesale — TaskNodes materialized at
    /// runtime for this execution only (FORK_JOIN_DYNAMIC branches + their synthetic join),
    /// never written back to the shared WorkflowDef. Orchestrator::effective_nodes() combines
    /// these with the WorkflowDef's own static nodes when computing eligibility for this exec.
    /// @param nodes the new dynamic node list.
    void set_dynamic_nodes(std::vector<TaskNode> nodes)
    {
        m_dynamic_nodes = std::move(nodes);
    }

    /// @brief Appends a single materialized dynamic node.
    /// @param node the node to add.
    void add_dynamic_node(TaskNode node)
    {
        m_dynamic_nodes.push_back(std::move(node));
    }

    /// @brief Sets the parent execution this one was spawned as a SUB_WORKFLOW child of.
    /// @param parent_exec_id the parent's execution id, or std::nullopt for a top-level
    /// execution (started directly, or via START_WORKFLOW's fire-and-forget path).
    void set_parent_exec_id(std::optional<ExecutionId> parent_exec_id)
    {
        m_parent_exec_id = parent_exec_id;
    }
    ...
    [[nodiscard]] std::uint32_t get_def_version() const noexcept
    {
    ...
    [[nodiscard]] const ExecutionTimings& get_timings() const noexcept
    {
        return m_timings;
    }

    /// @brief Gets the runtime-materialized dynamic nodes.
    /// @return the configured dynamic nodes, empty if none have been materialized.
    [[nodiscard]] const std::vector<TaskNode>& get_dynamic_nodes() const noexcept
    {
        return m_dynamic_nodes;
    }

    /// @brief Gets the parent execution this one was spawned as a SUB_WORKFLOW child of.
    /// @return the parent's execution id, or std::nullopt for a top-level execution.
    [[nodiscard]] const std::optional<ExecutionId>& get_parent_exec_id() const noexcept
    {
    ...
    [[nodiscard]] std::expected<void, std::string> validate() const noexcept
    {
        ...
        for (const auto& instance: m_task_instances) {
...
private:
    ...
    std::optional<ExecutionId> m_parent_exec_id;
};
...
suite<"WorkflowExecution"> workflow_execution_suite = [] {
    ...
    "a busted nested task instance propagates through validate"_test = [] {
        ...
        exec.add_task_instance(bad_instance);
