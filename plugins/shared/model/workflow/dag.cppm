...
public:
    ...
    [[nodiscard]] const std::string& get_target() const noexcept
    {
        return m_target;
    }
...
public:
    ...
    void add_mapping(InputMapping mapping)
    {
    ...
    void set_to(std::string target_name)
    {
        m_to = std::move(target_name);
    }
    ...
    void set_condition(std::optional<std::string> cond)
    {
    ...
    void set_mappings(std::vector<InputMapping> mappings)
    {
    ...
    [[nodiscard]] const std::string& get_from() const noexcept
    {
        return m_from;
    }
    ...
    [[nodiscard]] const std::string& get_to() const noexcept
    {
        return m_to;
    }
...
public:
    ...
    void set_task_def_name(std::string def_name)
    {
        m_def_name = std::move(def_name);
    }
    ...
    void set_ref_name(std::string ref_name)
    {
        m_ref_name = std::move(ref_name);
    }

    /// @brief Replaces the whole outgoing edge list wholesale.
    /// @param edges the new list of edges.
    void set_edges(std::vector<TaskEdge> edges)
    {
    ...
    void set_join_on(std::vector<std::string> join_on)
    {
        m_join_on = std::move(join_on);
    }
    ...
    void set_join_type(JoinType join_type) noexcept
    {
        m_join_type = join_type;
    }

    /// @brief Sets the ref_names of the nested subgraph this DO_WHILE node loops over — these
    /// must be ordinary nodes elsewhere in the same WorkflowDef's node list (DO_WHILE doesn't
    /// own a separate nested node list, it just re-runs this same set of refs each iteration).
    /// Empty means "not a loop node."
    /// @param loop_body the ref_names forming the loop body.
    void set_loop_body(std::vector<std::string> loop_body)
    {
    ...
    [[nodiscard]] const std::string& get_def_name() const noexcept
    {
        return m_def_name;
    }
    ...
    [[nodiscard]] const std::string& get_ref_name() const noexcept
    {
        return m_ref_name;
    }
    ...
    [[nodiscard]] const std::vector<TaskEdge>& get_edges() const noexcept
    {
        return m_edges;
    }
    ...
    [[nodiscard]] const std::vector<std::string>& get_join_on() const noexcept
    {
        return m_join_on;
    }
    ...
    /// @return ALL or ANY.
    [[nodiscard]] JoinType get_join_type() const noexcept
    {
        return m_join_type;
    }
     ...
     * @warning Doesn't check that each edge's `from` actually matches this node's ref_name,
     * that `to` points at a node that exists anywhere in the containing WorkflowDef, or that
     * ref_name is actually unique among sibling nodes — that cross-referencing is left entirely
     * to the caller (WorkflowDef doesn't do it either, see its own validate() warning). A
     * TaskNode can validate() fine while its edges dangle into nowhere or its ref_name collides
     * with another node's.
     * @return an empty expected if everything checks out, otherwise an unexpected describing
     * the first thing that's busted.
     */
...
/// @brief One branch of a FORK_JOIN_DYNAMIC node's runtime task list — deserialized straight
/// out of the workflow variable named by TaskNode::get_dynamic_tasks_input_key() (a JSON array
/// of these), never persisted standalone. Orchestrator materializes one TaskNode per spec.
class DynamicTaskSpec
{
public:
    DynamicTaskSpec() = default;

    void set_task_ref(std::string task_ref)
    {
        m_task_ref = std::move(task_ref);
    }
    ...
    [[nodiscard]] const std::string& get_task_ref() const noexcept
    {
        return m_task_ref;
    }
