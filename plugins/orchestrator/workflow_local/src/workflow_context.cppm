...
/// @brief Runtime dependency bundle for the moved DAG orchestrator — the subset of the engine's old
/// `EngineContext` the orchestrator actually uses: the shared `Connector`, the resolved "lua"
/// `IBridge` (edge/loop/event conditions via LuaEval + SystemTaskExecutor), and the search provider
/// (SummaryProjector's terminal-transition projections). Built once in the plugin's on_load from the
/// host callbacks. Named `WorkflowContext` so the moved `Orchestrator`/`SummaryProjector` code reads
/// the same as before (the sed rename from `EngineContext`).
class WorkflowContext {
  public:
    WorkflowContext() = default;

    /// @brief Wires in the shared connector. @param connector the connector for all DB/cache ops.
    ...
    /// @brief Wires in the resolved search provider. @param provider the ISearchProvider*, or nullptr.
...
class FakeSearchProvider final : public interfaces::ISearchProvider {
  public:
    [[nodiscard]] std::string_view backend_name() const noexcept override { return "fake_search"; }
    void index(std::string_view /*collection*/, std::string_view /*id*/,
               std::string_view /*document_json*/, shared::QueryReadFn &&callback) noexcept override {
        callback("ok");
    }
