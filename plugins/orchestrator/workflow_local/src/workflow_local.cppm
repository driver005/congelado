...
/// @brief The local (in-process) workflow-orchestrator backend — now the sole home of the DAG brain
/// (moved wholesale out of the engine). Wraps the moved `engine::Orchestrator` (the DAG walker) +
/// `engine::WorkflowContext` (connector/lua/search), translating the async string/map
...
class WorkflowLocal final : public interfaces::IWorkflowOrchestrator {
  public:
    /**
     * @brief Wires the backend from the host callbacks and registers the sweep contract. Called once
     * by the plugin's `on_load`. Takes opaque pointers so the plugin's entry `.cc` never imports
     * connector/contract types directly.
     ...
     * @param registry_ctx the host `core::contract::ContractRegistry*` (auto-release at shutdown).
     */
    void on_load(void *connector_ctx, void *lua_ctx, void *search_ctx, void *controller_ctx,
                 void *registry_ctx) {
        ...
        if (group == nullptr || registry == nullptr) {
            core::logger::error("workflow_orchestrator.local",
    ...
    void start_workflow(std::string_view def_name,
                        const std::unordered_map<std::string, std::string> &variables,
                        std::move_only_function<void(std::optional<std::string>)> callback) override {
        m_orchestrator.start(std::string{def_name}, variables, std::nullopt,
                             [callback = std::move(callback)](
                                 std::optional<model::WorkflowExecution> exec) mutable {
                                 callback(exec ? std::optional<std::string>{std::format(
                                                     "{}", exec->get_exec_id())}
                                               : std::nullopt);
                             });
    }

    void on_task_terminal(std::string_view task_id,
                          std::move_only_function<void(bool)> callback) override {
        m_context.get_connector().find<model::TaskInstance>(
            std::string{task_id}, [this, callback = std::move(callback)](
    ...
    void on_execution_terminal(std::string_view exec_id,
                               std::move_only_function<void(bool)> callback) override {
        m_context.get_connector().find<model::WorkflowExecution>(
            std::string{exec_id}, [this, callback = std::move(callback)](
                                      std::optional<model::WorkflowExecution> exec) mutable {
                if (!exec) {
                    callback(false);
                    return;
    ...
    void terminate(std::string_view exec_id,
                   std::move_only_function<void(bool)> callback) override {
        m_orchestrator.terminate(std::string{exec_id}, std::move(callback));
    }
    void reconcile(std::string_view exec_id,
                   std::move_only_function<void(bool)> callback) override {
        m_orchestrator.reconcile(std::string{exec_id}, std::move(callback));
    }

    void rerun(std::string_view exec_id, std::string_view node_ref,
               const interfaces::Value &input,
               std::move_only_function<void(bool)> callback) override {
        m_orchestrator.rerun(std::string{exec_id}, std::string{node_ref}, input,
                             std::move(callback));
    }

    void signal(std::string_view exec_id, std::string_view node_ref,
                std::optional<std::string_view> payload,
                std::move_only_function<void(bool)> callback) override {
        auto payload_owned =
            payload ? std::optional<std::string>{std::string{*payload}} : std::nullopt;
        m_orchestrator.signal(std::string{exec_id}, std::string{node_ref}, std::move(payload_owned),
                              std::move(callback));
    }

    void complete_task(std::string_view exec_id, std::string_view node_ref, bool success,
        ...
        // unconditionally right after, regardless of whether anything was actually found/updated
        // — so a caller of this IWorkflowOrchestrator::complete_task() (e.g. `POST
        // /api/v1/queue/update` via TaskHandler::queue_update()) gets a false-positive success
        // for a bogus exec_id/node_ref pair.
        m_orchestrator.queue_update(std::string{exec_id}, std::string{node_ref},
                                    success ? model::TaskStatus::COMPLETED
...
class WorkflowLocalFixture {
  public:
    WorkflowLocalFixture() { m_connector.set_cache(&m_cache); }
...
suite<"WorkflowLocal::on_task_terminal / on_execution_terminal"> workflow_local_terminal_suite = [] {
    "on_task_terminal reports false for an unknown task id"_test = [] {
        ...
        bool result = true;
        local.on_task_terminal(std::format("{}", model::generate_id()),
    ...
    "on_task_terminal reports true and re-finds a real instance by id"_test = [] {
        ...
        bool result = false;
        local.on_task_terminal(std::format("{}", instance.get_task_id()),
                               [&result](bool ok) { result = ok; });
        expect(result);
    };
    ...
    "on_execution_terminal reports false for an unknown exec id"_test = [] {
        ...
        bool result = true;
        local.on_execution_terminal(std::format("{}", model::generate_id()),
                                    [&result](bool ok) { result = ok; });
        expect(!result);
    };
    ...
    "on_execution_terminal reports true for a real execution"_test = [] {
        ...
        bool result = false;
        local.on_execution_terminal(std::format("{}", exec.get_exec_id()),
                                    [&result](bool ok) { result = ok; });
        expect(result);
    };
};
...
suite<"WorkflowLocal lifecycle wrappers forward to Orchestrator"> workflow_local_lifecycle_suite = [] {
    "pause/resume/retry/restart/reconcile all report false for an unknown exec id"_test = [] {
        ...
        local.pause(missing, [&](bool ok) { pause_result = ok; });
        local.resume(missing, [&](bool ok) { resume_result = ok; });
        local.retry(missing, [&](bool ok) { retry_result = ok; });
        local.restart(missing, [&](bool ok) { restart_result = ok; });
        local.reconcile(missing, [&](bool ok) { reconcile_result = ok; });
    ...
    "rerun reports false for an unknown exec id"_test = [] {
        ...
        bool result = true;
        local.rerun(std::format("{}", model::generate_id()), "some_ref",
                   interfaces::Value{std::string{"x"}}, [&result](bool ok) { result = ok; });
        expect(!result);
    };
    ...
    "signal reports false when there's no IN_PROGRESS instance for that node_ref"_test = [] {
        ...
        bool result = true;
        local.signal(std::format("{}", model::generate_id()), "some_ref", std::nullopt,
                    [&result](bool ok) { result = ok; });
        expect(!result);
    };
};
...
suite<"WorkflowLocal::complete_task"> workflow_local_complete_task_suite = [] {
    ...
    "BUG: reports true even for a bogus exec_id/node_ref that updates nothing"_test = [] {
        ...
        bool result = false;
        local.complete_task(std::format("{}", model::generate_id()), "no_such_node", true, {},
                           [&result](bool ok) { result = ok; });
