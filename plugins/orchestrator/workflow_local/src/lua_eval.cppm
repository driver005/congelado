...
class LuaEval {
  public:
     ...
     * variables — keys of the object value, each bound as its native Lua type (boolean/integer/
    ...
    eval_condition(std::string_view expr,
                   const serde::Value &bindings) const {
  ...
  private:
    ...
    lua_State *load_and_run(std::string_view expr,
        ...
        // Push every entry of the object value as a Lua global before evaluating — each mapped to
        // its native Lua type via push_value, so nested structures work instead of the old
        ...
        // POST /api/v1/workflows/:name/start. `state` is shared with lua_bridge's native_handle()
        // — if lua_bridge_plugin.cc's run_script() (which calls luaL_openlibs) ever ran first on
        // this same interpreter, the full Lua stdlib (os.execute, io.open, os.remove) becomes
        // reachable from here too, turning this into unauthenticated OS command execution.
    ...
    static void push_value(lua_State *state, const serde::Value &value) {
        if (value.is_null()) {
            ...
            lua_newtable(state);
            for (auto const &[key, child] : *as_object) {
...
/// @brief Minimal `IBridge` test double modeling a resolved "lua" bridge whose `native_handle()`
/// hands back a real, bare `lua_State*` the test owns directly (via `luaL_newstate()`, same
/// bare-interpreter construction pattern lua_bridge_plugin.cc's own tests use) — lets
/// `LuaEval::load_and_run()` see a genuine native handle without standing up a whole
/// `LuaBridgePlugin`/`congelado::Plugin` instance.
class MockLuaBridge final : public interfaces::IBridge {
  public:
    ...
    void install_method(std::unique_ptr<FnContext> /*ctx*/,
...
/// (empty) bindings is one hand-injected C function — no `luaL_openlibs()` call anywhere in this
/// test, so `os`/`io`/every stdlib table is unreachable — and `eval_condition` still evaluates a
/// full expression that (a) *calls* that function as a side effect and (b) combines the call
/// with ordinary arithmetic/comparison, then the mutation the C function made to an unrelated
/// Lua global (never passed in as a binding, invisible to the expression's own return value) is
/// read back through the raw `lua_State` afterward. A sandboxed/allow-listed evaluator would
/// either reject the function call outright or keep the expression from having any observable
/// side effect beyond its own result — neither restriction exists here, which is exactly the
/// "unauthenticated OS command execution" reachability the SECURITY comment above load_and_run()
/// describes (this test just proves the *mechanism*, without ever touching a real destructive
/// call like os.execute/io.open).
