...
/// @brief The connector-touching half of the local orchestrator backend, kept in a module TU so the
/// plugin's own `.cc` never imports `connector` directly (that import crashes clang's modules in a
/// plugin entry TU — the engine and the db_query worker dodge it the same way). Every method casts
/// the opaque `connector_ctx` the host injected and runs the same async connector ops the engine's
/// built-in Orchestrator uses, delivering results through the caller's callback. Shares the engine's
/// task store (the same shared Connector), so tasks the engine enqueues are claimable here.
class OrchestratorStore {
  public:
    /// @brief Enqueues a SCHEDULED TaskInstance for `task_type`, delivering its id.
    static void enqueue(void *connector_ctx, std::string task_type, const serde::Value &input,
    ...
    /// @brief Claims the oldest SCHEDULED instance for `worker_type` (optional `domain`), flips it to
    /// IN_PROGRESS, and delivers it serialized as JSON. std::nullopt = empty queue.
    static void claim(void *connector_ctx, std::string worker_type,
        ...
        conn.find_first<model::TaskInstance>(
            ...
            [&conn, callback = std::move(callback)](
                ...
                conn.find<model::TaskDef>(
                    claimed.get_def_name(), [&conn, claimed, callback = std::move(callback)](
    ...
    static void submit_result(void *connector_ctx, std::string task_id, bool success,
        ...
        conn.find<model::TaskInstance>(
            std::move(task_id), [&conn, success, output = std::move(output),
                ...
                found->set_status(success ? model::TaskStatus::COMPLETED
                                          : model::TaskStatus::FAILED);
                found->set_output_data(std::move(output));
                auto updated = std::move(*found);
                conn.update<model::TaskInstance>(
                    updated, [callback = std::move(callback)](bool oke) mutable { callback(oke); });
            });
    }

    /// @brief Ready-queue depth for `worker_type`.
    static void queue_size(void *connector_ctx, std::string worker_type,
        ...
        conn.find_all<model::TaskDef>(
            [&conn, worker_type, callback = std::move(callback)](
                std::vector<model::TaskDef> defs) mutable {
                ...
                conn.find_all<model::TaskInstance>(
                    [names, callback = std::move(callback)](
                        std::vector<model::TaskInstance> instances) mutable {
                        std::size_t count = 0;
    ...
    static void requeue(void *connector_ctx, std::string worker_type,
                        std::move_only_function<void(std::size_t)> callback) {
        if (connector_ctx == nullptr) {
            callback(0);
            return;
    ...
    /// callers.
    static void start_workflow(void *connector_ctx, std::string def_name,
        ...
        conn.find<model::WorkflowDef>(
            def_name, [&conn, variables = std::move(variables), callback = std::move(callback)](
                ...
                conn.insert<model::WorkflowExecution>(
                    ...
                    [exec_id = std::move(exec_id), callback = std::move(callback)](bool oke) mutable {
                        callback(oke ? std::optional<std::string>{std::move(exec_id)}
                                     : std::nullopt);
                    });
            });
    }
...
class StoreFixture {
  public:
    StoreFixture() { m_connector.set_cache(&m_cache); }

    [[nodiscard]] connector::Connector &get_connector() noexcept { return m_connector; }
    [[nodiscard]] void *ctx() noexcept { return &m_connector; }
...
suite<"OrchestratorStore::enqueue"> store_enqueue_suite = [] {
    "a nullptr connector_ctx reports std::nullopt"_test = [] {
        ...
        OrchestratorStore::enqueue(nullptr, "echo", serde::Value{serde::Value::Object{}},
    ...
    "inserts a SCHEDULED TaskInstance and hands back its id"_test = [] {
        ...
        OrchestratorStore::enqueue(fixture.ctx(), "echo", serde::Value{std::move(input)},
                                   [&result](std::optional<std::string> id) { result = id; });

        expect(result.has_value()) << fatal;
        std::optional<model::TaskInstance> found;
        fixture.get_connector().find<model::TaskInstance>(
            *result, [&found](std::optional<model::TaskInstance> value) { found = std::move(value); });
...
suite<"OrchestratorStore::claim"> store_claim_suite = [] {
    "a nullptr connector_ctx reports std::nullopt"_test = [] {
        std::optional<std::string> result{"unset"};
        OrchestratorStore::claim(nullptr, "echo", std::nullopt,
                                 [&result](std::optional<std::string> value) { result = value; });
        expect(!result.has_value());
    };
    ...
    "an empty queue reports std::nullopt"_test = [] {
        StoreFixture fixture;
        std::optional<std::string> result{"unset"};
        OrchestratorStore::claim(fixture.ctx(), "echo", std::nullopt,
                                 [&result](std::optional<std::string> value) { result = value; });
        expect(!result.has_value());
    };
    ...
    "IN_PROGRESS, returns it serialized"_test = [] {
        ...
        fixture.get_connector().find<model::TaskInstance>(
            ...
            [&updated](std::optional<model::TaskInstance> value) { updated = std::move(value); });
    ...
    "a worker_type that doesn't match any def's worker_type reports std::nullopt"_test = [] {
        ...
        std::optional<std::string> result{"unset"};
        OrchestratorStore::claim(fixture.ctx(), "other_worker_type", std::nullopt,
                                 [&result](std::optional<std::string> value) { result = value; });
        expect(!result.has_value());
    };
    ...
    "a domain filter only matches instances whose def's domain agrees exactly"_test = [] {
        ...
        OrchestratorStore::claim(
            ...
            [&wrong_domain](std::optional<std::string> value) { wrong_domain = value; });
        expect(!wrong_domain.has_value());

        std::optional<std::string> right_domain;
        OrchestratorStore::claim(
            fixture.ctx(), "echo", std::optional<std::string>{"tenant_a"},
            [&right_domain](std::optional<std::string> value) { right_domain = value; });
    ...
    "already-claimed (non-SCHEDULED) instances are never re-claimed"_test = [] {
        ...
        OrchestratorStore::claim(fixture.ctx(), "echo", std::nullopt,
                                 [&first](std::optional<std::string> value) { first = value; });
        expect(first.has_value()) << fatal;

        std::optional<std::string> second{"unset"};
        OrchestratorStore::claim(fixture.ctx(), "echo", std::nullopt,
                                 [&second](std::optional<std::string> value) { second = value; });
...
suite<"OrchestratorStore::submit_result"> store_submit_result_suite = [] {
    "a nullptr connector_ctx reports false"_test = [] {
        bool result = true;
        OrchestratorStore::submit_result(nullptr, "some-id", true, {},
    ...
    "an unknown task id reports false"_test = [] {
        StoreFixture fixture;
        bool result = true;
        OrchestratorStore::submit_result(fixture.ctx(), std::format("{}", model::generate_id()),
                                         true, {}, [&result](bool ok) { result = ok; });
        expect(!result);
    };
    ...
    "success=true records COMPLETED with the given output"_test = [] {
        ...
        bool result = false;
        OrchestratorStore::submit_result(fixture.ctx(), std::format("{}", instance.get_task_id()),
                                         true, {{"message_id", "abc"}},
                                         [&result](bool ok) { result = ok; });
        expect(result);
...
suite<"OrchestratorStore::queue_size"> store_queue_size_suite = [] {
    "a nullptr connector_ctx reports 0"_test = [] {
        ...
        OrchestratorStore::queue_size(nullptr, "echo", [&result](std::size_t count) { result = count; });
    ...
    "counts only SCHEDULED instances whose def's worker_type matches"_test = [] {
        ...
        std::size_t result = 0;
        OrchestratorStore::queue_size(fixture.ctx(), "echo",
                                      [&result](std::size_t count) { result = count; });
...
suite<"OrchestratorStore::requeue"> store_requeue_suite = [] {
    "a nullptr connector_ctx reports 0"_test = [] {
        std::size_t result = 99;
        OrchestratorStore::requeue(nullptr, "echo", [&result](std::size_t count) { result = count; });
        expect(result == std::size_t{0});
    };
    ...
    "flips IN_PROGRESS instances of the matching worker_type back to SCHEDULED"_test = [] {
        ...
        std::size_t result = 0;
        OrchestratorStore::requeue(fixture.ctx(), "echo",
                                   [&result](std::size_t count) { result = count; });
        expect(result == std::size_t{1});
...
suite<"OrchestratorStore::start_workflow"> store_start_workflow_suite = [] {
    "a nullptr connector_ctx reports std::nullopt"_test = [] {
        std::optional<std::string> result{"unset"};
        OrchestratorStore::start_workflow(nullptr, "order_pipeline", {},
                                          [&result](std::optional<std::string> id) { result = id; });
        expect(!result.has_value());
    };
    ...
    "engine-side"_test = [] {
        ...
        fixture.get_connector().find<model::WorkflowExecution>(
            *result, [&exec](std::optional<model::WorkflowExecution> value) { exec = std::move(value); });
