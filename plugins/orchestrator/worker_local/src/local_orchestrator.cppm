...
/// @brief The local (in-process, connector-backed) worker_orchestrator backend — the engine-side
/// dispatch/orchestration capability, symmetric to the external worker manager. Implements the async
/// IWorkerOrchestrator by delegating to the worker_orchestrator_local_store module (which owns all
/// connector/model contact, keeping this free of direct connector plumbing). Shares the engine's
/// task store via the host-injected connector, so it claims the same tasks the engine enqueues.
class LocalOrchestrator final : public interfaces::IWorkerOrchestrator {
  public:
    ...
    void enqueue(std::string_view task_type, const interfaces::Value &input,
                 std::move_only_function<void(std::optional<std::string>)> callback) override {
        OrchestratorStore::enqueue(m_connector_ctx, std::string{task_type}, input,
                                   std::move(callback));
    }

    void claim(std::string_view worker_type, std::optional<std::string_view> domain,
               std::move_only_function<void(std::optional<std::string>)> callback) override {
        auto domain_owned =
            domain ? std::optional<std::string>{std::string{*domain}} : std::nullopt;
        OrchestratorStore::claim(m_connector_ctx, std::string{worker_type}, std::move(domain_owned),
                                 std::move(callback));
    }
    ...
    void submit_result(std::string_view task_id, bool success,
        ...
        OrchestratorStore::submit_result(m_connector_ctx, std::string{task_id}, success, output,
                                         std::move(callback));
    }

    void requeue(std::string_view worker_type,
                 std::move_only_function<void(std::size_t)> callback) override {
        OrchestratorStore::requeue(m_connector_ctx, std::string{worker_type}, std::move(callback));
    }

    void queue_size(std::string_view worker_type,
                    std::move_only_function<void(std::size_t)> callback) override {
        OrchestratorStore::queue_size(m_connector_ctx, std::string{worker_type},
                                      std::move(callback));
    }
    ...
    void start_workflow(std::string_view def_name,
                        ...
                        std::move_only_function<void(std::optional<std::string>)> callback) override {
        OrchestratorStore::start_workflow(m_connector_ctx, std::string{def_name}, variables,
                                          std::move(callback));
    }
    ...
    void set_health_callback(
        std::move_only_function<void(const interfaces::OrchestratorInfo &)> callback) override {
    ...
    "publishes a health snapshot"_test = [] {
        ...
        expect(nothrow([&] {
            orchestrator.set_health_callback(
                [&invoked](const interfaces::OrchestratorInfo &) { invoked = true; });
        }));
        expect(!invoked);
    };
};
...
suite<"LocalOrchestrator with no connector wired"> local_orchestrator_no_connector_suite = [] {
    ...
    "claim reports std::nullopt"_test = [] {
        LocalOrchestrator orchestrator;
        std::optional<std::string> result{"unset"};
        orchestrator.claim("echo", std::nullopt,
                           [&result](std::optional<std::string> id) { result = id; });
        expect(!result.has_value());
    };
    ...
    "submit_result reports false"_test = [] {
        LocalOrchestrator orchestrator;
        bool result = true;
        orchestrator.submit_result("some-id", true, {}, [&result](bool ok) { result = ok; });
        expect(!result);
    };
    ...
    "requeue/queue_size report 0"_test = [] {
        ...
        orchestrator.requeue("echo", [&requeue_result](std::size_t n) { requeue_result = n; });
        orchestrator.queue_size("echo", [&queue_size_result](std::size_t n) { queue_size_result = n; });
    ...
    "start_workflow reports std::nullopt"_test = [] {
        LocalOrchestrator orchestrator;
        std::optional<std::string> result{"unset"};
        orchestrator.start_workflow("order_pipeline", {},
                                    [&result](std::optional<std::string> id) { result = id; });
        expect(!result.has_value());
    };
};
...
suite<"LocalOrchestrator delegates to a wired connector"> local_orchestrator_wired_suite = [] {
    "set_connector_ctx wires a real connector through to enqueue/claim/submit_result"_test = [] {
        ...
        orchestrator.enqueue("send_email", interfaces::Value{serde::Value::Object{}},
                             [&enqueued_id](std::optional<std::string> id) { enqueued_id = id; });
        expect(enqueued_id.has_value()) << fatal;

        std::optional<std::string> claimed;
        orchestrator.claim("echo", std::nullopt,
                           [&claimed](std::optional<std::string> value) { claimed = value; });
        expect(claimed.has_value()) << fatal;

        bool submit_ok = false;
        orchestrator.submit_result(*enqueued_id, true, {{"k", "v"}},
                                   [&submit_ok](bool ok) { submit_ok = ok; });
        expect(submit_ok);
    };
