 ...
 * imports `connector` directly (that import crashes clang's modules in a plugin entry TU — every
 * other connector-touching plugin dodges it the same way).
 ...
 * and abort.
 * @param connector_ctx the host's `connector_ctx` (a `connector::Connector*`, opaque as `void*`).
 ...
 */
bool register_connector_contract(void *connector_ctx,
    ...
    shared_connector->set_wake([c = db_contract]() mutable { c.schedule(); });
...
suite<"postgres::register_connector_contract"> register_connector_contract_suite = [] {
    ...
    "null contract_group returns false and registers nothing"_test = [] {
        ...
        auto result =
            postgres::register_connector_contract(&shared_connector, nullptr, &registry);
    ...
    "all valid pointers registers the contract and returns true"_test = [] {
        ...
        auto result =
            postgres::register_connector_contract(&shared_connector, &group, &registry);
