...
/// `serde::Ser::from_value` — see the `Serializable<EmailInput>` specialization below for the
/// field list. `smtp_url` is required (checked after parsing); `username`/`password` default to
/// empty.
class EmailInput
{
public:
    void setSmtpUrl(std::string value)
    {
        m_smtp_url = std::move(value);
    }
    ...
    [[nodiscard]] const std::string& getSmtpUrl() const noexcept
    {
        return m_smtp_url;
    }
...
private:
    ...
    // is `std::optional`, so `serde::Ser::from_value` requires BOTH present in the task input
    // or the WHOLE decode fails, despite the doc comment above claiming they "default to
    // empty". A task that omits either key (a very natural thing to do for
    // anonymous/unauthenticated relay) gets a hard parse error instead of the documented empty
    // default.
...
/// system task, generalized to any SMTP server). Reusable IWorker. Input: `smtp_url` (required,
/// e.g. `smtps://smtp.example.com:465`), `from`, `to`, `subject`, `body`, optional
/// `username`/`password`. Output: `email_status` ("ok"/"error") + `error`.
...
private:
    ...
    )
    {
        ...
        // SECURITY: `to`/`from`/`subject` are attacker-influenceable task input, spliced
        // directly into a raw RFC-5322 header block with no CRLF stripping/validation. A `to`
        // or `subject` value containing "\r\nBcc: attacker@evil.example" (or any other header
        // line) is classic email header injection — it rides straight through into the message
        // libcurl uploads.
        Upload upload{
            .payload = "To: " + to + "\r\nFrom: " + from + "\r\nSubject: " + subject + "\r\n\r\n" +
                       body + "\r\n",
        ...
        // SECURITY (SSRF-shaped): `smtp_url` is task input handed straight to CURLOPT_URL with
        // no allowlist/validation — a task can point this worker at any host:port reachable
        // from the process (internal SMTP relays, or any other service that speaks enough of
        // the SMTP handshake to not immediately error out), enabling internal network
        // probing/relay abuse via a zero-auth task submission. No host allowlist exists
    ...
    static std::size_t
    read_callback(char* buffer, std::size_t size, std::size_t count, void* userdata)
    {
...
// run() once smtp_url is non-empty — at which point run() unconditionally calls
// curl_easy_perform(), a real blocking network attempt with no injectable transport seam (unlike
// core_client's IProtocol seam used by the client/client_pool workers). There is no way to reach
// that code from a test without either a live SMTP endpoint or a real outbound connection attempt,
// both forbidden by this repo's test-safety rules. That's a legitimate, documented skip for
// send_blocking/read_callback specifically. Everything reachable WITHOUT triggering a network call
// — EmailInput parsing, and run()'s two guard clauses (missing 'smtp_url' key entirely, and
// present-but-empty) — is fully tested below.
#ifdef CONGELADO_TEST
