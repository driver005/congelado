...
/// `serde::Ser::from_value` — see the `Serializable<ClientInput>` specialization below. `header.<Name>`
/// keys don't fit a fixed field list, so they stay a manual scan over the raw input object in
/// `ClientWorker::run`. `method` defaults to "GET" via its in-class member initializer; `path` is
...
class ClientInput {
  public:
    ...
    void setBody(std::optional<std::string> value) { m_body = std::move(value); }
    void setContentType(std::optional<std::string> value) { m_content_type = std::move(value); }
    ...
    [[nodiscard]] const std::optional<std::string> &getBody() const noexcept { return m_body; }
...
struct serde::Serializable<worker_client::ClientInput> {
    static constexpr auto fields() {
        ...
        return std::tuple{
            ...
            serde::FieldDesc<"content_type", &ClientInput::getContentType,
...
/// `core::client::Register` (owned by this worker) ships it and correlates the response. The worker
/// owns its own outbound HTTP/2 client, built via the host-injected `interfaces::IProtocol` and
/// connected once at load. Single fixed endpoint per deployment, so input is `path` (not a full
/// url). Input: `path` (required), `method` (default GET), `body`, `content_type`, `header.<Name>`
/// keys. Output: `status` (HTTP code), `body`, `http_status` ("ok"/"error").
class ClientWorker final : public interfaces::IWorker {
  public:
    [[nodiscard]] std::string_view get_task_type() const noexcept override { return "http"; }
    ...
    void connect_downstream(io::base::socket::Endpoint endpoint, bool verify_peer) {
        ...
        if (!connect_result) {
            core::logger::error("client_worker", "downstream connect failed: {}",
    ...
    void run(const serde::Value &input,
        ...
        if (parsed->getContentType().has_value()) {
            builder.add_header(interfaces::io::types::Token::CONTENT_TYPE, *parsed->getContentType());
        }
        ...
        // attacker-controlled (fixed at load from this plugin's own config), so this isn't SSRF,
        // but it is an unvalidated-header-injection-shaped gap onto whatever the operator pointed
        // this worker at.
        ...
        m_register.send(std::move(request), [on_complete = std::move(on_complete)](
            ...
            on_complete(interfaces::WorkerOutput{
                {"http_status", "ok"},
...
/// @brief Builds a `serde::Value` straight from a JSON literal — same recipe used by every other
/// worker's inline tests in this repo (see hash_worker.cppm).
...
suite<"ClientInput"> client_input_suite = [] {
    ...
    // BUG: pins the finding documented above ClientInput::m_method — omitting "method" fails the
    // whole decode despite the doc comment claiming it defaults to GET.
    ...
    "from_value succeeds when every field is present"_test = [] {
        auto value = make_value(
            R"({"path":"/foo","method":"POST","body":"b","content_type":"text/plain"})");
        auto parsed = serde::Ser::from_value<ClientInput>(value);
        expect(parsed.has_value()) << fatal;
        expect(parsed->getPath() == "/foo");
