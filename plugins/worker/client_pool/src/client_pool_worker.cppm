...
/// @brief Typed input for the `client_pool` worker, parsed from the task's dynamic input value
/// via `serde::Ser::from_value` — see the `Serializable<ClientPoolInput>` specialization below.
/// `header.<Name>` keys don't fit a fixed field list, so they stay a manual scan over the raw
/// input object in `ClientPoolWorker::run`.
/// `method`/`expected_status`/`interval_ms`/`max_attempts` default via in-class member
/// initializers; `path` is required (checked after parsing).
...
public:
    ...
    void setExpectedStatus(int value) noexcept
    {
        m_expected_status = value;
    }

    void setIntervalMs(long value) noexcept
    {
        m_interval_ms = value;
    }

    void setMaxAttempts(int value) noexcept
    {
    ...
    [[nodiscard]] int getExpectedStatus() const noexcept
    {
        return m_expected_status;
    }

    [[nodiscard]] long getIntervalMs() const noexcept
    {
        return m_interval_ms;
    }

    [[nodiscard]] int getMaxAttempts() const noexcept
    {
...
private:
    ...
    // four fields is `std::optional`, so `serde::Ser::from_value` requires ALL of them present
    // in the task input or the WHOLE decode fails; these in-class defaults are only ever
    // reached via direct C++ construction, never via task input that omits the key. See the
    // pinning tests below.
    std::string m_method{"GET"};
    int m_expected_status{200};
    long m_interval_ms{1'000};
    // SECURITY: `max_attempts`/`interval_ms` come straight from zero-auth task input with no
    // upper/lower bound check — a task can set max_attempts to a huge number and interval_ms to
    // 0 to hammer the fixed downstream as fast as the event loop allows, indefinitely, or set
...
/// `client` worker: repeatedly requests the host-configured downstream service until it returns
/// the expected status (or attempts run out). Owns its own outbound HTTP/2 client, built via
/// the host-injected `interfaces::IProtocol` and connected once at load. Retries are driven by
/// the host's io_uring leverager timer — no thread ever blocks or sleeps. Input: `path`
/// (required), `method` (GET), `expected_status` (200), `interval_ms` (1000), `max_attempts`
/// (5), `header.<Name>` keys. Output: `status` (last HTTP code), `body`, `attempts`,
/// `client_pool_status`
...
public:
    ...
    /// @brief Injects the host's io_uring leverager, needed to connect this worker's own client
    /// and to drive the retry timer.
    ...
    void run(const serde::Value& input, interfaces::WorkerCompletion on_complete) override
    {
        ...
        // — see the SECURITY note there. Host/port aren't attacker-controlled here either
        // (fixed at load), only header name/value and path are.
...
private:
    /// @brief One in-flight poll's retry bookkeeping — kept alive by shared ownership across
    /// every attempt so a concurrent second task on this same worker doesn't share state with
    /// it.
    class RetryState
    {
    public:
        ...
        void set_expected_status(int status) noexcept
        {
            m_expected_status = status;
        }

        void set_interval_ms(long interval_ms) noexcept
        {
            m_interval_ms = interval_ms;
        }

        void set_max_attempts(int max_attempts) noexcept
        {
        ...
        [[nodiscard]] int get_expected_status() const noexcept
        {
            return m_expected_status;
        }

        [[nodiscard]] long get_interval_ms() const noexcept
        {
            return m_interval_ms;
        }

        [[nodiscard]] int get_attempt() const noexcept
        {
            return m_attempt;
        }
        ...
        [[nodiscard]] __kernel_timespec& get_timeout_spec() noexcept
        {
            return m_timeout_spec;
        }

        /// @brief Whether another attempt is allowed after this one.
        [[nodiscard]] bool has_more_attempts() const noexcept
        {
            return m_attempt < m_max_attempts;
        }

        /// @brief Counts the attempt that just finished.
        void increment_attempt() noexcept
        {
            ++m_attempt;
        }
...
suite<"ClientPoolInput"> client_pool_input_suite = [] {
    ...
    "default-constructed fields match the documented defaults"_test = [] {
        ...
        expect(input.getIntervalMs() == 1'000L);
