...
/// `serde::Ser::from_value` — see the `Serializable<LlmInput>` specialization below.
/// `api_url`/`model` default via in-class member initializers (used whenever the object has no
/// such key); `prompt` is required (checked after parsing).
class LlmInput
{
public:
    void setApiUrl(std::string value)
    {
        m_api_url = std::move(value);
    }
    ...
    {
        m_prompt = std::move(value);
    }

    [[nodiscard]] const std::string& getApiUrl() const noexcept
    {
        return m_api_url;
    }

    [[nodiscard]] const std::string& getApiKey() const noexcept
    {
        return m_api_key;
    }
    ...
    [[nodiscard]] const std::string& getPrompt() const noexcept
    {
        return m_prompt;
    }
...
private:
    ...
    // initializers". A caller relying on "omit api_url to get OpenAI's default" gets a hard
    // parse error instead.
...
/// @brief The `llm` worker — calls an OpenAI-compatible chat-completions endpoint (Conductor's
/// LLM text/chat system tasks), as a reusable IWorker via libcurl. Input: `prompt` (required),
/// `api_key`, `model` (default gpt-4o-mini), `api_url` (default OpenAI's). Output: `response`
/// (raw JSON the caller parses), `status` (HTTP code), `llm_status`. Kept minimal on purpose —
/// a single-turn chat call; richer AI tasks (embeddings, vector search) are their own workers
/// later.
...
public:
    ...
    /// dedicated TaskQueue contract (see IWorker::run), so the blocking `curl_easy_perform`
    /// call never holds up the caller or the shared contract pool beyond this one contract.
    // BUG: unlike worker_email::EmailWorker::run() (which explicitly rejects an empty
    // 'smtp_url' before ever touching curl), this run() has NO post-parse validation at all —
    // an empty (but present) 'prompt', or an api_url/api_key/model that are empty strings, all
    // sail straight through to send_blocking() and a real outbound call. The only thing that
    // stops a call from firing is from_value() itself failing (a required key missing
    // outright).
    void run(const serde::Value& input, interfaces::WorkerCompletion on_complete) override
    {
        ...
        on_complete(send_blocking(
            parsed->getApiUrl(), parsed->getApiKey(), parsed->getModel(), parsed->getPrompt()
        ));
    }
...
private:
    ...
    /// contract.
    static interfaces::WorkerResult send_blocking(
    ...
    )
    {
        ...
        // json_escape(). A model value containing a `"` breaks out of the JSON string and lets
        // a task submitter inject arbitrary extra JSON fields/messages into the request body
        // sent upstream (e.g. `model` =
        // `x","messages":[{"role":"system","content":"..."}],"x":"`).
        ...
        // allowlist/validation — worse than client/client_pool's fixed-at-load host, this one
        // is fully attacker-chosen PER TASK (defaults to OpenAI's endpoint but nothing stops a
        // task from overriding it). A task can point this worker at an internal service, a
        // cloud metadata endpoint (e.g. 169.254.169.254), or anywhere else reachable from the
        // process — and it'll even carry the configured `api_key`/Authorization header there if
        // one is set. This is the clearest SSRF vector across all five plugins covered in this
        // pass.
    ...
    // bounds wall-clock time (120s) but not response size, so a slow-but-huge response can
    // still exhaust memory within that window.
    static std::size_t
    write_callback(char* pointer, std::size_t size, std::size_t count, void* userdata)
    {
...
suite<"LlmInput"> llm_input_suite = [] {
    ...
    // BUG: pins the finding documented above LlmInput's m_api_url/m_model — omitting either
    // fails the whole decode despite the doc comment claiming they default via in-class
    // initializers.
