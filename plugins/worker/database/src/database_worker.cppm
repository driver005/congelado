...
class DatabaseInput {
  public:
    void setSql(std::string value) { m_sql = std::move(value); }

    [[nodiscard]] const std::string &getSql() const noexcept { return m_sql; }
...
/// @brief A generic "a database call is a worker" primitive — runs a raw query against the injected
/// `IDatabase` backend (resolved by the worker host, same as the engine's) and returns its result.
/// Shared/reusable (lives in plugins/worker/, not inside any app). Reads `sql` from the input map
/// and returns the backend's result string under `result`. `IDatabase::query` is already
/// callback-based — no thread ever blocks.
class DatabaseWorker final : public interfaces::IWorker {
  public:
    ...
    void set_database_ctx(void *database_ctx) noexcept { m_database_ctx = database_ctx; }

    [[nodiscard]] std::string_view get_task_type() const noexcept override { return "database"; }
    ...
    void run(const serde::Value &input,
        ...
        // set via the zero-auth workflow/task API (see plugins/manager/engine/src/routes.cppm's
        // top-of-file SECURITY note on why "zero-auth" matters here). There is no allowlist, no
        // statement-type restriction (SELECT-only, say), no parameter binding, nothing: any task
        // submitter can run arbitrary SQL — including DDL/DML — against whatever backend the host
        // wired up. This worker is arbitrary-query-execution by design, not a bug in the usual
        // sense, but it's the sharpest edge in this whole file and worth flagging loudly for
        // anyone wiring `database` tasks into a workflow reachable by untrusted input.
        ...
        database->query(parsed->getSql(), [shared_complete](std::string_view rows) {
            (*shared_complete)(interfaces::WorkerOutput{{"db_status", "ok"}, {"result", std::string{rows}}});
        });
    }
...
suite<"DatabaseWorker"> database_worker_suite = [] {
    ...
    // finding documented above DatabaseWorker::run(). A destructive statement goes through exactly
    // like a SELECT would.
