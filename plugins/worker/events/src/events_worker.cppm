...
/// @brief Typed input for the `events` worker's fixed field, parsed from the task's dynamic input
/// value via `serde::Ser::from_value` — see the `Serializable<EventsInput>` specialization below.
/// Every OTHER input key becomes payload and doesn't fit a fixed field list, so `EventsWorker::
/// execute` still scans the raw input object for those.
class EventsInput {
  public:
    void setEventName(std::string value) { m_event_name = std::move(value); }

    [[nodiscard]] const std::string &getEventName() const noexcept { return m_event_name; }
...
struct serde::Serializable<worker_events::EventsInput> {
    static constexpr auto fields() {
        ...
        return std::tuple{
            serde::FieldDesc<"event_name", &EventsInput::getEventName,
                             &EventsInput::setEventName>{},
        };
    }
};
...
/// (generalizes Conductor's KAFKA_PUBLISH / event-publish system tasks). It links no broker library:
/// it calls `core::events::publish`, which the worker host fans out to every injected `IEventSink`
/// (kafka, rabbitmq, redis, memory — whichever sinks the host wired). This is the "use injected
/// infra" model — the broker/topic live in the sink's config, the worker only supplies the event
/// name + payload. Input: `event_name` (required) + any other keys become the JSON payload. Output:
/// `events_status` ("ok"/"error").
class EventsWorker final : public interfaces::IWorker {
  public:
    [[nodiscard]] std::string_view get_task_type() const noexcept override { return "events"; }
    ...
    execute(const serde::Value &input) override {
        ...
        // Everything but the control key rides through as the published payload — string values go
