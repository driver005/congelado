...
/// @brief Typed input for the `jwt` worker's fixed fields, parsed from the task's dynamic input
/// value via `serde::Ser::from_value` — see the `Serializable<JwtInput>` specialization below.
/// Every OTHER input key becomes a claim and doesn't fit a fixed field list, so
/// `JwtWorker::execute` still scans the raw input object for those.
class JwtInput
{
public:
    void setSecret(std::string value)
    {
        m_secret = std::move(value);
    }

    [[nodiscard]] const std::string& getSecret() const noexcept
    {
        return m_secret;
    }
...
/// @brief The `jwt` worker — issues a signed JSON Web Token (Conductor's Get-Signed-JWT system
/// task), as a reusable IWorker. HS256 (HMAC-SHA256) via OpenSSL. Input: `secret` (the HMAC
/// key) + every other input key becomes a string claim in the payload (e.g. `sub`, `iss`,
/// `exp`). Output: `token` (the compact JWS) + `jwt_status`.
...
public:
    ...
    [[nodiscard]] interfaces::WorkerResult execute(const serde::Value& input) override
    {
        ...
        // Build the claims payload from every input key except the reserved ones — string
        // values go in raw (matching the old flat-map behavior), anything else as its JSON
        // encoding. SECURITY: string claim keys/values are spliced straight into the payload
        // with NO JSON escaping — a claim value containing an unescaped `"` breaks out of its
        // string literal and can inject additional sibling claims into the signed payload (e.g.
        // a claim value of `x","admin":"true` becomes two top-level keys, "name" and a forged
        // "admin", in the signed JSON). See the pinning test below ("SECURITY: unescaped claim
        // values allow injecting extra top-level JSON claims").
        std::string payload = "{";
        bool first = true;
        ...
        // supplying `"secret":""` still gets back a "successfully" HMAC-signed token
        ...
        std::string token = signing_input + "." + base64url_bytes(mac.data(), mac_length);
...
/// helper, needed so a test can re-parse a produced token segment's decoded JSON text. Kept as
/// a small class (not a free function), same convention as every other test helper in this
/// repo.
...
suite<"JwtWorker"> jwt_worker_suite = [] {
    ...
    "execute produces the exact expected HS256 token for a known secret/claim"_test = [] {
        ...
        expect(
            result->at("token") == "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9."
