  ...
  public:
    ...
    void on_ready() noexcept override {
        ...
        try {
            ...
            // Hand the server the contract group each accepted connection's ServerFlow registers
            // its per-session frame executor against — must be set before any connection lands.
            if (m_contract_group != nullptr) {
...
#ifdef CONGELADO_TEST
namespace http2_plugin_tests {
using namespace boost::ut;

// NOTE on coverage gaps, deliberate: on_load()'s success path (past the "host" key check) builds
// a real io::layer::http2::Http2Protocol (which throws unless host/cert/key/port are all
// present), and on_ready()'s success path stands up a real listening socket flow — neither is
// reachable without a live TLS cert/key pair and a bindable port, so this suite only exercises
// the parts of the plugin's lifecycle that are reachable with no live server: metadata accessors,
// the class-level @warning's documented early-return in on_load() (missing "host" config key,
// entirely config-driven, no I/O), and the guard clauses in on_ready()/on_unload()/
// on_shutdown_requested() that all key off m_server/m_socket_flow never having been set.
suite<"Http2Plugin"> http2_plugin_suite = [] {
    "get_name reports 'http2'"_test = [] {
        Http2Plugin plugin;
        expect(plugin.get_name() == "http2");
    };
    ...
    "get_unique_type reports 'protocol'"_test = [] {
        Http2Plugin plugin;
        expect(plugin.get_unique_type() == "protocol");
    };

    "get_requires reports a single 'file_logger' dependency"_test = [] {
        Http2Plugin plugin;
        auto required_types = plugin.get_requires();
        expect(required_types.size() == 1_ul) << fatal;
        expect(required_types[0] == "file_logger");
    };

    "capabilities reports CONGELADO_CAP_PROTOCOL"_test = [] {
        Http2Plugin plugin;
        expect(plugin.capabilities() == CONGELADO_CAP_PROTOCOL);
    };

    "protocol_get returns nullptr before on_load ever builds a server"_test = [] {
        Http2Plugin plugin;
        expect(plugin.protocol_get() == nullptr);
    };

    "on_load with no 'host' config key bails out early without building a server"_test = [] {
        Http2Plugin plugin;
        CongeladoHostCallbacks host{};
        CongeladoConfigView cfg{.keys = nullptr, .values = nullptr, .count = 0};

        expect(nothrow([&] { plugin.on_load(host, cfg); }));
        // Confirms the class-level @warning's early-return path: m_server is left unset, so
        // protocol_get() still reports nullptr afterward.
        expect(plugin.protocol_get() == nullptr);
    };

    "on_load with unrelated config keys (still missing 'host') also bails out early"_test = [] {
        Http2Plugin plugin;
        CongeladoHostCallbacks host{};
        const char *keys[] = {"runtimes"};       // NOLINT(cppcoreguidelines-avoid-c-arrays)
        const char *values[] = {"none"};          // NOLINT(cppcoreguidelines-avoid-c-arrays)
    ...
    "on_ready before on_load is a safe no-op (m_server/m_router_ctx both unset)"_test = [] {
        Http2Plugin plugin;
    ...
    "on_shutdown_requested before on_load is a safe no-op"_test = [] {
        Http2Plugin plugin;
        expect(nothrow([&] { plugin.on_shutdown_requested(); }));
    };

    "on_shutdown_requested after a bailed-out on_load is still a safe no-op"_test = [] {
        Http2Plugin plugin;
        CongeladoHostCallbacks host{};
        CongeladoConfigView cfg{.keys = nullptr, .values = nullptr, .count = 0};
        plugin.on_load(host, cfg); // bails early, m_server stays null
