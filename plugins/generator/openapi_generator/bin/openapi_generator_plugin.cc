...
suite<"OpenApiGeneratorPlugin"> openapi_generator_plugin_suite = [] {
    ...
    "write_document(): writes the serialized document to output_path"_test = [] {
        ...
        // Same isolated-test-target property as doc_generator.cppm's own write() tests: no
        // JSON format plugin is linked here, so serde::Ser::serialize() falls back to this
        // fixed error payload -- write_document() still faithfully writes it to disk.
