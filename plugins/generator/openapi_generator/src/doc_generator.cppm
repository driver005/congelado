    ...
    [[nodiscard]] Document generate() const
    {
        ...
        for (const auto& route: routes) {
            ...
            for (const auto& [method, operation]: route.get_operations()) {
                auto method_name = to_lower(
                    interfaces::io::types::method_str(
...
private:
    ...
    resolve_full_path(const RouteMeta& route, const std::vector<RouteMeta>& all_routes)
    {
...
suite<"utils::openapi::Generator"> doc_generator_suite = [] {
    ...
    "'{param}', and an embedded '/' inside one segment stays part of that segment"_test = [] {
        ...
        expect(
            document.get_paths().at("/{taskId}/enqueue").at("get").get_summary() ==
            "docgen_test_enqueue"
        );
    };
