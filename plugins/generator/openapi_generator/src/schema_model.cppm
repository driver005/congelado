...
public:
    ...
    void set_kind(SchemaKind value) noexcept
    {
        m_kind = value;
    }
     ...
     */
    void set_items(SchemaType type)
    {
        m_items = std::make_shared<SchemaType>(std::move(type));
    }

    /** @brief Gets which schema kind this node reps. @return the kind. */
    [[nodiscard]] SchemaKind get_kind() const noexcept
    {
        return m_kind;
    }

    /** @brief Gets the referenced schema name. @return bare schema name, empty if this ain't a
     * Ref. */
     ...
     */
    [[nodiscard]] const std::unordered_map<std::string, SchemaType>& get_properties() const noexcept
    {
        return m_properties;
    }
     ...
     * ever gets set by set_items()/parse() for Array, so every other kind means a null
     * shared_ptr dereference. UB with no safety net. Don't be that guy.
     * @return the array element's schema.
     */
    [[nodiscard]] const SchemaType& get_items() const noexcept
    {
        return *m_items;
    }
...
private:
     ...
     * @return the matching SchemaKind, or an L in string form if `type` isn't a name we
     * recognize.
     */
