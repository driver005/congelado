...
class RouteWriter {
  public:
     ...
     * Each generated method builds a request via `core::client::Client::custom(...).build(...)`,
     * fills in body/content-type, and dispatches through the member `Register`'s `send()`.
    ...
    write(const std::vector<OperationInfo> &operations, std::string_view routesModuleName,
        ...
        auto &set_runtime_fn = client.addMethod("void", "setRuntime");  // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor naming convention (camelCase after prefix), not a real naming defect — the shared clang-tidy config has no accessor exception
  ...
  private:
     ...
     */
    static void emit_operation(core::generator::Class &client, std::string_view groupName,
                               const OperationInfo &operation, std::size_t prefixSize,
        ...
        std::string function_name =
            std::format("{}_{}", groupName, resolve_function_name(operation.get_method(), literal_tail));
        ...
        fn.add_statement(core::generator::Stmt::raw(std::format(
            "    auto request = core::client::Client::custom(\"{}\", {}).build(m_register.runtime());\n",
        ...
        if (operation.get_response()) {
            fn.add_statement(core::generator::Stmt::raw(std::format(
                "    m_register.send(std::move(request), [on_response = std::move(onResponse), "
                ...
                "        auto result = serde::Ser::deserialize<{}>(response.get_content_type(), body);\n"
...
suite<"RouteWriter"> route_writer_suite = [] {
    "write: a single no-param GET operation renders full request wiring, no response type"_test = [] {
        ...
        expect(result->contains(
            "void tasks_get(std::function<void()> onResponse, std::function<void(std::string)> "
            "onError = [](std::string) {})"));
        expect(result->contains(
            "core::client::Client::custom(\"GET\", \"/api/v1/tasks\").build(m_register.runtime())"));
    ...
    "std::format path expression, method name joins group + method + tail with underscores"_test =
        ...
        expect(result.has_value()) << fatal;
        expect(result->contains(
            "void tasks_post_enqueue(std::string_view id, std::function<void()> onResponse, "
    ...
    "write: a request body emits a body param, content-type wiring, and a serialize call"_test = [] {
        ...
        expect(result.has_value()) << fatal;
        expect(result->contains(
            "void tasks_post(const dto_mod::TaskCreate &body, std::function<void()> onResponse, "
            "std::function<void(std::string)> onError = [](std::string) {})"));
    ...
    "write: a $ref response is qualified with the dto module and decoded via deserialize<T>"_test =
        ...
        expect(result.has_value()) << fatal;
        expect(result->contains(
            "void tasks_get(std::string_view id, std::function<void(dto_mod::TaskDef)> onResponse, "
            "std::function<void(std::string)> onError = [](std::string) {})"));
        expect(result->contains(
    ...
    "write: Integer/Number/Boolean/Object responses resolve like DtoWriter's own type mapping"_test =
        ...
        auto int_result = RouteWriter::write(std::vector<OperationInfo>{int_op}, "routes_mod",
                                             "dto_mod", "client");
        ...
        auto num_result = RouteWriter::write(std::vector<OperationInfo>{num_op}, "routes_mod",
                                             "dto_mod", "client");
        ...
        auto bool_result = RouteWriter::write(std::vector<OperationInfo>{bool_op}, "routes_mod",
                                              "dto_mod", "client");
        ...
        auto object_result = RouteWriter::write(std::vector<OperationInfo>{object_op},
                                                 "routes_mod", "dto_mod", "client");
    ...
    "write: groups sort alphabetically by method-name prefix, independent of insertion order"_test =
        [] {
        std::vector<OperationInfo> ops{make_operation("get", "/api/v1/users"),
                                       make_operation("get", "/api/v1/tasks")};
    ...
    "prefix and is silently dropped as a parameter -- looks like a bug, not fixed here"_test = [] {
        std::vector<OperationInfo> ops{make_operation("get", "/a/{id}/b"),
    ...
    "shell (setRuntime/dispatch, no route methods)"_test = [] {
        ...
        expect(result->contains("void dispatch(interfaces::io::IRequest &request, "
