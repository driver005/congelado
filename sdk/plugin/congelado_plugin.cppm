...
public:
     ...
     */
    [[nodiscard]] virtual std::string_view get_unique_type() const noexcept
    {
        return {};
    }
     ...
     */
    [[nodiscard]] virtual std::string_view get_type() const noexcept
    {
        return "plugin";
    }
     ...
     */
    [[nodiscard]] virtual std::string_view get_worker_type() const noexcept
    {
        return {};
    }
     ...
     * @note Default no-op returns an empty view — only plugins that override
     * `get_worker_type()` with a non-empty value are expected to actually do something here.
     * @param input the task's config key/value input view; unnamed param, unused in the
     * default.
     * @return the task's config key/value output view, empty by default.
     */
    [[nodiscard]] virtual CongeladoConfigView execute_worker(const CongeladoConfigView* /*input*/)
    {
        return {};
    }
    ...
    /// default no-op. Protocol plugins override this to stop accepting new connections and
    /// close every live one immediately, instead of waiting for `on_unload()` at full process
    /// teardown.
     ...
     */
    [[nodiscard]] virtual bool on_reload_requested() noexcept
    {
        return true;
    }
     ...
     */
    void error(std::string_view msg) noexcept final
    {
        logger_write(4, msg);
    }
};
...
namespace _cap_dispatch {
    ...
    template<typename T>
    concept has_logger_write =
        requires(T* plugin) { plugin->logger_write(int{}, std::string_view{}); };
    ...
    template<typename T>
    concept has_worker_orchestrator_get =
        requires(T* plugin) { plugin->worker_orchestrator_get(); };
     ...
     * actions (SERDE/STORAGE/PROTOCOL) hand back a raw interface pointer the host casts and
     * calls virtuals on directly, in-process — same as the old storage_get/protocol_get.
     * WRITE/ERROR (LOGGER) carry real marshaled data since ILogger is deliberately kept
     * non-templated/ABI-stable.
     */
    template<typename T>
    CongeladoAny call(
        T* plugin,
...
public:
    ...
    [[nodiscard]] int get_last_level() const noexcept
    {
        return m_last_level;
    }
