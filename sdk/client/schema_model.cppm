...
class SchemaType {
  public:
    ...
    [[nodiscard]] std::expected<void, std::string> parse(const serde::Value &element) {
        if (auto nullable = serde::Document::at(element, {"nullable"})) {
            if (auto value = nullable->to_bool()) {
                set_nullable(*value);
            }
        }

        if (auto reference = serde::Document::at(element, {"$ref"})) {
        ...
        auto type_value = serde::Document::at(element, {"type"});
        if (!type_value) {
        ...
        if (*kind == SchemaKind::OBJECT) {
            if (auto properties = serde::Document::at(element, {"properties"})) {
            ...
            auto items = serde::Document::at(element, {"items"});
            if (!items) {
