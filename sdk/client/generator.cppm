...
class Generator {
  public:
    ...
    generate(const std::filesystem::path &openapi_path,
        ...
        auto document = serde::Document::load(openapi_path);
        if (!document) {
  ...
  private:
    ...
    parse_components(const serde::Value &document) {
        ...
        auto named = serde::Document::at(document, {"components", "schemas"});
        if (!named) {
    ...
    parse_operation_response(OperationInfo &operation, const serde::Value &operation_element) {
        auto responses_value = serde::Document::at(operation_element, {"responses"});
        if (!responses_value) {
            return {};
        }
        ...
        for (auto &[status, response_element] : *responses) {
            ...
            if (auto response_schema = serde::Document::at(
    ...
    parse_operation(const std::string &path, const std::string &method,
        ...
        if (auto request_schema = serde::Document::at(
    ...
    parse_operations(const serde::Value &document) {
        ...
        auto paths_value = serde::Document::at(document, {"paths"});
        if (!paths_value) {
