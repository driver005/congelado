...
public:
     ...
     */
    explicit TaskInput(const std::unordered_map<std::string, std::string>& data) noexcept :
        m_data(data)
    {
    }
    ...
    [[nodiscard]] std::optional<T> get(std::string_view key) const
    {
        ...
        if constexpr (std::same_as<T, std::string>) {
            ...
            // Lifetime: returned view is valid only while the source map passed to TaskInput
            // lives.
    ...
    [[nodiscard]] const std::unordered_map<std::string, std::string>& get_data_map() const noexcept
    {
        return m_data;
    }
...
private:
    const std::unordered_map<std::string, std::string>&
        m_data; // FIXME(clang-tidy): cppcoreguidelines-avoid-const-or-ref-data-members —
                // deliberate: TaskInput is a zero-copy view over the caller's map (see ctor
                // warning above), switching to a copy or pointer would change the documented
                // lifetime contract
};
...
public:
    ...
    /// raw output map.
    [[nodiscard]] const std::unordered_map<std::string, std::string>& get_data() const noexcept
    {
        return m_data;
    }
...
public:
     ...
     */
    [[nodiscard]] virtual TaskOutput execute(const TaskInput& input) = 0;
...
public:
     ...
     */
    FfiWorker(std::string type, WorkerExecuteFn exec) :
     ...
     */
    [[nodiscard]] TaskOutput execute(const TaskInput& input) override
    {
...
public:
     ...
     */
    explicit TaskRunner(std::string_view workerId = {}) :
    ...
    void setWorkerId(std::string_view workerId)
    {
        m_worker_id = workerId;
    } // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention (camelCase after the prefix), not snake_case
     ...
     */
    void load_workers(
        const std::optional<std::filesystem::path>& external_directory = std::nullopt,
     ...
     */
    void addTaskWorker(ITaskWorker* worker)
    { // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention
     ...
     */
    [[nodiscard]] std::optional<TaskOutput>
    execute(std::string_view taskType, const TaskInput& input) const
    {
    ...
    [[nodiscard]] ITaskWorker* getTaskWorker(std::string_view taskType) const noexcept
    { // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention
        for (auto* entry: m_workers) {
    ...
    /// registration order.
    [[nodiscard]] std::vector<std::string_view> getTaskTypes() const noexcept
    { // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention
    ...
    [[nodiscard]] std::string_view getWorkerId() const noexcept
    {
        return m_worker_id;
    } // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention
...
{
public:
    explicit RecordingWorker(std::string type) :
        m_type{std::move(type)}
    {
    }
    ...
    void setShouldThrow(bool value) noexcept
    {
        m_should_throw = value;
    } // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention (camelCase after prefix)

    [[nodiscard]] bool getExecuted() const noexcept
    {
        return m_executed;
    } // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention (camelCase after prefix)

    [[nodiscard]] bool getReleased() const noexcept
    {
        return m_released;
    } // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention (camelCase after prefix)

    [[nodiscard]] bool getErrorFired() const noexcept
    {
        return m_error_fired;
    } // NOLINT(readability-identifier-naming) — matches this project's get/set/add accessor
      // naming convention (camelCase after prefix)
...
// buffer, regardless of the real input — every slot is a real, valid, non-null C string, so
// this is a safe OOB-*shaped* demonstration (no genuine out-of-bounds read happens), but it
// exercises detail::FfiWorker::execute()'s `for (i < out.count) result.set(out.keys[i],
// out.values[i])` loop (see that method's own doc) with a `count`/`keys`/`values` triple that's
// fully plugin-controlled and never cross-checked against anything — no cap on `count`, no
// check that it matches the real input size, nothing.
