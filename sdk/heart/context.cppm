...
public:
     ...
     * the top-level `threads` config key (see core::config::Config::get_threads); defaults
     * to 1.
     ...
     * still touch (e.g. via `core::plugin::SharedLibrary::close_all()`) — a worker thread can
     * be mid-job, reading plugin-owned objects (a protocol plugin's socket/`Endpoint`, say) at
     * the exact moment `on_unload()` frees them, which is a real use-after-free, not a
     * hypothetical one. No-op if already stopped.
     */
    ...
    [[nodiscard]] core::router::RouterContext<>* get_router() noexcept
    {
        return &m_router;
    }
    ...
    /// log line. `ServerRunner::run()` points the ambient `core::events::publish(...)` facade
    /// at this via `set_active()` right after construction.
    ...
    /// `core::otel::start_span(...)` facade at this via `set_active()` right after
    /// construction.
    ...
    /**
     * @brief Clean shutdown order: release all registered contracts first, then stop and join
     * the contract thread pool.
     */
    void stop() noexcept
    {
        if (m_connector) {
